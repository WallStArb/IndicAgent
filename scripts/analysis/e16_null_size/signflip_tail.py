...
raw = np.array(raw)
print(
    f"phi={phi} sims={sims} R={R} | raw P(p<=.01)={np.mean(raw<=.01):.5f} P(p<=.00167)={np.mean(raw<=.00167):.5f} hits={int(np.sum(raw<=.00167))}"
    f" | calibrated @.01={np.mean(cal[0.01]):.5f} @.00167={np.mean(cal[0.00167]):.5f} hits={int(np.sum(cal[0.00167]))}",
    flush=True,
)
