...
from src.intelligence.portfolio.weighting import (  # noqa: E402
    compute_mu,
    equal_weight_arm,
    ic_proportional_arm,
...
# A symbol enters a refit's covariance only with a current return and returns on >= 95% of the
# trailing window, admitted jointly so the complete rows clear the same bar; the fit uses those
# rows (instrument_covariance's docstring; spec revision 2026-09-24: the old
# _CORR_MIN_PERIODS admission with zero-filled gaps understated a new listing's variance, e.g.
# TLT at 46% coverage in the 2017 refit, sigma ~0.68x, vol_normalized weight ~1.47x).
_MIN_COVERAGE_FRACTION = 0.95
...
# ---------------------------------------------------------------------------
# Pure functions. The instrument-level primitives (score standardization, IC shrinkage, mu,
# realized-return covariance, the four arms, exposure stats) live in
# src/intelligence/portfolio/weighting.py, shared with the Phase 179 walk-forward harness;
# only the regime proxy and the walk-forward orchestration below are specific to this script.
# ---------------------------------------------------------------------------
...
def run_walk_forward(
    ...
    for pos, rebal_date in enumerate(dates):
        ...
        if pos - last_refit_pos >= _REFIT_EVERY_BARS:
            ...
            cov, cov_symbols = instrument_covariance(
                hist_returns, min_coverage_fraction=_MIN_COVERAGE_FRACTION
            )
        ...
        for arm in _ARM_FUNCS:
            if arm == "equal_weight":
                ...
                w, method, _cond = mean_variance_arm(
                    current_cov,
                    mu,
                    _MV_CONDITION_MAX,
                    ridge_epsilon_fraction=_RIDGE_EPSILON_FRACTION,
                )
