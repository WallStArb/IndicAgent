...
# ---------------------------------------------------------------------------
# Tests 6-11: Live OHLCV batch write path
# ---------------------------------------------------------------------------


def test_queue_ohlcv_write_1m_bar():
    """1m bar must be appended to _ohlcv_buffer."""
    from datetime import UTC, datetime
    ...
    svc = FeaturePipelineService.__new__(FeaturePipelineService)
    svc._ohlcv_buffer: list = []
    svc._ohlcv_last_flush: float = 0.0
    ...
    assert len(svc._ohlcv_buffer) == 1


def test_queue_ohlcv_write_skips_htf():
    """5m, 15m, 1h bars must NOT be appended to _ohlcv_buffer."""
    from datetime import UTC, datetime
    ...
    assert len(svc._ohlcv_buffer) == 0


def test_queue_ohlcv_write_tuple_structure():
    """Queued tuple must be 8 elements matching INSERT column order."""
    from datetime import UTC, datetime
    ...
    tup = svc._ohlcv_buffer[0]
    assert len(tup) == 8, f"Expected 8-element tuple, got {len(tup)}"
    # Column order: timestamp, symbol, timeframe, open, high, low, close, volume
    assert tup[0] == ts
    assert tup[1] == "ES"
    assert tup[2] == "1m"
    assert tup[3] == 5100.0
...
def test_flush_ohlcv_calls_executemany():
    """_flush_ohlcv() must call executemany with _INSERT_OHLCV_SQL and buffer rows."""
    import asyncio
    from datetime import UTC, datetime
    from unittest.mock import AsyncMock, MagicMock

    from services.feature_pipeline_service import _INSERT_OHLCV_SQL, FeaturePipelineService
    ...
    svc.logger = MagicMock()
    svc.ohlcv_writes_total = MagicMock()
    ...
    assert call_args[0][0] == _INSERT_OHLCV_SQL
    assert len(call_args[0][1]) == 1


def test_flush_ohlcv_clears_buffer_on_success():
    """_ohlcv_buffer must be empty after a successful _flush_ohlcv()."""
    import asyncio
    from datetime import UTC, datetime
    from unittest.mock import AsyncMock, MagicMock
    ...
    assert svc._ohlcv_buffer == [], "Buffer must be cleared after successful flush"
    svc.ohlcv_writes_total.inc.assert_called_once_with(2)


def test_flush_ohlcv_retains_buffer_on_failure():
    """_ohlcv_buffer must remain intact after a DB error in _flush_ohlcv()."""
    import asyncio
    from datetime import UTC, datetime
    from unittest.mock import AsyncMock, MagicMock
    ...
    mock_conn = AsyncMock()
    mock_conn.executemany = AsyncMock(side_effect=Exception("DB connection refused"))
    mock_pool = MagicMock()
    mock_pool.acquire = MagicMock(
        return_value=AsyncMock(
            __aenter__=AsyncMock(return_value=mock_conn),
    ...
    assert len(svc._ohlcv_buffer) == 1, "Buffer must be retained after DB error"
    assert svc._ohlcv_buffer[0] == original_row
    svc.ohlcv_writes_total.inc.assert_not_called()
