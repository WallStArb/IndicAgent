...
def _run_hook(tmp_path: Path, cells: list[dict]) -> None:
    ...
    concept = _make_concept_registry({name: {"status": "active"} for name in feature_names})
    _run_lifecycle_hook(conn, concept, config, _T1, _FakeManifest(tmp_path))
