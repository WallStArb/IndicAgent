...
The legacy Redis signal age filter (_signal_entry_stale / _signal_max_age_s) was
removed in the fix for signals-not-loading-on-refresh. The Kafka-based SSE broadcaster
uses latest-per-key state with no age filtering; signals are pre-populated via the
REST /signals/active endpoint on mount instead.
"""
