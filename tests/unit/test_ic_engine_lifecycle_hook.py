...
not just the Python-side aggregation.
"""
...
class _FakeLifecycleConn:
    ...
    def __init__(
        ...
        # that the facts exist. See ConceptRegistryService's real conn.transaction()
        # commit-on-exit semantics, mirrored below by _FakeConceptRegistryService.
...
class _FakeConceptRegistryService:
    ...
    hook calls (Phase 170 Plan 08: concept_svc is the sole feature-lifecycle
    registry). Uses the concept-table sync API's keyword-only argument names
    (domain/name/from_status/to_status/reason/gate_metric/gate_n/ci_lower/
    fdr_passed).
    """

    def __init__(self, concepts: dict[str, dict]):
        # concepts: name -> {"status": str, "eligible": bool}
    ...
    def record_transition_sync(
        ...
        *,
        ...
        # Real ConceptRegistryService.record_transition_sync wraps its SQL in
        # conn.transaction(), which commits on clean exit -- mirrored here so
        # tests can verify _run_lifecycle_hook's deferred guard-fact writes
        # actually survive this commit rather than assuming it (todo 144
        # /simplify pass).
        conn.commit()
...
def _make_concept_registry(features: dict[str, dict]) -> _FakeConceptRegistryService:
    """Build a fake concept registry from a status dict (Phase 170 Plan 08:
    the single remaining registry after feature_registry's retirement).
    Independent dict copy so a test mutating the fake's internal state never
    accidentally mutates the caller's fixture dict."""
    return _FakeConceptRegistryService({k: dict(v) for k, v in features.items()})
...
def test_demotion_triggers_on_materially_failing_active_feature(tmp_path):
    ...
    concept = _make_concept_registry({"featA": {"status": "active"}})
    ...
    assert call[1] == "featA"
    assert call[2] == "active"
    assert call[3] == "shadow_only"
    assert call[4] == "demotion_performance"
    assert all(c[3] != "deprecated" for c in concept.transition_calls)
...
def test_promotion_when_eligible_zero_ensemble_weights_writes(tmp_path):
    ...
    concept = _make_concept_registry({"featB": {"status": "shadow_only", "eligible": True}})
...
def test_no_promotion_when_ineligible(tmp_path):
    ...
    conn = _FakeLifecycleConn(cells)
    concept = _make_concept_registry({"featB": {"status": "shadow_only", "eligible": False}})
...
def test_regime_shift_cold_start_within_seeded_rails_does_not_hold(tmp_path):
    ...
    assert len(concept.transition_calls) == 1
    assert concept.transition_calls[0][1:5] == (
        "featA",
        "active",
        "shadow_only",
...
def test_regime_shift_small_stratum_never_hold_authoritative(tmp_path):
    ...
    assert len(concept.transition_calls) == 1  # demotion proceeded, guard didn't block it
    assert len(conn.guard_fact_inserts) == 1
...
def test_regime_shift_suspiciously_low_fail_rate_alerts_not_holds(tmp_path):
    ...
    concept = _make_concept_registry({"featB": {"status": "active"}})
...
def test_regime_shift_two_independent_strata_only_failing_one_holds(tmp_path):
    ...
    concept = _make_concept_registry({"featA": {"status": "active"}, "featB": {"status": "active"}})
    ...
    assert concept.transition_calls == []  # global hold, even though only one stratum tripped
...
def test_guard_facts_deferred_past_registry_triggered_commit(tmp_path):
    ...
    ConceptRegistryService.record_transition_sync/advance_shadow_counters_sync
    each wrap their own SQL in conn.transaction() on the SAME write_conn, which
    commits immediately on exit -- eagerly writing guard facts inside Step 3's
    loop (the original, buggy shape) would let the FIRST such registry call
    silently commit or roll back those facts before the hook's own intended
    single commit point. The fix defers the guard-fact writes past Step 4/5
    entirely.
    ...
    concept registry's commit-on-exit simulation (see _FakeConceptRegistryService
    above) mid-run. Asserts the ordering directly: the registry-triggered commit
    must appear in the event log BEFORE the guard-fact INSERT, proving the write
    was deferred past it rather than landing eagerly beforehand. A regression
    back to eager per-stratum writes (todo 144's original defect) would reverse
    this order and fail this assertion, even though every other test in this
    file (whose fakes only check final state, not commit ordering) would still
    pass."""
...
def test_regime_shift_empirical_band_takes_over_after_min_history(tmp_path):
    ...
    assert concept.transition_calls == []  # held: 0.99 is far outside the tight empirical band
...
def test_zero_cell_run_does_not_short_circuit_a_later_run_with_cells(tmp_path):
    ...
    concept = _make_concept_registry({"featA": {"status": "active"}})
    _run_lifecycle_hook(empty_conn, concept, config, _T1, _make_manifest(tmp_path))
    ...
    _run_lifecycle_hook(conn2, concept, config, _T1, _make_manifest(tmp_path))
    # One decay_cells_flagged fact per non-hold run.
    assert len(conn2.integrity_inserts) == 1


# ---------------------------------------------------------------------------
# integrity_monitor facts per normal run (decay_cells_flagged)
# ---------------------------------------------------------------------------
...
def test_sign_symmetric_significant_contrarian_not_demoted(tmp_path):
    ...
    conn = _FakeLifecycleConn(cells, ensemble_weight_rows=ew)
    concept = _make_concept_registry({"featC": {"status": "active"}})

    _run_lifecycle_hook(conn, concept, config, _T1, _make_manifest(tmp_path))

    assert (
        concept.transition_calls == []
...
def test_sign_symmetric_off_positive_cell_decision_is_byte_identical(tmp_path):
    ...
    assert len(concept.transition_calls) == 1
    call = concept.transition_calls[0]
    assert call[1:5] == ("featA", "active", "shadow_only", "demotion_performance")
