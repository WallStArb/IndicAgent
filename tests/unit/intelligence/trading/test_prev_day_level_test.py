...
def test_fade_higher_confidence_in_ranging_regime():
    ...
    features_ranging = _base_features(
        atr_14=10.0,
        prior_session_high=5050.0,
        prior_session_low=4950.0,
        prior_session_close=5000.0,
...
def test_continuation_higher_confidence_in_trend_regime():
    """Continuation variant + hmm_regime=1.0 (trending) has higher confidence than hmm_regime=0.0."""
    from src.intelligence.trading.prev_day_level_test import PrevDayLevelTestPlugin

    # First fire: breakout above PDH to set state
    close_breakout = np.full(25, 5065.0)  # Above PDH=5050 + 5 proximity
    open_arr_bo = np.full(25, 5060.0)

    # Second fire: pullback to within 0.5xATR of PDH from above -> continuation
    close_pullback = np.full(25, 5052.0)  # Within 5 of PDH=5050, above PDH
    open_arr_pb = np.full(25, 5049.0)  # close > open = bullish bar at PDH from above

    features_trend = _base_features(
        atr_14=10.0,
        prior_session_high=5050.0,
        prior_session_low=4950.0,
        prior_session_close=5000.0,
