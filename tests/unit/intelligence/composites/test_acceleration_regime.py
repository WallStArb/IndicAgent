...
def test_accel_regime_building_when_score_above_0_5():
    ...
    result = plugin.compute_next(make_frames(
        rsi_curvature=1.0, macd_hist_slope=0.5, price_accel=0.3, hma_accel=0.4,
    ))
...
def test_accel_regime_neutral_when_score_near_zero():
    ...
    result = plugin.compute_next(make_frames(
        rsi_curvature=0.1, macd_hist_slope=-0.1, price_accel=0.05, hma_accel=-0.05,
    ))
...
def test_accel_regime_peak_fires_once_on_inflection_bar():
    ...
    result = plugin.compute_next(make_frames(
        rsi_curvature=-0.1, macd_hist_slope=-0.1, price_accel=-0.1, hma_accel=-0.1,
    ))
...
def test_accel_score_is_float_in_range():
    ...
    plugin = AccelerationRegimePlugin()
    result = plugin.compute_next(make_frames(
        rsi_curvature=0.5, macd_hist_slope=0.3, price_accel=0.2, hma_accel=0.1,
    ))
