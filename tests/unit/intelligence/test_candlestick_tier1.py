...
def _three_white_soldiers(base: float = 100.0) -> dict:
    ...
    return _frames([
        ...
        {"open": base, "high": base + 2.0, "low": base - 0.2, "close": base + 1.8, "volume": 1000},
...
def _three_black_crows(base: float = 110.0) -> dict:
    ...
    return _frames([
        ...
        {"open": base, "high": base + 0.2, "low": base - 2.0, "close": base - 1.8, "volume": 1000},
        # p: opens within pp body (between pp_c and pp_o), closes near low
        {"open": base - 1.5, "high": base - 1.3, "low": base - 3.8, "close": base - 3.5, "volume": 1200},
        # c: opens within p body, closes near low
        {"open": base - 3.2, "high": base - 3.0, "low": base - 5.8, "close": base - 5.5, "volume": 1100},
    ])
...
def _morning_star(base: float = 100.0) -> dict:
    ...
    return _frames([
        ...
        {"open": base + 5.0, "high": base + 5.2, "low": base + 0.1, "close": base + 0.5, "volume": 1000},
        # p: small body (star), body < 0.3 * range
        {"open": base + 0.4, "high": base + 1.0, "low": base - 0.5, "close": base + 0.5, "volume": 800},
        # c: large bullish, c_c > (pp_o + pp_c) / 2 = (base+5 + base+0.5)/2 = base+2.75
        {"open": base + 0.6, "high": base + 4.0, "low": base + 0.3, "close": base + 3.5, "volume": 1200},
    ])
...
def _evening_star(base: float = 100.0) -> dict:
    ...
    return _frames([
        ...
        {"open": base, "high": base + 5.1, "low": base - 0.1, "close": base + 4.8, "volume": 1000},
        # p: small body (star), body < 0.3 * range
        {"open": base + 4.9, "high": base + 5.5, "low": base + 4.1, "close": base + 5.0, "volume": 800},
        # c: large bearish, c_c < (pp_o + pp_c) / 2 = (base + base+4.8)/2 = base+2.4
        {"open": base + 4.7, "high": base + 4.8, "low": base + 0.3, "close": base + 1.5, "volume": 1200},
    ])
...
def _three_inside_up(base: float = 100.0) -> dict:
    ...
    return _frames([
        ...
        {"open": base + 4.0, "high": base + 4.2, "low": base - 0.2, "close": base + 0.2, "volume": 1000},
        ...
        {"open": base + 0.8, "high": base + 2.8, "low": base + 0.5, "close": base + 2.5, "volume": 900},
        # c: bullish, c_c > pp_o (base+4.0)
        {"open": base + 2.6, "high": base + 4.8, "low": base + 2.4, "close": base + 4.5, "volume": 1100},
    ])
...
def _three_inside_down(base: float = 100.0) -> dict:
    ...
    return _frames([
        ...
        {"open": base, "high": base + 4.2, "low": base - 0.2, "close": base + 4.0, "volume": 1000},
        ...
        {"open": base + 3.0, "high": base + 3.3, "low": base + 1.2, "close": base + 1.5, "volume": 900},
        # c: bearish, c_c < pp_o (base)
        {"open": base + 1.4, "high": base + 1.6, "low": base - 0.8, "close": base - 0.5, "volume": 1100},
    ])
...
def _harami_cross(base: float = 100.0) -> dict:
    ...
    return _frames([
        ...
        # p_open ≈ p_close (doji); body=0.1, range=1.0 → ratio=0.1 (just at threshold, use 0.09 ratio)
        ...
        {"open": base + 2.0, "high": base + 2.5, "low": base + 1.5, "close": base + 2.09, "volume": 800},
        # c: not used for harami_cross detection (only pp and p matter)
        {"open": base + 2.1, "high": base + 2.8, "low": base + 1.8, "close": base + 2.5, "volume": 900},
    ])
...
def _dark_cloud_cover(base: float = 100.0) -> dict:
    ...
    return _frames([
        ...
        {"open": base + 4.0, "high": base + 4.3, "low": base + 3.8, "close": base + 4.1, "volume": 900},
        ...
        {"open": base + 4.5, "high": base + 4.6, "low": base + 1.6, "close": base + 1.8, "volume": 1200},
    ])
...
def _piercing_line(base: float = 100.0) -> dict:
    ...
    return _frames([
        ...
        # filler p bar
        {"open": base, "high": base + 0.3, "low": base - 0.4, "close": base - 0.1, "volume": 900},
        ...
        {"open": base - 0.5, "high": base + 2.4, "low": base - 0.7, "close": base + 2.2, "volume": 1200},
    ])
...
class TestThreeBarTier1Patterns:
    ...
    def test_three_white_soldiers(self, plugin: CandlestickPatternsPlugin) -> None:
        ...
        assert result.get("three_white_soldiers") == 1.0, (
    ...
    def test_three_black_crows(self, plugin: CandlestickPatternsPlugin) -> None:
        ...
        assert result.get("three_black_crows") == 1.0, (
    ...
    def test_morning_star(self, plugin: CandlestickPatternsPlugin) -> None:
        ...
        assert result.get("morning_star") == 1.0, (
    ...
    def test_evening_star(self, plugin: CandlestickPatternsPlugin) -> None:
        ...
        assert result.get("evening_star") == 1.0, (
    ...
    def test_three_inside_up(self, plugin: CandlestickPatternsPlugin) -> None:
        ...
        assert result.get("three_inside_up") == 1.0, (
    ...
    def test_three_inside_down(self, plugin: CandlestickPatternsPlugin) -> None:
        ...
        assert result.get("three_inside_down") == 1.0, (
    ...
    def test_harami_cross(self, plugin: CandlestickPatternsPlugin) -> None:
        ...
        assert result.get("harami_cross") == 1.0, (
    ...
    def test_dark_cloud_cover(self, plugin: CandlestickPatternsPlugin) -> None:
        ...
        assert result.get("dark_cloud_cover") == 1.0, (
    ...
    def test_piercing_line(self, plugin: CandlestickPatternsPlugin) -> None:
        ...
        assert result.get("piercing_line") == 1.0, (
...
class TestExistingPatternsUnchanged:
    ...
    def test_engulfing_bull_still_fires(self, plugin: CandlestickPatternsPlugin) -> None:
        ...
        frames = _frames([
            {"open": 102.0, "high": 102.5, "low": 101.0, "close": 101.5, "volume": 1000},  # pp (filler)
            {"open": 101.0, "high": 101.5, "low": 99.0, "close": 99.5, "volume": 1000},   # p: bearish
            {"open": 98.5, "high": 102.5, "low": 98.0, "close": 102.0, "volume": 2000},   # c: bullish engulf
        ])
    ...
    def test_doji_still_fires(self, plugin: CandlestickPatternsPlugin) -> None:
        ...
        frames = _frames([
            {"open": 100.0, "high": 101.0, "low": 99.0, "close": 100.5, "volume": 1000},  # pp filler
            {"open": 100.5, "high": 101.0, "low": 99.5, "close": 100.2, "volume": 1000},  # p filler
...
class TestNoPatternOnRandom:
    ...
    def test_flat_bars_no_three_bar_pattern(self, plugin: CandlestickPatternsPlugin) -> None:
        ...
        new_pattern_keys = [
            ...
            "three_inside_up", "three_inside_down", "harami_cross", "dark_cloud_cover",
            "piercing_line",
        ]
        for key in new_pattern_keys:
            assert result.get(key, 0.0) == 0.0, f"Expected {key}=0.0 for flat bars, got {result.get(key)}"

    def test_outputs_frozenset_contains_all_new_fields(self, plugin: CandlestickPatternsPlugin) -> None:
