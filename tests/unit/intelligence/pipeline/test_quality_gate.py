...
@pytest.mark.asyncio
async def test_applies_min_hurst_entropy_times_drift():
    ...
    result = await apply_quality_gate([sig], thresholds)
...
@pytest.mark.asyncio
async def test_defaults_to_one_on_missing_thresholds():
    """Empty thresholds → confidence unchanged, quality_score=1.0."""
    sig = make_signal(confidence=0.8)
    result = await apply_quality_gate([sig], {})
...
@pytest.mark.asyncio
async def test_hurst_lower_than_entropy_uses_hurst():
