...
def test_returns_none_on_empty_list():
    ...
    cis = MagicMock()
    winner, all_ranked, method = select_winner([], cis)
...
def test_no_active_returns_none():
    ...
    winner, all_ranked, method = select_winner(signals, cis)
    assert winner is None
...
def test_cis_override_selects_matching():
    ...
    cis = MagicMock()
    cis.score.return_value = MagicMock(direction=1)
    signals = [
...
def test_fallback_majority_direction():
    """No CIS → majority direction wins, sorted by adjusted_rank."""
    cis = MagicMock()
    cis.score.return_value = None
    signals = [
        make_signal(direction=1, plugin="trad_TrendFollowing", adjusted_rank=3),
...
def test_confidence_boost_per_agree():
    ...
    winner, _, method = select_winner(signals, cis)
...
def test_cis_direction_no_match_falls_back():
    ...
    cis = MagicMock()
    cis.score.return_value = MagicMock(direction=-1)  # No -1 signals
    signals = [
        make_signal(direction=1, plugin="trad_TrendFollowing", adjusted_rank=3),
        make_signal(direction=1, plugin="trad_MTFAlignment", adjusted_rank=4),
    ]
...
def test_cis_score_exception_falls_back():
    """CIS scorer exception → falls back gracefully."""
    cis = MagicMock()
    cis.score.side_effect = Exception("CIS error")
