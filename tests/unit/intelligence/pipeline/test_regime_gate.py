...
@pytest.mark.asyncio
async def test_wrong_regime_suppresses():
...
@pytest.mark.asyncio
async def test_matching_regime_passes():
...
@pytest.mark.asyncio
async def test_any_regime_type_always_passes():
...
async def test_does_not_mutate_input():
    ...
    sig = make_signal()
    original = dict(sig)
    await apply_regime_gate([sig], None)
    assert sig == original
...
@pytest.mark.asyncio
async def test_safety_floor_allows_previously_suppressed_signals():
    ...
    result = await apply_regime_gate([sig], regime_data, prob_min=0.30, dur_min=1)
    assert result[0]["regime_eligible"] is True
    assert result[0]["suppression_reason"] is None


@pytest.mark.asyncio
async def test_safety_floor_suppresses_below_floor():
...
@pytest.mark.asyncio
async def test_dur_min_1_suppresses_duration_0():
...
@pytest.mark.asyncio
async def test_custom_prob_min_threads_correctly():
    ...
    result = await apply_regime_gate([sig], regime_data, prob_min=0.70, dur_min=1)
    assert result[0]["regime_eligible"] is False
    assert result[0]["suppression_reason"] == "regime_prob"


@pytest.mark.asyncio
async def test_regime_map_unchanged_with_parametric_gate():
...
@pytest.mark.asyncio
async def test_default_settings_have_safety_floor_thresholds():
