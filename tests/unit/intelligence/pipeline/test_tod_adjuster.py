...
@pytest.mark.asyncio
async def test_clamps_multiplier_low():
...
async def test_does_not_mutate_input():
    ...
    await apply_tod_adjustment([sig], {("trend", "1m", 9, "*"): 1.2}, "1m", 9)
...
async def test_symbol_specific_key_tried_first():
    ...
    result = await apply_tod_adjustment([sig], tod_table, "1m", 9, symbol="ES")
...
@pytest.mark.asyncio
async def test_symbol_no_key_falls_to_session_priors():
    """When neither symbol-specific nor global '*' key exists, falls to session priors."""
    sig = make_signal(confidence=0.8, regime_type="trend")
    result = await apply_tod_adjustment([sig], {}, "1m", 9, symbol="ES")
    assert result[0]["tod_multiplier"] == 1.10


@pytest.mark.asyncio
async def test_symbol_tod_multiplier_stamped():
