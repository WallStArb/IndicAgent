...
class TestComputeSetupPerformanceBasicBehavior:
    ...
    def test_compute_setup_performance_at_threshold_included(self):
        """Exactly n=100 resolved signals returns a row with required keys."""
        signals = _recent_signals("trad_TrendFollowing", [0.5] * 100)
    ...
    def test_compute_setup_performance_above_threshold(self):
        """n=120 resolved signals returns a populated entry."""
        signals = _recent_signals("trad_MeanReversion", [1.0] * 100 + [-0.5] * 20)
        result = compute_setup_performance(signals)
    ...
    def test_compute_setup_performance_win_rate_calculation(self):
        ...
        result = compute_setup_performance(signals)
        assert "trad_TrendFollowing" in result
        assert result["trad_TrendFollowing"]["win_rate"] == pytest.approx(80 / 100)
    ...
    def test_compute_setup_performance_sharpe_ratio_nonzero(self):
        ...
        random.seed(42)
        pnl_rs = [round(random.gauss(0.3, 1.0), 4) for _ in range(100)]
    ...
    def test_compute_setup_performance_multiple_setups(self):
        """Two setup_plugins each with n>=100 → both appear in result dict."""
        signals_a = _recent_signals("trad_TrendFollowing", [0.5] * 100)
        signals_b = _recent_signals("trad_MeanReversion", [0.3] * 105)
...
class TestPromotionGate:
    ...
    def test_compute_setup_performance_below_threshold_returns_empty(self):
        """n=50 resolved signals for a setup → returns empty dict (FEED-02 gate)."""
        signals = _recent_signals("trad_TrendFollowing", [1.0] * 50)
        result = compute_setup_performance(signals)
        assert result == {}

    @pytest.mark.unit
    def test_compute_setup_performance_mixed_threshold(self):
        """One setup n=100, another n=50 → only the n=30 setup appears in result."""
        signals_pass = _recent_signals("trad_TrendFollowing", [0.5] * 100)
        signals_fail = _recent_signals("trad_MeanReversion", [0.5] * 50)
...
class TestWindowAndNullHandling:
    ...
    def test_compute_setup_performance_30day_window(self):
        ...
        # 25 old signals (excluded) + 100 recent signals = 100 eligible
        old_signals = [
            _make_resolved_signal("trad_TrendFollowing", 1.0, resolved_at=old_resolved_at)
            for _ in range(25)
        ]
        recent_signals = _recent_signals("trad_TrendFollowing", [0.5] * 100)
        result = compute_setup_performance(old_signals + recent_signals)
        # Should be included (100 recent meet threshold)
        assert "trad_TrendFollowing" in result
        # Old signals are excluded so sample_size == 100, not 125
        assert result["trad_TrendFollowing"]["sample_size"] == 100

    @pytest.mark.unit
    def test_compute_setup_performance_null_pnl_r_excluded(self):
        """Rows with pnl_r=None are not counted toward sample_size."""
        # 10 None + 100 valid = 100 eligible (None rows excluded)
        signals = _recent_signals(
            "trad_TrendFollowing",
            [None] * 10 + [1.0] * 100,
        )
