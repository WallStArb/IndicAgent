...
class TestOFIPlugin:
    ...
    def test_divergence_sign(self):
        """When price goes up but OFI is negative (sell pressure), ofi_divergence is negative."""
        # Create df where close rises but sell ticks dominate
        df = _make_df(20, base_close=100.0)
        # Force sell pressure via tick_buf: price goes down → sell vol
        tick_buf = [
            {"price": "100", "size": "10"},
            {"price": "99", "size": "50"},  # big sell
            {"price": "98", "size": "50"},  # more sell
        ]
        result = self.plugin.compute_full({"main": df, "tick_buffer": tick_buf})
        divergence = result.get("ofi_divergence")
        assert divergence is not None
        assert isinstance(divergence, float)
        # Range must be within [-2, 2]
        assert -2.0 <= divergence <= 2.0
