...
_ALLOW_LIST: dict[str, str] = {
    ...
    "services/ic_engine.py": (
        "PERMANENT: _FEATURE_STATUS_REFRESH_SQL, a raw UPDATE feature_ic_scores call site "
        "genuinely exposed to the same forced-full-scan cost as every other entry here -- "
        "deliberately deferred out of the 2026-08-14 sweep that added this guard (ic_engine.py "
        "is this codebase's largest, most heavily-relied-upon, already-audited write path, "
        "deserving its own focused session), fixed 2026-08-20 (todo 307): wrapped in "
        "compressed_hypertable_write_session, same pattern as the three entries above. Stays on "
        "the allow-list, not removed -- wrapping the write in a session doesn't remove the "
        "literal 'UPDATE feature_ic_scores' text this guard's regex matches, same as the three "
        "already-wrapped PERMANENT entries above. The file's OTHER raw-UPDATE call site "
        "(_backfill_bh_fdr's bh_adjusted_p/passes_fdr pass) was migrated to bulk_update_by_key "
        "in the same fix (altitude review finding -- matches scripts/ops/alpha/ops_ic_shrinkage."
        "py's own bulk_update_by_key call against this exact table/key shape), which builds its "
        "UPDATE dynamically and so no longer matches this guard's regex at all -- structurally "
        "protected via bulk_update_by_key's own compressed-hypertable guard now, not via this "
        "allow-list."
    ),
