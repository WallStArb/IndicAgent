    ...
    async def _phase_a_insert(self) -> int:
        ...
        async with self._pool.acquire() as conn:
            ...
            SELECT
                ...
                f.symbol,
                f.tf,
                (tf_sig.value->>'setup_plugin')::text,
                sl.pnl_r,
                (sl.pnl_r > 0),
                sl.outcome,
                (tf_sig.value->>'confidence')::float8,
                (tf_sig.value->'ctf_confluence'->>'ctf_score')::float8,
                (tf_sig.value->'ctf_confluence'->>'trend_alignment')::float8,
                (tf_sig.value->'ctf_confluence'->>'structure_alignment')::float8,
                (tf_sig.value->'ctf_confluence'->>'regime_agreement')::float8,
                (tf_sig.value->'ctf_confluence'->>'fvg_alignment')::float8,
                (tf_sig.value->'ctf_confluence'->>'ob_alignment')::float8,
                (f.regime_features->>'vix_level')::float8,
                (f.regime_features->>'vix_z')::float8,
                (f.regime_features->>'eq_spread_z')::float8,
                (f.regime_features->>'eq_pairs_confirming')::float8,
                (f.regime_features->>'ctf_momentum_divergence')::float8,
                (f.regime_features->>'ctf_sr_confluence')::float8,
                (f.regime_features->>'ctf_hmm_regime_agreement')::float8,
                (f.regime_features->>'ctf_volatility_divergence')::float8,
                (f.regime_features->>'ctf_orderflow_alignment')::float8,
                (f.regime_features->>'exhaustion_score')::float8,
                (tf_sig.value->>'profile')::text,
