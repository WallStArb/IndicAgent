"""trad_ORB15 — Opening Range Breakout (15-min window) evidence-contributor.

Accumulates the high/low of bars from 09:30-09:45 ET to define the opening range.
...
- Instrument everything: gap bias and regime context captured in every signal
"""
...
# Range accumulation window for ORB15: 09:30-09:45 ET
_RANGE_START = (9, 30)
_RANGE_END = (9, 45)
...
@dataclass
class ORB15Plugin:
    """I7 evidence contributor: fires on first post-09:45 breakout beyond the 15-min opening range.

    Range: max(high) / min(low) over 09:30-09:44 ET bars.
    ...
    def compute_full(self, frames: dict[str, Any]) -> dict[str, Any]:
        ...
        # ── Range accumulation: 09:30-09:44 ET ──────────────────────────────
        ...
        # Condition: we are past RANGE_END and range is not yet marked complete
        if not state.get("range_complete"):
            ...
            if (
                ...
                # Range never accumulated (e.g., session started after range window)
                self._state[(symbol, tf)] = state
                return no_signal()
        ...
        if close_price > orb_high:
            direction = 1
            signal_type = "orb15_breakout_long"
        elif close_price < orb_low:
            direction = -1
            signal_type = "orb15_breakout_short"
        else:
            self._state[(symbol, tf)] = state
            return no_signal()

        # ── Volume gate ──────────────────────────────────────────────────────
        # Prefer rel_volume from features if available
