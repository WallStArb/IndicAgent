...
**Pipeline (v3.0):** `IBKR TWS → FeatureVectorPipeline (compute) → FeatureVectorWriter → feature_vectors → forward_return_writer → ic_engine → feature_lifecycle (concept_evaluation → concept_registry) → ensemble_trainer/EnsembleICEngine (alpha_ensemble_ic) → alpha_publisher → alpha_events`. `alpha_publisher` is the sole `alpha_events` writer.
...
Where each instrument sits in a single-parent tree (asset class > sector > industry group > industry) lives in `classification_scheme`/`classification_node`/`instrument_classification` (migration 364, Phase 182), a third registry separate from ITR (falsifiable claims) and CVR (flat codes). One scheme today, `indicagent_v1` (migration 365, 118 nodes, every `instruments` row assigned): project-owned, authority `IndicAgent`, equity levels named after the public GICS structure but not GICS. Point-in-time and append-only in effect (reclassify = close `valid_to` + insert, one current row per symbol, `ON DELETE RESTRICT`, node parent/level immutable); no history before 2026-09-25. Read through `ClassificationService` (`src/config/classification_service.py`, cached at init, returns `indicagent_v1:unclassified`, never None) or `current_level_name_sql()` for synchronous SQL builders. `Instrument.sector` is the level-2 node name; `contract_details->>'sector'` is historical only, never a source of truth. Coverage is enforced by the seed guard, the `onboard_instrument()` classification requirement and the nightly `classification_coverage` audit; its integration tests are local-only (not GitHub-CI-enforced). Full spec: `docs/foundation/security-classification-hierarchy.md`.

## Unified Concept Registry (UCR)

Evidence-gated lifecycle governance for research artifacts (features, ensemble strategies) — `concept_registry`/`concept_gate`/`concept_transition_log`/`concept_annotation`/`concept_parent`, governs recipes not their outputs (a feature definition is governed here; its computed values in `feature_vectors` are not). Four-state lifecycle `candidate → shadow_only → active → deprecated`; the ONLY code path that flips an *existing* concept's `status` is `ConceptRegistryService` (`src/intelligence/concept_registry_service.py`) — no LLM, no proposer override, ever (Invariant 1). Exempt: migration-time genesis seeding — a migration adding a new `FeatureVector` field may INSERT its `concept_registry` row with `status='active'` directly (schema-definition-time DDL establishing a new concept, not a runtime lifecycle transition on an existing row); established practice across 5 migrations (288/289/290/291/316). Both domains are live: `domain='ensemble_strategy'` (async `record_comparison_outcome()`, called by `ops_ensemble_weight_compare.py`) and `domain='feature'` (row count tracks `FeatureVector`'s field count 1:1, re-check live rather than citing a number here — it moves every time a feature is added; sole writer is the `feature_lifecycle` node (`services/feature_lifecycle.py`, chained after `ic_engine`), which derives status from the append-only `concept_evaluation` ledger (migration 357, todo 402) and flips it through async `record_transition()`). **Phase 170 complete 2026-08-10** (migration 311): `feature_registry`/`feature_transition_log` were `DROP`ped, `FeatureRegistryService` deleted — `concept_registry` is the sole feature-lifecycle system now, no parallel table remains. Full spec: `docs/foundation/unified-concept-registry.md`.
