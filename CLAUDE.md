...
Version: 5.41.0 | Status: v2.5 PARTIAL — Phases 69+71+72+73+74+75+76+77+78+79 shipped (merged to main); Phase 70 deferred (~May 10 data gate). Phase 64 core complete; 03C+04 deferred (~May 10 data gate). Next: Phase 70 (ML Scoring) after ~May 10 data gate. Phase 79 signal quality fix merged — activation rate should jump from 0.3% to meaningful levels on next trading day.
...
- **`KafkaProducerClient.publish()` kwarg is `msg=`**: `publish(topic, msg=row)` — not `value=`. Wrong kwarg silently raises `TypeError` at flush time with no data written.
- **`BaseGroupService` agent construction**: agents that need `self._llm_chain` must be constructed in `_setup()` after `super()._setup()` — `_llm_chain` is `None` in `__init__`. Pattern: `self._agents: list[BaseAIAgent] = []` in `__init__`, populate in `_setup()`.
- **`LineageRecorder` lifecycle**: requires `await recorder.start()` after construction to activate the periodic flush loop; `await recorder.stop()` in teardown drains the buffer and cancels the task. Without `start()`, records only flush when batch hits `batch_size` or on shutdown.
