...
**Principles:** Instrument everything · shadow mode first · data quality over model complexity · never drop data that could contain signal · earn promotion through proof (p<0.05, sufficient N) · segment by regime · automate manual tasks · let the system run. Full doc: `docs/foundation/principles.md`.
**Design mindset:** Think as a council of senior engineers at Renaissance Technologies. Ruthlessly eliminate complexity. Clean DAG topology — every node does one thing, data flows one direction. SoC: compute ≠ persistence ≠ transport. Async-first — blocking I/O in the hot path is a defect. Guard against silent failures. Reuse over duplication. Before committing to a design: (1) survives 10x volume? (2) what fails silently? (3) does the DAG still hold?
