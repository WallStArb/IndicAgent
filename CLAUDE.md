...
- **TWS `bars_processed` freeze**: `seen_bar_timestamps` dedup caches all timestamps from initial poll — subsequent polls return 0 new bars. Symptom: counter stuck at N×60 forever. **Restart does NOT fix it** — root cause is `endDateTime` timezone handling in TWS daemon. Workaround: restart after market open when live bars flow. See `src/providers/CLAUDE.md` for full details.
