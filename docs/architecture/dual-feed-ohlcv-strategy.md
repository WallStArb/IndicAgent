...
1. **Real-Time Stream (`market.ticks`)**:
   ...
   - **Aggregation**: The `DataProviderAgent` aggregates these 5s bars into 1m bars for downstream pipeline ingestion.
...
- **Flat Bars**: During periods of zero volume, the aggregation services (`DataProviderAgent`, `TimeframeBuilder`) emit a "flat bar" containing the last traded `close` price, `volume = 0`, and the `is_flat_bar: true` flag.
...
- The `DataProviderAgent` maintains an internal `_official_bars_cache` (last 20 bars).
...
- **Service**: `services/data_provider_agent.py`
