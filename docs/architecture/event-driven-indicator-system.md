...
- `services/feature_pipeline_service.py` — Unified I1+I2+I3+I4+I5+SMC+I6 pipeline
...
- `services/tws_daemon.py` — IBKR tick + bar collection with built-in multi-TF aggregation
...
signal_lifecycle_service reads market:{symbol}:TF independently
...
1. **Market Data** — `tws_daemon.py` collects IBKR ticks + bars, publishes to `market:{symbol}:{tf}`
2. **I1-I6 Features** — `feature_pipeline_service` reads market bars, runs tiers I1 through I6 sequentially, publishes typed `IntelligenceEvent` to `intelligence:{symbol}:{tf}`
...
Unified in `feature_pipeline_service`. 98 plugins total.
...
|---------|------|--------|---------|
| TWS Daemon | `indicagent-tws` | `services/tws_daemon.py` | — |
| Feature Pipeline | `indicagent-feature-pipeline` | `services/feature_pipeline_service.py` | :9125 |
