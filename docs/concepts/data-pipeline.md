...
```
{env}:indicators:{SYMBOL}:{TF}          # I1 indicator output
{env}:intelligence:{SYMBOL}:{TF}        # I2–I6 typed IntelligenceEvent
{env}:signals:{SYMBOL}:{TF}:aggregated  # I7 CISScorer aggregated signal
{env}:narratives:{SYMBOL}:{TF}          # I8 per-signal AI narrative
{env}:narratives:group:{GROUP}          # I8 asset-group synthesis
```
...
| `indicagent-signal-generator` | `intelligence:SYMBOL:TF` | `signals:SYMBOL:TF:aggregated` + `signal_ledger` |
| `indicagent-signal-lifecycle` | 1m bar streams | Reads `signal_ledger`, writes updates |
| `indicagent-ai-narrative` | `signals:SYMBOL:TF:aggregated` | `narratives:SYMBOL:TF`, `narratives:group:*` |
| `indicagent-feature-writer` | `intelligence:SYMBOL:TF` | `intelligence_features` (TimescaleDB) |

### Multi-Stream Reading

Services that monitor 23 symbols × 4 timeframes = 92 streams use a **single `xreadgroup` call** with all stream names in one dict. This avoids worst-case 9.2s polling latency from sequential blocking reads.
...
|-------|-----------|---------|
| `market_data_ohlcv` | `historical_backfill.py` only | Raw OHLCV cold storage |
| `intelligence_features` | `feature_writer_service` | Full feature vectors per bar — ML training dataset |
| `signal_ledger` | `signal_generator_service` | I7 signals; lifecycle updated by `signal_lifecycle_service` |
