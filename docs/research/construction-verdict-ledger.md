...
- **Reconsidered graveyard refinements, 2026-09-11** — a deliberately neutral-framed Fable
  ...
  measurement artifact. Item #1 (`alpha_score_residual` bucketed retest) has since run and
  FAST-KILLED (0/8 buckets, see its ledger row above) — closed same day. Items #2-4 remain
  queued: `bars_since_high_fast`'s regime-stratified positive IC was never turned into an
  actual regime-gated construction (only the always-on version was tested);
