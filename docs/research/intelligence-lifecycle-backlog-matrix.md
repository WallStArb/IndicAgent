...
| Phase 148: Alpha Scoring System (OOS Proof Gates) - retitled 2026-07-14, formerly "+ v2.x Retirement Gate" | L | Med | High, eventual | PLANNED. Dependency on Phase 147 removed 2026-07-14 (147 cancelled; Gate 1/Gate 2 read only pure v3.0 tables, zero I7 dependency). Depends on Phase 142A OOS data (exists, EIC-04 now PASS) + Phase 142B accumulating ≥60 trading days of closed `alpha_frames` (the backfill has since run, todo 093 closed). **Updated 2026-07-21:** the "re-evaluate after 143.1" note this row carried is stale — 143.1 completed 2026-07-21, but no one has re-run FRAME-04 against the post-143.1 corpus yet (last known result: 16/17 cells fail, on the pre-143.1-fix baseline). This gate is now genuinely actionable, not waiting on anything — flagging as a real next-action candidate, not just a stale caveat. v2.x's fate (retire vs run as an independent parallel system) is a separate open question, todo 056 - no longer part of this phase. Still don't let it compete with 144 for near-term attention despite the eventual payoff. |
