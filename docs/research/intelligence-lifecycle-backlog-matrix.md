...
lower-ranked (least ready, real raw-price-trap risk). **Phase 144's HIGH-tier row needs
re-reading before acting on it** — its blocker changed 2026-07-21 (see the row itself). The
...
**Corrected 2026-07-22:** Phases 147 and 148 were miscategorized — MEDIUM tier carried a
"Phase 147 CANCELLED 2026-07-14" note that was itself superseded 2026-07-19 (todo 056 revived
147 as a single-deliverable CORPUS-07 phase, and ROADMAP.md's Phase 148 section was rewritten
the same day to restore the dependency) — a 5-day-old correction this table never absorbed.
Both moved to HIGH: Phase 147 is zero-dependency, 1-plan, and the sole remaining gate on
Phase 148 — which is the actual two-gate OOS proof milestone ("does the system have real,
provable alpha") this whole measurement pipeline exists to produce. Its data prerequisites
(EIC-04 PASS, ≥60 days closed `alpha_frames`) are already met by a wide margin. This is
independent of both Phase 144's D-05 track (regime-model refinement, currently blocked on the
symbol_hmm restoration fix) and Phase 162 (infra throughput, separate concurrent session) —
three parallel tracks, not one queue.
...
| Phase 162: ic_engine Corpus Pipeline Throughput | M-L | Med | High | **Newly scored 2026-07-21** (registered 2026-07-18, sat unscored). **Foundational** — not just speed: todo 140 (folded in) is a live, reproducible OOM in `_compute_symbol_tf`/`_compute_cross_sectional_tf` (peak memory still `O(cell_size × n_features)`, unbounded) — a cell ~2x today's largest (5m/low_bull, ~599K timestamps) reproduces it, and the corpus keeps growing toward that threshold. Third-incident-shaped per its own todo text. Bundles todos 134 (incremental recompute), 133 (bootstrap threading), 122 (checkpoint/APR-drift), 139/140/129/009E (folded 2026-07-19, same two functions). Risk is Med not Low because this touches the same numerically-sensitive per-cell compute path this session's own worktree just carefully extracted (Task 1) — regressions here are silent-wrong-IC risk, not just a crash. **Planned 2026-07-21/22 (4 plans, 4 waves), in a separate concurrent session — not yet executed.** Belongs in HIGH on the stability angle alone. |
| Phase 147: I7 CORPUS-07 Evaluation | S | Low | — (gate, not alpha) | **Corrected 2026-07-22** — this table had it filed as "CANCELLED 2026-07-14" in MEDIUM; that status was itself superseded 2026-07-19 when todo 056's operator call revived it as a single-deliverable phase (ROADMAP.md rewritten same day). Current scope: map each active I7 plugin to its constituent `feature_vectors` dimensions + Phase 141 IC results; default outcome is retirement (`shadow_registry.status='deprecated'`), rare survivors with IC on dimensions `feature_vectors` doesn't capture register as ordinary predictors. **Zero dependencies** — "CORPUS-07 is this phase's own first deliverable," not a prerequisite run elsewhere. 1 plan. **This is the cheapest, highest-leverage item on this entire table** — it's the sole remaining gate on Phase 148 (below), and nothing else here unblocks a milestone-level proof gate for this little effort. Not planned yet. |
| Phase 148: Alpha Scoring System (OOS Proof Gates) | L | Med | High | **Corrected 2026-07-22** — this table's MEDIUM-tier row claimed "dependency on 147 removed," which was stale (see 147's row above); ROADMAP.md's current Phase 148 section (rewritten 2026-07-19, same day as 147's revival) explicitly lists "Depends on: Phase 147 complete" alongside Phase 142A OOS data (exists, EIC-04 PASS) and Phase 142B's ≥60 trading days of closed `alpha_frames`. That data prerequisite is met by a wide margin — live-checked 2026-07-22: 15.6M `closed_max_hold` rows alone span 2006-09 to 2026-07 (~4,883 distinct trading days), OOS window (`alpha.validation.oos_start`=2025-12-24) still covers ~7 months of that. **Once Phase 147 lands, this phase is fully unblocked with no other gate.** One open nuance worth checking first: FRAME-04 (Phase 142B's own frame-quality gate) hasn't been re-run against the post-143.1 corpus — last known result 16/17 cells fail, on the pre-143.1-fix baseline — re-run before trusting Gate 2's `alpha_frames` input quality. **This is the actual "does the system have real, provable alpha" milestone** — Gate 1 (signal proof, `ic_ci_lower > 0` OOS) + Gate 2 (execution proof, `mean(counterfactual_pnl_r) > 0` OOS) are independent, pre-committed, and exactly the empirical promotion bar this project's whole measurement pipeline exists to clear. |
...
| StratificationDimension formalization (`.planning/todos/pending/111-stratification-classification.md`) — **registered as ROADMAP Phase 145** (2026-07-13) | **not scoreable yet** | — | — | **Updated 2026-07-21:** blocked on Phase 144's D-05 empirical verdict — the 143.1-07 corpus re-run this row previously cited cleared 2026-07-21 (Phase 143.1 COMPLETE), but D-05 itself is now blocked on the symbol_hmm restoration fix (see Phase 144's HIGH-tier row) — same gate every regime-candidate row on this page already respects. Don't force an Effort/Risk/Reward score before the row-grain decision (Option A vs. B, see `concept-unified-registry.md` Domain Vetting) has real evidence to ratify against. Revisit once that verdict lands. |
...
| ~~Phase 147: I7 Alpha Scorer Transition~~ | — | — | — | **Moved to HIGH tier 2026-07-22** — this row's "CANCELLED 2026-07-14" status was itself superseded 2026-07-19 (todo 056's operator call revived it as a lightweight CORPUS-07 evaluation) and this table never caught the reversal, silently burying the cheapest unblock for Phase 148 in MEDIUM for 3 days. See HIGH tier. |
| ~~Phase 148: Alpha Scoring System (OOS Proof Gates)~~ | — | — | — | **Moved to HIGH tier 2026-07-22** — see HIGH tier; the "dependency on 147 removed" claim below was the same stale artifact. |
