...
**Last rewritten:** 2026-07-08, full **Operational context refresh 2026-07-26, touched up 2026-07-27, 2026-07-31, 2026-08-02** (Phase 167 completion, Phase 164 planning, nonlinear_interaction_combiner 1d replication, Phase 168 planned, corpus-throughput todos 215/216/217/226/229, todo 005 measurement-first design) (the dated
...
features (Edge Source Thesis cross_sectional_relative_value/nonlinear_interaction_combiner), or accept no OOS-detectable edge on this branch.** Resolved
2026-07-26: **cross_sectional_relative_value (cross-sectional long-short) passed decisively**, both lookahead scales,
...
first — do not resume 156-159 scoping without the user re-raising it. nonlinear_interaction_combiner (non-linear combiner)
...
provisional. Doesn't affect cross_sectional_relative_value/Phase 167's own result (no regime dependency).
...
168 (Cost-Hurdle-Adjusted Spread Construction, cross_sectional_relative_value follow-on)** was scoped and planned 2026-07-31
...
| Phase 148: Alpha Scoring System (OOS Proof Gates) | L | Med | High | **COMPLETE 2026-07-22 (5/5 plans) — VERDICT: do not promote to live capital.** Gate 1 (signal proof) PASS (140/640 = 21.875% of 5m/15m cells qualify, >10x the 2% floor). Gate 2 (execution proof) FAIL, decisively — 3 of 5 SHADOW-REVIEW criteria, ~960% max drawdown vs a 0.25 ceiling, not a borderline call under any methodology variant tested. Full evidence: `docs/plans/archive/2026-07-22-phase148-promotion-decision.md`. **This delivered the milestone's defining answer — the signal is real, the current frame/execution design doesn't yet capture it profitably.** Direct follow-on: Phase 166 (new row above). |
...
| Phase 165: Swing/Fib/Trend Structure Primitives | L | Low-Med | Med, unproven | Context/research done 2026-07-21, Fable-reviewed, not yet planned. 41 new columns across 5 files (swing/momentum/trend-structure/fibonacci-zones/session-levels). Risk scored Low-Med because the research pass already found and pre-scoped fixes for 2 silent-wrong-answer bugs before any code was written. Reward stays "unproven" (no pilot has run for these primitives, unlike Phase 151's todo 037 or Phase 167's cross_sectional_relative_value). **Deprioritized behind Phase 167 as of 2026-07-26** — no evidence more features are the bottleneck when Phase 167's construction already found signal in the existing 150. |
...
| StratificationDimension formalization (`.planning/todos/completed/111-stratification-classification.md`, closed 2026-08-03) — **registered as ROADMAP Phase 145** (2026-07-13) | **not yet scored** | — | — | **Updated 2026-08-06:** its blocking gate cleared — Phase 144's D-05 verdict landed (F1 not triggered, F2 triggered for 15m/5m; F2's own pending-confirmation check, todo 170, resolved 2026-07-27 with a FAIL result, further unblocking the factor-augmented HMM challenger option). Unblocked for `/gsd-plan-phase 145`, but this row itself still needs a real Effort/Risk/Reward pass once planning starts — the row-grain decision (Option A vs. B, `concept-unified-registry.md` Domain Vetting) can now be informed by real evidence instead of planned blind. Todo 167 (equity-side D-05 equivalent) is still open and unstarted — Phase 144's verdict only covered `rates`/TLT, so planning should either wait for it or explicitly stay agnostic to what it finds. Don't leave this "not yet scored" indefinitely now that its gate is clear — same gap this table has caught itself making twice before (Phases 156-159, then 162-165). |
