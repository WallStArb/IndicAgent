...
| Phase 148: Alpha Scoring System (OOS Proof Gates) | L | Med | High | **Corrected again 2026-07-22** — earlier same-day correction wrongly claimed this still needed Phase 147; that "Depends on" line in ROADMAP.md was itself stale (SCORE-01/02/03 read only pure v3.0 tables, zero I7 lineage — the one place 147 ever connected was SCORE-04, already downgraded to documentation-only 2026-07-19). **Unblocked today, no dependency on Phase 147.** Data prerequisites already met by a wide margin — live-checked 2026-07-22: EIC-04 PASS, 15.6M `closed_max_hold` `alpha_frames` rows spanning 2006-09 to 2026-07 (~4,883 distinct trading days), OOS window (`alpha.validation.oos_start`=2025-12-24) covers ~7 months of that. One open nuance worth checking first: FRAME-04 (Phase 142B's own frame-quality gate) hasn't been re-run against the post-143.1 corpus — last known result 16/17 cells fail, on the pre-143.1-fix baseline. **This is the actual "does the system have real, provable alpha" milestone** — Gate 1 (signal proof, `ic_ci_lower > 0` OOS) + Gate 2 (execution proof, `mean(counterfactual_pnl_r) > 0` OOS) are independent, pre-committed, and exactly the empirical promotion bar this project's whole measurement pipeline exists to clear. **The single highest-priority item on this table.** |
| Phase 147: I7 CORPUS-07 Evaluation | S | Low | Low (due diligence, not alpha) | **Downgraded 2026-07-22** — not a gate on anything (see Phase 148's row: the dependency was stale). Scope: map each of the 35 registered I7 plugins (`shadow_registry`, `component_type='i7_plugin'`, zero ever promoted/evaluated live) to `feature_vectors` dimensions + Phase 141 IC results; default outcome retirement, rare survivors register as ordinary v3.0 features. The pipeline that would run these plugins (`indicagent-intelligence-pipeline.service`) has been `failed` since 2026-07-17 with its `ExecStart` target deleted from disk — zero live blast radius either way. Worth doing eventually for "never drop data that could contain signal" completeness; do not let it compete with Phase 148 for attention. Not planned yet. |
...
| ~~Phase 148: Alpha Scoring System (OOS Proof Gates)~~ | — | — | — | **Moved to HIGH tier 2026-07-22 — the single highest-priority item on this table.** See HIGH tier for the corrected (no-147-dependency) status. |
