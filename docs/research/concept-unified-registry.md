...
**Canonical doc for stable architecture/mechanics (schema, the 9 invariants, `ConceptRegistryService`'s two call surfaces):** `docs/foundation/unified-concept-registry.md` (added 2026-08-08). This doc remains the live design reference for domain vetting and the full ten-table reference architecture. Phase 170's `feature`-domain migration completed 2026-08-10 (see the dated entry below) — check `.planning/STATE.md` for anything since.

**Status**: `domain='feature'` migration COMPLETE 2026-08-10 (todo 118, Phase 170, migrations 283/284/310/311): `concept_registry`/`concept_gate`/`concept_transition_log`/`concept_annotation` live, both `domain='ensemble_strategy'` (5 concepts) and `domain='feature'` (294 rows: 249 original + 43 seeded when Phase 151 shipped + 2 orphan tombstones) seeded, `ConceptRegistryService` (`src/intelligence/concept_registry_service.py`) is the sole feature-lifecycle writer via `ic_engine.py`'s post-run hook. `feature_registry`/`feature_transition_log` were DROPped by migration 311 — the four-table Minimal Viable Version described below is no longer just the design target, it is what's live; reference-architecture (ten-table) tables remain unbuilt by design. Surviving sibling/precursor registries, each still a separate system: APR (425+ params), tag vocabulary (71+ tags). `shadow_registry` (36 rows) exists but is legacy and was never evaluated - not "live" in any operational sense.
...
- **2026-08-10 (Phase 170 completion, Claude).** `domain='feature'` migration executed end to end: migration 310 backfilled provenance metadata for 43 features Phase 151 seeded directly into `concept_registry` (bypassing migration 284's original batch seed, so they carried correct status/gates/lineage but no `migrated_from` tag), bringing `ops_concept_feature_migration_verify.py` to `VERDICT: PASS` (11/11 checks). Migration 311 then `DROP`ped `feature_registry`/`feature_transition_log` and `fn_cascade_parent_deprecation`, deleted the three now-orphaned `alpha.feature_registry.*` APR keys, and `services/ic_engine.py` had `FeatureRegistryService`'s dual write (Plan 06's shadow-mode comparison machinery) removed entirely — `ConceptRegistryService` is the sole feature-lifecycle writer now. Deviation from 170-08-PLAN.md's original design, made explicitly rather than silently: the plan's evidence bar for authorizing the DROP was a live `registry_dual_write_verified` `integrity_monitor` fact from a post-cutover `ic_engine` lifecycle-hook run. That bar turned out to be structurally unreachable, not merely unmet — every in-sample `ic_engine` run's `training_window_end` clamps to the pinned `alpha.validation.oos_start` (2025-12-24), and that exact window already carried lifecycle-hook facts from 2026-07-22, three weeks before Plan 06's dual-write code shipped, so the hook's own idempotency guard (Step 0) would short-circuit before the dual-write comparison on every possible re-run regardless of data quality. Advancing `oos_start` to manufacture fresh evidence is its own separately-flagged "bigger call" (`docs/plans/OOS-EVAL-PROTOCOL.md`), not appropriate to make casually just to unblock this retirement. Explicit user direction (2026-08-10) authorized proceeding on the static parity check plus sign-off instead: `feature_registry` is governance/bookkeeping metadata, not something gating live capital or trading decisions, so the stricter dynamic-evidence standard was disproportionate once shown to be structurally unreachable. The three in-migration `RAISE EXCEPTION` guards (replay completeness, row-count parity, status parity) still ran inside the same transaction as the `DROP`, so an actual data shortfall still would have refused it mechanically — only the "was a live comparison run" requirement was relaxed, not "do the two registries actually agree." Full governance history survived: `concept_transition_log(domain='feature')` = 294 rows (249 original genesis + 43 backfilled genesis + 2 replayed).
...
**9. Status transitions are compare-and-swap, in one transaction.** The promotion/demotion write is `UPDATE concept_registry SET status = :to WHERE concept_id = :id AND status = :from`; zero rows updated aborts the whole transaction, including the `concept_transition_log` insert. Without the `AND status = :from` guard, two racing evaluators (or one evaluator holding a stale in-memory cache) can log a transition whose `from_status` never matched the row, corrupting the exact audit trail this system exists to keep trustworthy. (`FeatureRegistryService._write_transition_record()` used to fail this invariant - unconditional `UPDATE ... SET status`, scheduled fire-and-forget, failure reduced to a log line the caller never sees. Fixed at migration time, not generalized as a pattern: the service itself was retired 2026-08-10 (Phase 170), and `ConceptRegistryService.record_transition_sync`'s CAS discipline is now universal across every domain.)
