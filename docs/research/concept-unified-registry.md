...
**Canonical doc for stable architecture/mechanics (schema, the 9 invariants, `ConceptRegistryService`'s two call surfaces):** `docs/foundation/unified-concept-registry.md` (added 2026-08-08). This doc remains the live design reference for domain vetting, the full ten-table reference architecture, and Phase 170's in-flight `feature`-domain migration — it is not superseded, unlike the fully-shipped ITR/CVR design docs. Check `.planning/STATE.md` for current Phase 170 build status regardless of which doc you're reading.
