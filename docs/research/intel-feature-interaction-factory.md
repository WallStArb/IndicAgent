...
**Status:** Evidence trigger CLEARED (2026-07-10), but this doc's original combinatorial mechanism is superseded by Phase 151's independent decision to build a curated theory-motivated layer instead (see the pilot-result note below) — historical/reference context, not an active implementation spec. **2026-07-24: a v2 redesign section below addresses the specific statistical-power objection that killed the original mechanism** (see "v2 Design — Power-Preserving Candidate Generation"). It is a documented candidate for future work, not a decision to build — see that section's own sequencing recommendation (gate on Phase 151 landing first). Tracked at `.planning/todos/completed/181-interaction-factory-v2-power-preserving-fdr.md`.
