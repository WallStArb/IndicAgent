...
**Version:** 1.0
**Status:** SUPERSEDED (2026-07-18) — see "Where each level landed" below. Not built as a unified
service; the underlying validation discipline is real and already captured, distributed across
existing phases rather than centralized here.
...
correct, but each of the 4 proposed levels already has a home, one better-specified than this
doc and one explicitly rejected by a later architectural decision:
...
| **162.2 Prediction** (AlphaEngine vs. AnalogEngine incremental-R² combination test) | New weekly batch + combination math | **Architecturally moot as originally framed, not just premature (corrected 2026-07-18, Fable review).** "AnalogEngine" was renamed **PrecedentEngine** 2026-07-09, and decision D4 (ROADMAP.md, PRECEDENT-01 section) already settles that its outputs are "ordinary predictors... entering the same pipeline as every parametric feature, not a second system" — there is no second engine to run an incremental-R² combination test against. Reframed as [todo 136](../../.planning/todos/deferred/136-prediction-combination-test-analog-engine.md): a family-level ablation (does the precedent predictor family add ensemble-level IC beyond parametric features, after Ledoit-Wolf deflation), gated on Phase 150 (PrecedentEngine) shipping, not AnalogEngine |
