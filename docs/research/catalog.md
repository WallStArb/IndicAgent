...
| [Edge Source Thesis](data-edge-source-thesis.md) | draft, high (v1.9) | | Ten falsifiable theses on where edge comes from (renamed from T1-T10 to descriptive names 2026-08-03 -- a bare ticket number isn't a concept name); standing doc, revisit per thesis. **Reorganized 2026-08-03** into Trade Constructions (retail_immediacy_provision/cross_sectional_relative_value/horizon_risk_premium) vs Signal-Extraction Questions (regime_conditional_persistence/nonlinear_interaction_combiner/cointegrated_pairs_residual/statistical_factor_residual/cross_asset_lead_lag/adaptive_combiner_weights/jump_diffusion_decomposition); nonlinear_interaction_combiner's original idea doc archived, fully absorbed inline. **regime_conditional_persistence falsified 2026-07-24** (todo 179's 234-cell sweep); **cross_sectional_relative_value passed 2026-07-26** (cross-sectional decile spread, both scales, cleared shuffled-null guard, strongest evidence-backed candidate on the doc; which tf it should trade at is itself now open, todo 235); **nonlinear_interaction_combiner confirmed real, SUBSTANTIAL at 1h and 15m (the actionable tf), small at 1d** (all three tfs re-verified 2026-08-02/03 under corrected corpus; 5m replication in progress 2026-08-03, closing a gap where the script existed but was never run); the five newest candidates (cointegrated pairs, statistical factor residual, lead-lag structure, adaptive combiner weights, jump/diffusion decomposition), none tested yet, sequenced behind nonlinear_interaction_combiner-at-5m and todo 238 (pre-registered test of cross_sectional_relative_value ranked by nonlinear_interaction_combiner's tree score instead of `ctf_momentum`) |
...
| [Trade Construction Layer](trade-construction-layer.md) | draft, high — **cross_sectional_relative_value passed 2026-07-26**, ready to scope as a real phase | | Forecast → position via cross-sectional long-short (cross_sectional_relative_value); scoping gate (Phase 142A OOS proof) cleared 2026-07-22, cross_sectional_relative_value's falsification test passed decisively both scales — cost-hurdle treatment of the spread construction is the next open item before scoping |
