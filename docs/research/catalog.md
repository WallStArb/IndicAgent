...
| [Symbol State Query Layer](intel-symbol-state-query-layer.md) | draft, medium, scope-corrected same session | first-principles/Renaissance-lens critique in-session (2026-07-31), not literal Fable | Read-only query API exposing already-measured `market_regimes` + raw z-scored `feature_vectors`, grouped by `feature_registry.group_name` for readability, never blended into new scores; a group-averaged/composite "strength score" version was proposed then rejected — revives CIS's hand-assigned-weight mistake and `last_ic_sharpe` (the one non-subjective weighting candidate) is 0% populated |
