...
cross_sectional_relative_value (cross-sectional long-short) passed decisively at both lookahead scales, clearing a
...
methodology in the docs themselves, not duplicated here. nonlinear_interaction_combiner (non-linear combiner) came back
...
Trade Construction Layer against todo 179's 2026-07-24 finding (regime_conditional_persistence falsified — no
regime-conditional edge survives OOS replication anywhere in the current champion population).
Both docs updated in place: Edge Source Thesis gained a falsified regime_conditional_persistence and a new candidate nonlinear_interaction_combiner
...
| [Edge Source Thesis](data-edge-source-thesis.md) | draft, high (v2.1) | | Ten falsifiable theses on where edge comes from (renamed from T1-T10 to descriptive names 2026-08-03 -- a bare ticket number isn't a concept name); standing doc, revisit per thesis. **Reorganized 2026-08-03** into Trade Constructions (retail_immediacy_provision/cross_sectional_relative_value/horizon_risk_premium) vs Signal-Extraction Questions (regime_conditional_persistence/nonlinear_interaction_combiner/cointegrated_pairs_residual/statistical_factor_residual/cross_asset_lead_lag/adaptive_combiner_weights/jump_diffusion_decomposition); nonlinear_interaction_combiner's original idea doc archived, fully absorbed inline. **regime_conditional_persistence falsified 2026-07-24** (todo 179's 234-cell sweep); **cross_sectional_relative_value's Phase 167 PASS is UNVERIFIED as of 2026-08-04** (sole ranking feature `ctf_momentum` confirmed carrying a lookahead-leaking batch join, todo 243; diagnostic-tier re-verification found Gate 1 now FAILS under the corrected join, not yet authoritative-tier confirmed — do not start Phase 168); **nonlinear_interaction_combiner's "SUBSTANTIAL at 1h/15m" verdict is SUPERSEDED, confirmed overwhelmingly leak-driven at every tf (90.6%/79.1%/43.8% collapse 1h/15m/5m, todo 245 CLOSED 2026-08-04) — a small, real residual survives at every tf, ~15-70x smaller than the original claim**; the five newest candidates (cointegrated pairs, statistical factor residual, lead-lag structure, adaptive combiner weights, jump/diffusion decomposition), none tested yet, sequenced behind todo 238 (pre-registered test of cross_sectional_relative_value ranked by nonlinear_interaction_combiner's tree score instead of `ctf_momentum` — itself gated on cross_sectional_relative_value's own re-verification landing first) |
...
| [Trade Construction Layer](trade-construction-layer.md) | draft, high — **cross_sectional_relative_value's Gate 1/Gate 2 PASS is UNVERIFIED as of 2026-08-04**, re-verification blocks scoping Phase 168 | | Forecast → position via cross-sectional long-short (cross_sectional_relative_value); scoping gate (Phase 142A OOS proof) cleared 2026-07-22, cross_sectional_relative_value's falsification test passed decisively both scales 2026-07-26 — but the productionized construction's own live Gate 1/Gate 2 (2026-07-27) ranked on a since-confirmed-leaked feature (todo 243); a diagnostic-tier re-check found Gate 1 now fails under the corrected join, authoritative re-verification still pending (todo 253) |
...
| [Symbol State Query Layer](intel-symbol-state-query-layer.md) | draft, medium, scope-corrected same session | first-principles/Renaissance-lens critique in-session (2026-07-31), not literal Fable | Read-only query API exposing already-measured `market_regimes` + raw z-scored `feature_vectors`, grouped by `concept_registry.group_name` for readability (corrected 2026-08-21, was `feature_registry.group_name` -- table DROPped by migration 311), never blended into new scores; a group-averaged/composite "strength score" version was proposed then rejected — revives CIS's hand-assigned-weight mistake and `last_ic_sharpe` (the one non-subjective weighting candidate) is 0% populated |
