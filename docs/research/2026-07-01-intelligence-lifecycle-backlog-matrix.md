...
are unblocked. `regime_group` dispatcher (Phase 144) was already a real ROADMAP.md phase, not
...
`docs/research/intel-12-stratification-dimension.md` — one unified governance gate
...
for current status. AnalogEngine (Phase 148/149, LOW tier below) is similarly rescoped in
`docs/research/intel-13-analog-engine.md` — the "no cheap pilot step" gap that row flags is
...
| Interaction Primitives pilot (037) | S | Low | High/cost | Gatekeeper for the XL primitives-expansion item below (Phase 150) — run before, not alongside. Todo-only, no phase needed for a pilot. |
| `regime_group` dispatcher (migration 189) | L | Med | High | **Correction 2026-07-02: this row's "not yet in ROADMAP.md" claim was wrong** — Phase 144 already exists as its own ROADMAP.md phase. Status per the 2026-07-01 architecture review (`.planning/research/2026-07-01-v3-architecture-review.md` §4): execution-ready, 0/9 tasks started, live problem not a future one (15/58 corpus symbols mislabeled today). Two policy gaps found and fixed in the plan doc itself: unrouted symbols (GLD/SLV/VNQ/IBIT) now excluded+logged instead of silently defaulting to equity; commodity-group enablement now has an explicit dependency edge on todo 041 (tag taxonomy — see MEDIUM tier). Still **foundational** — Cross-Group Lead-Lag IC below can't build without it. |
| Phase 142A: Ensemble IC Measurement | L | Med | High | **COMPLETE** (2026-07-02 correction — was "next roadmap phase"). `alpha_ensemble_ic` schema + `EnsembleICEngine` + hold_max_bars calibration + EIC-04/05 shipped, 10/10 verification truths. Unlocked `is_shadow` (011a), Phase 152, and Phase 142B.1. |
...
| Phase 152: EnsembleHealthMonitor | M | Low-Med | High | Correctly roadmapped, depends on Phase 142A (`alpha_ensemble_ic`). Live-safety gate, matters more as system nears real capital. |
...
| Canonical Simulator binding rule (`docs/research/canonical-simulator.md`) | **SHIPPED — nothing queued** | Low | Critical | **Added 2026-07-03 — this matrix's scope statement covers it ("pulled from wherever they live") but it was never folded in; predates the matrix by one day.** Fable-reviewed 2026-07-03: the *engine* proposal (one shared point-in-time replay engine) was rejected — second-system shell, no client actually needs bar-by-bar state reconstruction. That is the only thing NOT being built. What IS shipped, this session: the binding rule (no client builds its own counterfactual/replay path — routes through `alpha_frames` + Invariant 1 + a future shared cost kernel) plus its automated enforcement (pre-commit Check 9, live now, blocks new `*_events`/`*_frames`/`*_claims`/`*_ledger`-shaped tables without justification). Nothing further is scheduled or queued — the row exists so this matrix reflects that the invariant is live, not to flag pending work. Priority `critical` describes the stakes of the invariant holding (two prior look-ahead incidents), not an active task. The only latent future item — a shared cost kernel — is explicitly deferred until a second real consumer needs it. |
| Confluence as governed predictor (`docs/research/intel-10-confluence-detection-persistence-layer.md`, v3) | — | — | High | **Added 2026-07-03** — rewritten same session as a predictor family (not a second system), gated on Phase 150 interactions + `feature-scoring-beyond-ic.md` §0b/0c landing as a *hard* prerequisite of its gate 1 (a real silent-wrong-answer risk Fable found: gate 1's null model must be the calibrated/shrunk additive baseline, not today's raw `alpha_score`). Not independently actionable yet — surfaced here because §0b/0c's priority should be read as partially driven by this gate, not just the ensemble-weighting need already in this matrix. |
| One Model, One Book (`docs/foundation/principles.md`) | — (already shipped as a principle) | — | Foundational | **Added 2026-07-03** — promoted from the now-retired `intel-11` to a binding foundation invariant: one forecast per (symbol, tf, bar); no new forecasting system ships as a second book. Governs how every row in this matrix's HIGH tier should be built — flagging here so future matrix entries get checked against it before being added, not after. |
...
| Cross-Group Lead-Lag IC (`docs/research/cross-group-lead-lag-ic.md`) | M | Med | Med, unproven | Reuses existing `ic_engine` machinery (new join pattern: group A's regime/feature vs. group B's forward returns, not new infra). 6 candidate pairs identified (rates→precious metals is the theoretically cleanest). Real open risk: testing multiple pairs × lags × TFs needs the same BH-FDR discipline as cross-sectional IC, not ad hoc pair-by-pair testing. **Gated on Phase 144 (`regime_group`)** — needs clean peer groups on both sides of the join before it's buildable at all; Phase 144 is execution-ready per the row above, so this is closer than "gated" alone implies. |
| HMM non-causal-fit diagnostic / 026 Decision-Gate Step 1 (034) | S | ~0 | Contingent | **Unblocked 2026-07-02** — `feature_ic_scores` is now populated (256,566 rows), so the baseline-separation query (`trending_up` vs `trending_down` mean IC gap) is runnable today, not waiting on corpus completion anymore. This is the literal next step in the master sequence from the 2026-07-01 architecture review (Phase B → 026 Step 1 → 142A → Phase 144). Still don't build todo 026's Rolling HMM Refit / Expanding Scaler items (its priority-tier `P4a`/`P4b`) until the query result says to — already tried once, killed for no baseline. |
| Tag exposure-vs-sensitivity taxonomy audit (041) | M | Low | Med, **now load-bearing** | **New row 2026-07-02** — previously absent from this matrix. Discovered dependency: Phase 144's commodity/fx regime groups cannot be enabled without this (OIH/XLE/XOP carry both `eq_*` and `commodity_*` tags; enabling `commodity_energy` today raises `AmbiguousRegimeGroupError` at ic_engine startup). Was implicitly MEDIUM/latent (feeds Instrument Tag Calibrator, 040/148); now explicitly gates Phase 144's second-stage rollout, not just the calibrator. |
...
| Instrument Tag Calibrator (040/Phase 145, renumbered 2026-07-04) | L-XL | Med | High, **latent** | Verified: routing ignores `weight` today — nothing consumes calibrated weight yet. Sound, not urgent. |
...
| IntegrityMonitor platform (Phase 151 + Phase 152; former "149B" item merged into Phase 143 — renumbered 2026-07-04) | XL | Low | High long-run, low now | Insurance, not a fix. Don't let it jump items with present-tense value. |
...
| HMM variants — IOHMM / Hamilton / factor-augmented | L each | Med-High | Unproven | Gated on proof current HMM is deficient (todo 026 Decision Gate — Step 1 now runnable, see MEDIUM tier). **Added 2026-07-02:** the IOHMM and factor-augmented variants turn out to structurally depend on Phase 144 — IOHMM's exogenous inputs (VIX, breadth, yield spread) are literally Phase 144's signal-module outputs; the factor-augmented variant's "cross-sectional factor returns" reuse Phase 144's peer-resolution mechanism. Building either before Phase 144 ships means redoing work Phase 144 will obsolete — one more reason these stay LOW regardless of the gate outcome. The Hamilton variant has no such dependency, is a pure per-symbol simplification, but still gated on the same deficiency proof. Adding complexity contradicts this codebase's own "simple features beat complex" principle either way. |
...
| AnalogEngine (Phase 148/146) | XL | **High** | Speculative | **Corrected 2026-07-03 — row previously contradicted this doc's own 2026-07-02 evening preamble.** The "no cheap pilot step" gap is resolved: `docs/research/intel-13-analog-engine.md` rescoped the substrate to ship and validate (embedding calibration study, retrieval quality) before any full historical build — a cheap pilot step now exists. Stays LOW/XL/High-risk on remaining grounds: still gated on Phase 142A's OOS proof pattern generalizing, and ROADMAP.md Phases 148-149 themselves are now stale text (still spec the pre-rescope Score Object/`score_cache` design intel-13 deleted — see todo 055) and must be rewritten against intel-13 before either phase is planned. |
| Alternative Data Vectors (Phase 154, renumbered 2026-07-04) | L | Med | Med | Not actionable — no data source chosen. |
