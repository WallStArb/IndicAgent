...
**Goal:** Rewrite `production/scripts/lifecycle_replay.py` to match the post-Phase-104 schema (signal_ledger/signal_outcomes split), add integrated house cleaning, and replay 2.83M corrupt signals from May 21 - June 1 with fixed lifecycle code.

**Architecture:** Single idempotent script with four phases: (0) preflight — advisory lock, service quiescence check, orphan seeding; (1) reset — wipe corrupt outcomes and derived tables within exact timestamp bounds; (2) replay — re-evaluate every signal against actual bar data using current `lifecycle_tracker.evaluate_signal()` and `evaluate_market_entry()`; (3) verify — before/after row counts, impossible combination checks, stale unresolved detection. The script SELECTs from `signal_ledger` JOIN `signal_outcomes` explicitly (no view). Writes target `signal_outcomes` only (lifecycle fields never touch `signal_ledger`).

**Tech Stack:** Python 3.11+, asyncpg, asyncio, TimescaleDB
