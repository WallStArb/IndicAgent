...
All DLQ topics set to 7-day retention (`retention.ms=604800000`). Configured via `production/scripts/ensure_topics.sh` — idempotent, run at deploy. Non-DLQ topics retain their current settings.
...
12. **`production/scripts/ensure_topics.sh`** — idempotent topic provisioning script
