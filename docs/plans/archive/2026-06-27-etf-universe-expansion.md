...
**Dependency:** `docs/plans/archive/2026-07-01-cross-sectional-regime-model.md` — regime group tag filters (`commodity_energy_crude`, `commodity_metals_precious`, `fx_*`, etc.) must exist in `tag_vocabulary` before regime groups can be enabled.
...
**STATUS: COMPLETE** (2026-07-29 status check: `instruments` table has a live CWB row — migration 190 applied). CWB was added after migration 188 already ran; 189 was reserved by `docs/plans/archive/2026-07-01-cross-sectional-regime-model.md` (`189_regime_group.sql`), so this was a standalone follow-up migration.
