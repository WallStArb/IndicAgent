...
**"Concept Governance Registries" is a conceptual family name** — it groups three different *types* of systems that share a governance mindset (identity, auditability, operators can query what happened 10 years later) but use **different infrastructure** and have **different purposes**.

```
Concept Governance Registries (conceptual family)
├── Type 1: APR (Adaptive Parameter Registry)
│   └── Tables: config_state, config_history, config_schema (separate infrastructure)
...
│   │
│   └── Concept Registry (the unified piece)
...
│
└── Type 3: Vocabulary registries (static taxonomy, no lifecycle)
...
**What's actually unified:** Concept Registry (Type 2) is the single-schema system that governs multiple research domains. APR and the vocabularies are related by governance philosophy only — they have separate tables, separate services, and separate purposes.
...
| Tag Vocabulary | `tag_vocabulary`, `instrument_tags`, `instrument_annotations` | 71 tags / 410 assignments | Human curation | — |
...
stale — see `.planning/research/2026-07-04-concept-registry-cluster-fable-review.md` F4.)
...
CREATE TABLE concept_registry (
    ...
    domain            TEXT    NOT NULL
        -- 7 values today (2026-07-04). `confluence` and an embedding/analog-substrate domain are
        -- anticipated (see Domains table below) but not added until each has real candidates —
        -- keep this list and that table in sync when either lands.
...
`feature_interaction`'s eventual promoted-survivor storage from Interaction Factory (`docs/ideas/interaction-factory.md`), is already covered by the `feature_interaction` row above — no separate `compound_primitive_registry` table needed once Concept Registry exists.
