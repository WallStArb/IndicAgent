...
**Tags:** signal-combination, independent-conviction, effective-n, ic-weighting, decorrelation, shrinkage, vil, vil-04, vil-02, vil-03
...
- **vil-04** measures *independence* — effective-N and the pairwise correlation of live edges
- **vil-02** measures *trust* — the IC and IC Sharpe of every predictor
- **vil-03** measures *each edge in isolation* — the Score Object: E[R], conviction, distribution, per (symbol × TF × setup)
...
This system produces the raw materials and then stops at the threshold of the most important step. vil-03 scores each edge beautifully and in isolation; nothing combines them. We would count 18 live signals as 18 independent reads when vil-04's whole thesis — now repeated one level up — is that they might be 4. The aggregate conviction is inflated by exactly the redundancy nobody is measuring across the live set.

vil-05 is the missing layer. It is where the firm's actual edge would live.
...
Not "which signal is best" (that is vil-03, per-edge, in isolation). The combination question is about the set as a whole: two top-ranked signals may be the same underlying bet, and counting both as conviction is the inflation this layer exists to remove.
...
A live edge that correlates 0.9 with one already counted adds almost nothing — it is the same bet twice. Each edge is weighted by what it adds *after* accounting for everything already in the set, using the correlation fabric from VIL (`similarity_pairs`, signal-level). This is the live-set expression of vil-04's whole thesis: independence, not count.

**3. Aggregate conviction is bounded by effective-N, not edge count.**
Eighteen live signals at effective-N 4 carry four independent reads, not eighteen. The aggregate conviction of the set scales with effective-N (the independence it actually has), never with how many signals happen to be firing. This is the single most important honesty gate, and vil-04 already computes the number — at the signal level (see vil-04's effective-N-everywhere extension).

**4. Reads must be net of cost.**
A gross edge that evaporates after slippage is not an edge. Expected returns entering the combiner are net of modeled transaction cost (see vil-06's cost-aware net scoring). At the short horizons this system targets, cost is often larger than the edge — ignoring it is self-deception.
...
This is the discipline that outranks the rest. The combined read is *itself a predictor*, so it is accountable to the same IC machinery as everything else (vil-02). Measure the IC Sharpe of the decorrelated-combined conviction against the naive equal-weight aggregate, out-of-sample. If the elaborate combination does not beat equal-weight, it is complexity for nothing and gets cut. "Decorrelation is principled, therefore good" is not an argument Simons would accept — only the out-of-sample IC is. The combiner registers as a predictor in the IC Factory like any other.
...
```
Live edges  ── all current Score Objects (symbol × TF × setup), from vil-03
        │
        ▼
   Shrink ──── read_shrunk = E[R]_net · f(IC_Sharpe)      # trust-discount, toward zero
   (vil-02)  low-trust edges pulled to ~0 before they count
        │
        ▼
   Decorrelate ── greedy residualization against the live correlation matrix
   (vil-04)     (similarity_pairs, signal-level): take the strongest edge,
   ...
   (vil-04)   4 independent reads → conviction of 4, not 18
        │
        ▼
...
| IC Sharpe weights | weekly (vil-02) | batch, off the hot path |
| Live correlation matrix | weekly (vil-04) | batch, off the hot path |
...
```
vil-04 (independence)  ┐
vil-02 (trust)         ├──►  vil-05 (combine → independent-conviction view)  ──►  research / I7 lever / eventual sizing
vil-03 (per-edge score)┘
```

vil-05 measures nothing and scores nothing — it *combines* what the three layers below already measured and scored. It owns the combined view, the shrink/decorrelate/bound logic, and the set-level diagnostics. It owns no embeddings, no IC computation, no retrieval.
...
- **Not a replacement for vil-03.** vil-03 scores each edge in isolation; vil-05 combines the set and removes cross-edge redundancy. Different questions — per-edge versus the set as a whole.
...
|---|---|
| `vil-03` (Scoring Engine) | Direct input. Each live Score Object is a candidate edge. vil-05 is vil-03's set-level consumer — it adds the cross-edge decorrelation vil-03 deliberately does not do. |
| `vil-02` (IC Factory) | Supplies the IC Sharpe used for shrinkage — how hard to pull each edge toward zero. |
| `vil-04` (Correlation) | Supplies signal-level effective-N (conviction bound) and the correlation matrix (decorrelation). vil-05 is the live-set expression of vil-04's independence thesis. |
| `vil-06` (cost-aware net scoring) | Supplies E[R] net of modeled cost — the input the combiner shrinks and combines. |
