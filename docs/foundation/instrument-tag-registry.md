...
**Last Updated:** 2026-09-18 -- audited against live code/DB (session that planned Phase 175); Consumers and Known Gaps sections corrected, see those sections for what changed and why.
...
- **`discovery_oos_days` is computed but not enforced** (todo 125). A freshly discovered tag writes a fully live `source='empirical'` row immediately -- `discovery_state: pending_oos` is stashed in `evidence` JSONB but nothing reads it to gate weight or downstream consumption. Practical urgency is low today: neither live reader touches the `sensitivity`/`macro_driver` tags this affects (both key off `exposure` prefixes, further restricted to `source='human'` -- see Consumers above). **In progress:** Phase 175 (planned 2026-09-18, not yet executed) folds this todo in as part of a materiality-filter design -- promoting `discovery_state`/`first_measured_at` to typed, enforced columns is one of its five plans. Until that phase ships, this gap stands as described.
- **`sensitivity`/`macro_driver` are measured but entirely unconsumed -- and not just unconsumed, redundant with an already-computed signal.** Verified live 2026-09-18 (up from 211/300 at the original 2026-08-20 finding -- TagCalibrator has run since): 578 `sensitivity` rows and 875 `macro_driver` rows carry real `TagCalibrator`-measured betas (`source='empirical'`, non-null `loading`/`p_value`), but several of them (`rate_sensitive`→`TLT`, `credit_risk`→`HYG-IEF`, `volatility`→`SPY_REALIZED_VOL`, `dollar_strength`→`UUP`) target the same reference instruments `market_regimes`' `regime_group` system already computes continuous group-level signals for (`curve_pct`/`credit_pct` in the `rates` group, `vix_pct` in `equity`, `dollar_z` in `fx`) -- via a completely separate, never-joined pipeline. See `docs/ideas/signal-sensitivity-regime-interaction-primitives.md` for the full proxy-overlap table and a concrete, zero-new-measurement first test (`rate_sensitive` loading × `rates` group's `curve_pct`, both already live) -- that doc received a Fable rigor pass 2026-09-18 and is revision-required, not yet promoted to `docs/research/`; treat its specific numbers as provisional until re-verified. `factor_regime`'s 0 empirical rows are NOT part of this gap -- confirmed `measurement_type='definitional'` with no `factor_series` set on all 6 rows, a deliberate seed choice, not an oversight. Consuming either tag category safely is gated on the materiality-filter work above (raw `loading`/`passes_fdr` alone does not separate real sensitivity from common-beta noise -- confirmed by the 144-collision incident behind todo 379/380).
- **Neither live consumer filters on `valid_to`.** Verified live 2026-09-18 (`grep -n valid_to services/cross_sectional_regime_model.py services/ic_engine.py` -- zero matches in either file): both readers query `source = 'human'` unconditionally, with no `valid_to IS NULL` clause. Currently benign in practice -- human rows are permanent unless a human changes them, and none has been retired to date -- but this is a latent data-integrity gap, not a proof of safety: if a human-asserted tag is ever corrected (`valid_to` set on the old row), both consumers would silently continue including it, since neither checks expiry. `instrument_tags_active` (a `WHERE valid_to IS NULL` view every consumer should read through instead of re-deriving the filter itself) was proposed as todo 126 and confirmed never built. Phase 175 (see above) builds this view as part of the same migration; until it ships and both consumers are repointed at it, this gap stands.
