...
- **`sensitivity`/`macro_driver` are measured but entirely unconsumed — and not just unconsumed, redundant with an already-computed signal.** Verified live (2026-08-20): 211 `sensitivity` rows and 300 `macro_driver` rows carry real `TagCalibrator`-measured betas (`source='empirical'`, non-null `loading`/`p_value`), but several of them (`rate_sensitive`→`TLT`, `credit_risk`→`HYG-IEF`, `volatility`→`SPY_REALIZED_VOL`, `dollar_strength`→`UUP`) target the same reference instruments `market_regimes`' `regime_group` system already computes continuous group-level signals for (`curve_pct`/`credit_pct` in the `rates` group, `vix_pct` in `equity`, `dollar_z` in `fx`) — via a completely separate, never-joined pipeline. See `docs/ideas/signal-sensitivity-regime-interaction-primitives.md` for the full proxy-overlap table and a concrete, zero-new-measurement first test (`rate_sensitive` loading × `rates` group's `curve_pct`, both already live). `factor_regime`'s 0 empirical rows are NOT part of this gap — confirmed `measurement_type='definitional'` with no `factor_series` set on all 6 rows, a deliberate seed choice, not an oversight.
