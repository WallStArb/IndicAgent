...
- **`ps`'s `%CPU` column is a lifetime average, not current activity** — for a process that's been running for days, that average stays high (e.g. 63%) for a long stretch after the process has effectively gone idle, because a couple of days of prior heavy compute dominates the ratio. Confirmed live 2026-08-21 diagnosing a multi-day `ic_engine.py` corpus run that looked "stuck" by this metric alone. Two reliable alternatives instead: (1) sample `/proc/<pid>/stat`'s `utime+stime` fields twice a few seconds apart and compute the delta yourself (instantaneous rate, not lifetime average); (2) `py-spy dump --pid <pid>` for an actual Python stack trace — in this incident it showed the main thread genuinely parked in `psycopg`'s `cursor.execute()` waiting on Postgres, and `pg_stat_activity` confirmed the DB backend was actively executing (not lock-waiting), with the query completing and a new one starting moments later. Net: not hung, just a legitimately large query for an outsized data cell — but `%CPU` alone would have supported either conclusion equally badly.
...
- **Pre-commit runs 9 automated checks (`tools/pre-commit.hook`, corrected 2026-08-14 — was stale at "8"), including glossary enforcement**: a commit can fail with "glossary violation" because a changed file uses a term banned in `docs/foundation/glossary.md` in place of its canonical replacement — not a broken hook. Fix the term, don't bypass with `--no-verify`.
...
- **A local `.git/hooks/pre-commit` check is not a real gate — only a CI job is**: `tools/pre-commit.hook` (the tracked source, meant to be copied/symlinked to `.git/hooks/pre-commit`, which is itself untracked infrastructure) is opt-in per clone and `--no-verify` bypasses it outright. Found 2026-08-14: its Ring 0 boundary check (`src/core/`/`src/observability/` must not import domain layers — CLAUDE.md's Ring rule) had existed in the hook for a while but was never mirrored into `.github/workflows/ci.yml`'s `plugin-guards` job, despite that job's own header comment claiming to mirror "the bash checks... so `--no-verify` can't bypass them." A stated invariant enforced only locally is an unenforced convention, not a real gate. Fixed 2026-08-14 (Ring 0 boundary + duplicate-test-names now also run in CI) and 2026-08-21 (glossary + counterfactual-ledger, hook checks 8/9, todo 310) -- all 9 hook checks now have a matching `ci.yml` step. Before citing any `tools/pre-commit.hook` check as "enforced," confirm it also has a `ci.yml` step, not just a hook function.
- **`vulture` (dead-code detection) sat in `requirements.txt` unwired until 2026-08-14** — no config, no CI, no pre-commit check, pure dead weight for an unknown span of time. Now wired: `[tool.vulture]` in `pyproject.toml`, CI step in `ci.yml`'s `lint` job, baseline of 1136 pre-existing findings frozen in `tools/vulture_whitelist.py` (grandfather-existing/block-new, same pattern as `[tool.ruff]`'s E501 baseline) — real triage of that backlog is todo 309, not done. `[tool.mypy]` had the identical shape (real config, zero wiring) — added as report-only (`continue-on-error: true`) since 795 pre-existing errors make it unsafe to block on without a baseline mechanism first (todo 311). Check for "configured but never actually run" as its own class of gap in any tool that shows up in `pyproject.toml`/`requirements.txt` but not in `ci.yml`.
- **`pgrep -f` liveness checks must match a bare substring, not a path/flag-specific pattern**: `pgrep -f "services/regime_writer.py"` never matches the `-m services.regime_writer` module-invocation form this project actually uses (no literal `/`+`.py` in that cmdline) — confirmed live 2026-08-14 against an actually-running process. Found and fixed in 3 separate places the same session (`per_symbol_regime_candidates_stage2_orthogonality.py`'s in-flight-write guard, `ops_regime_monitor.sh`, `ops_pipeline_monitor.sh`) — a recurring bug class, not a one-off typo. Any new liveness check should match the bare process/module name, not assume a specific invocation style.
