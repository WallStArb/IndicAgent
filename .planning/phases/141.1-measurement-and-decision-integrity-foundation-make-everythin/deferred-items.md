# Deferred Items — Phase 141.1 Plan 02

## Pre-existing test failures (out of scope, not caused by this plan)

9 pre-existing failures, all traced to the same root cause, confirmed present at this
worktree's base commit `efe64993` (before any changes in this plan) — not a regression
introduced by Plan 02. None touch `production/migrations/192_feature_ic_scores_regime_scope.sql`,
`services/ic_engine.py`, or `tests/unit/test_regime_scope.py` (the only files this plan modifies).
...
## Full-suite hang: tests/unit/providers/test_ibkr_equity.py (pre-existing, not caused by this plan)
...
useRTH=True for historical bars"), well before this plan's base commit `efe64993`. Not
touched by this plan's files (`production/migrations/192_feature_ic_scores_regime_scope.sql`,
`services/ic_engine.py`, `tests/unit/test_regime_scope.py`).
...
completed in 226.62s): **33 failed, 5309 passed, 41 skipped**. Zero failures in
`services/ic_engine.py`-related tests or `tests/unit/test_regime_scope.py` (this plan's
own test file passed cleanly, no FAILED entries). All 33 failures cluster into three
pre-existing, unrelated groups:
...
3. **`regime_writer`/`_alpha_pass` signature mismatch** (documented above) — 14 failures
   ...
   and `tests/unit/test_causal_hmm_decoding.py::TestCausalVsViterbi` (5 more instances of
   the same root cause not previously enumerated).

None of these files were touched by this plan. Logged here per scope-boundary rules;
not fixed.
