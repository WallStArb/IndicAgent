...
<task type="auto">
  <name>Task 2: Build services/regime_writer.py oneshot with per-(symbol,tf) HMM decoding</name>
  <files>services/regime_writer.py</files>
  <read_first>
    - services/regime_writer.py (the file being created — does not exist yet)
  ...
  <action>
    ...
    4. Wrap the whole run in observed_span("regime_writer.run"); record REGIME_WRITER_RUN_LATENCY_SECONDS at end.
  ...
  <acceptance_criteria>
    ...
    - `grep -c "model.predict\|\.predict(" services/regime_writer.py` returns >= 1 (Viterbi/filtered decoding)
    - No hardcoded numeric threshold for n_components: `grep -n "n_components" services/regime_writer.py` shows it read via cfg.get_sync
  ...
  <done>regime_writer.py UPDATEs feature_vectors.regime with canonical text labels for >95% of SPY 5m rows; D-06 + OTel + spans wired; APR-compliant.</done>
</task>
