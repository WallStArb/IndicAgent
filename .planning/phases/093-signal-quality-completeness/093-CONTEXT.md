# Phase 093: Signal Quality Completeness - Context
...
Add four distribution-shape metrics to `signal_metrics` (skewness, kurtosis, min_r, recovery_factor) and run the compute per-symbol in addition to the global '*' aggregate. Makes the signal factory quantitatively rigorous before v2.7 expands it with new lanes — knowing average R is insufficient; distribution shape (left-tail blowup risk) and per-instrument breakdown are institutional requirements.
...
- **D-01:** Four new columns in `signal_metrics` table:
  ...
  - `min_r float` — `min(pnl_rs)` — worst single outcome in the rolling window. Simple, interpretable, complements avg_r directly.
  - `recovery_factor float` — `avg_mfe / abs(min_r)` if `min_r < 0` else `NULL`. Ratio of average best-case to worst single loss. Classic institutional quality screen.
...
### DB Migration
- **D-04:** Idempotent migration — four `ALTER TABLE signal_metrics ADD COLUMN IF NOT EXISTS ...` statements. Run at `SignalMetricsComputeAgent` startup (inline in `_setup()` before first compute cycle) or as a standalone migration script. Prefer startup migration for automation — no manual step.
...
### Compute Logic
- **D-09:** `_build_metrics_result()` receives `acc` dict with `pnl_rs`, `maes`, `mfes` lists. Skewness and kurtosis computed only when `len(pnl_rs) >= 3`. min_r = `min(pnl_rs)`. recovery_factor = `round(avg_mfe / abs(min_r), 4)` when `min_r < -1e-9` else `None`.
...
### Claude's Discretion
- Whether startup migration runs in `_setup()` or a dedicated `ensure_schema()` method (prefer `ensure_schema()` — follows CacheManager self-management principle)
- Whether to inline the scipy calls or extract `_distribution_shape(pnl_rs)` helper returning a dataclass (prefer helper — testable in isolation)
...
### Why These Metrics Matter for v2.7
- When a new qualitative lane (earnings, macro) fires signals, the first question is: "is this signal factory sound?" Mean R and Sharpe are insufficient. Negative skewness (-0.8 or worse) means the distribution has a fat left tail — rare large losses that a Sharpe ratio masks. `min_r` makes the worst case explicit. `recovery_factor` asks: does the average upside (avg_mfe) justify the worst downside?
...
<specifics>
- "Measure everything, especially tails" — Medallion Fund's risk models were built around tail exposure, not just expected return. Skewness and kurtosis are the minimum viable distribution shape metrics. A signal factory with Sharpe=1.5 but skewness=-2.0 is a liability.
- "Scientific rigor before expansion" — adding new lanes (v2.7) without complete per-lane quality measurement is building on sand. These metrics are the evaluation framework for every v2.7 lane.
</specifics>

<deferred>
- Maximum drawdown series (not just min_r — full drawdown curve) — requires timeseries analysis beyond the rolling window
- Conditional Value at Risk (CVaR) — 95th percentile loss — more rigorous than min_r but requires sorting
- IC decay curve (how information coefficient degrades with signal age)
- Benchmark-relative metrics (alpha vs SPY) — deferred to v2.7 evaluation framework
</deferred>
