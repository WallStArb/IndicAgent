...
**What this means:** V1 Quant (the starting vector) is the hypothesis that quant price action primitives — momentum, range position, intra-bar conviction, vol dynamics — jointly predict short-horizon returns. The IC Engine in Phase B will either confirm or refute this hypothesis.
...
**Jim Simons' rule:** "We don't have preconceived notions. We look for things that can be replicated thousands of times." The vector is the precondition; IC is the replication test. Only one vector (V1 Quant) exists at Phase A. V2 and beyond are added after V1 IC is proven, never before.

### D-02: V1 Quant is the Only Vector at Phase A

**Decision:** Phase A builds all 35 primitives but only one vector is active: **V1 Quant (price action)**. No other vector is defined, seeded, or measured until V1 IC is empirically validated in Phase B.
...
**Why:** Renaissance would never hardcode which inputs belong to which model. The membership is a hypothesis that the data may refute. If Phase B shows that `hma_slope_z` has negative IC in all regimes, it is removed from V1 by updating APR — no code change, no migration.
...
**Why:** Researcher interpretation must not contaminate the primitive layer. If `feature_vectors` stored "V1 score = 0.7," you have pre-baked a weighting decision into the data. Future IC measurement would be measuring a human-defined aggregation, not the raw signal. The Ledoit-Wolf solve in Phase C cannot work correctly on pre-aggregated data.

**The separation is absolute:** feature_vectors is the raw measurement layer. Vector scores are a derived analytical layer computed in Phase B/C from primitives × IC weights.

### D-05: Phase A Does Not Touch `intelligence_features`

**Decision:** `intelligence_features` is the v2.x table. Phase A does not read from it, write to it, or depend on it. The source of truth for Phase A is `market_data_ohlcv` only.
...
### D-06: Historical Backfill is a Hard Gate Before Phase B

**Decision:** Phase B (IC measurement) does not begin until the full backfill is verified at target depths: 58 ETFs × 4 TFs, row counts within 5% of theoretical maximum. This is not a preference — it is a correctness requirement.
...
**Verification criterion:** SQL count check per (symbol, tf) vs theoretical bar count at target depth. Log the per-symbol shortfall. Any symbol with < 80% of theoretical bars gets flagged and excluded from Phase B IC measurement (not blocked, just excluded with a known gate).
...
**Phase A scope:** Phase A does not implement this loop. It is noted here because the `feature_vectors` schema and `FeatureVector` dataclass must be designed with this downstream attribution in mind. No decisions made in Phase A should block the attribution loop.
...
This seeded value is a starting hypothesis. Phase B IC measurement may remove members with negative or insignificant IC.
...
Before Phase A: `IntelligencePipeline` calls `PluginRegistry.process_bar(bar)` → 138 plugin calls.
After Phase A: `IntelligencePipeline` calls `FeatureFactory.compute(bars, symbol, tf)` → 1 function call → 35 typed outputs.
...
- **IC measurement** — Phase B. Phase A produces data; Phase B asks what the data means.
- **V2, V3, and beyond** — no vector is defined or seeded until V1 IC is proven. See D-02.
- **Vector score computation** — Phase B derives these from primitive IC weights.
- **Attribution loop (Kafka return path)** — Phase C architecture. Phase A must not block it (D-10), but does not implement it.
