...
must_haves:
  truths:
    ...
    - "outcome_writer emits D-06 job_completed_total + per-service OTel metrics"
  artifacts:
    - path: "services/outcome_writer.py"
      provides: "Oneshot computing LEAD()-based forward returns into outcome_labels"
      min_lines: 200
    - path: "src/observability/metrics.py"
      provides: "outcome_writer_rows_written_total, outcome_writer_run_latency_seconds, outcome_labels_coverage"
      contains: "outcome_writer"
  key_links:
    - from: "outcome_writer.py"
...
<objective>
Build `services/outcome_writer.py` — a Ring 2 oneshot that computes executable, causal forward log returns and writes them to outcome_labels. This is the dependent variable (Y) for IC measurement.
...
<task type="auto">
  <name>Task 1: Add outcome_writer OTel metrics to metrics.py</name>
  ...
  <action>
    ...
    - OUTCOME_WRITER_ROWS_WRITTEN_TOTAL = _meter.create_counter("outcome_writer_rows_written_total", description="rows inserted into outcome_labels; labels symbol, tf")
    - OUTCOME_WRITER_RUN_LATENCY_SECONDS = _meter.create_histogram("outcome_writer_run_latency_seconds", description="Full outcome labeler run duration")
  ...
  <acceptance_criteria>
    - `.venv/bin/python -c "from src.observability.metrics import OUTCOME_WRITER_ROWS_WRITTEN_TOTAL, OUTCOME_WRITER_RUN_LATENCY_SECONDS, OUTCOME_LABELS_COVERAGE; print('ok')"` exits 0
...
<task type="auto">
  <name>Task 2: Build services/outcome_writer.py with causal LEAD() forward returns</name>
  <files>services/outcome_writer.py</files>
  <read_first>
    - services/outcome_writer.py (the file being created — does not exist yet)
  ...
  <action>
    ...
    3. For each (symbol, tf) wrapped in observed_span("outcome_writer.label_symbol_tf"):
       ...
       - OUTCOME_WRITER_ROWS_WRITTEN_TOTAL.add(inserted, {"symbol": symbol, "tf": tf})
    ...
    5. Wrap run in observed_span("outcome_writer.run"); record OUTCOME_WRITER_RUN_LATENCY_SECONDS.
  ...
  <acceptance_criteria>
    - `.venv/bin/python services/outcome_writer.py --symbols SPY --tf 5m` exits 0
    ...
    - `grep -c "ROWS BETWEEN CURRENT ROW" services/outcome_writer.py` returns >= 1
    - `grep -c "observed_span" services/outcome_writer.py` returns >= 2
    - `grep -c "JOB_COMPLETED_TOTAL\|flush_and_shutdown_metrics" services/outcome_writer.py` returns >= 2
    - `.venv/bin/ruff check services/outcome_writer.py` passes
  </acceptance_criteria>
  <verify>.venv/bin/python services/outcome_writer.py --symbols SPY --tf 5m && PGPASSWORD=postgres psql -U postgres -h localhost -d indicagent -c "SELECT count(*) total, count(*) FILTER (WHERE complete_5bar) c5, count(*) FILTER (WHERE complete_60bar) c60 FROM outcome_labels WHERE symbol='SPY' AND tf='5m';"</verify>
  <done>outcome_writer.py produces causal forward returns for SPY 5m; formula validated against raw LEAD(); JOIN gate clean; D-06 + OTel + spans wired.</done>
</task>
...
<task type="auto">
  ...
  <read_first>
    - services/outcome_writer.py (just built)
  </read_first>
  <action>
    Run `.venv/bin/python services/outcome_writer.py` with no symbol filter (all feature_vectors symbols x 4 TFs). Run in background; poll outcome_labels row counts.
  </action>
  <acceptance_criteria>
    ...
    - Re-run idempotency: running `.venv/bin/python services/outcome_writer.py --symbols SPY --tf 5m` a second time inserts 0 new rows. Capture count before/after; they must be equal.
    - `job_completed_total{job="outcome-writer", status="success"}` emitted
  </acceptance_criteria>
