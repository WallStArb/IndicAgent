...
must_haves:
  ...
  artifacts:
    - path: "production/migrations/157_ic_engine_tables.sql"
      ...
      contains: "CREATE TABLE outcome_labels"
...
Output: Populated feature_vectors, outcome_labels + feature_ic_scores tables, seeded alpha.* APR keys, OPS_PREFIXES updated, service_auditor registrations.
</objective>
...
<task type="auto">
  ...
  <read_first>
    ...
    - .planning/phases/138-ic-engine-outcome-labels/138-RESEARCH.md (Finding 1, Finding 7, Risk 1)
...
<task type="auto">
  <name>Task 2: Migration 157 — outcome_labels + feature_ic_scores tables</name>
  <files>production/migrations/157_ic_engine_tables.sql</files>
  <read_first>
    - docs/plans/2026-06-20-alphaengine-ic-spec.md (§XIV.1 outcome_labels DDL, §XIV.4 feature_ic_scores DDL — copy these exactly)
  ...
  <acceptance_criteria>
    - `PGPASSWORD=postgres psql -U postgres -h localhost -d indicagent -c "\d outcome_labels"` shows columns return_1bar/5/20/60, complete_*bar, has_gap_before_entry, PK (symbol, tf, bar_ts)
    - `PGPASSWORD=postgres psql -U postgres -h localhost -d indicagent -t -c "SELECT count(*) FROM timescaledb_information.hypertables WHERE hypertable_name='outcome_labels';"` returns 1
  ...
  <done>outcome_labels (hypertable) and feature_ic_scores tables exist with exact spec DDL and pooled unique index.</done>
</task>
...
<task type="auto">
  ...
  <read_first>
    ...
    - .planning/phases/138-ic-engine-outcome-labels/138-RESEARCH.md (Finding 9, Finding 12)
  </read_first>
  <action>
...
- outcome_labels + feature_ic_scores tables exist with exact spec DDL
...
<output>
After completion, create `.planning/phases/138-ic-engine-outcome-labels/138-01-SUMMARY.md` documenting feature_vectors row counts per (symbol, tf), the actual alpha.* keys seeded, and whether alpha. was already in OPS_PREFIXES.
</output>
