# Phase 167: Cross-Sectional Trade Construction (T3) - Discussion Log
...
the deep research (T3's falsification script, cost-hurdle sweep, temporal-stability check)
...
|--------|-------------|----------|
| Rank on `ctf_momentum` directly | Bypasses the linear ensemble combiner entirely; matches exactly what T3's falsification script measured and validated | ✓ |
...
building Phase 167 on top of it would silently retest a rejected input under a new name. T3's
...
| Option | Description | Selected |
|--------|-------------|----------|
| `ctf_momentum` only | Exact feature/construction T3 validated | ✓ |
...
| Option | Description | Selected |
|--------|-------------|----------|
| Rebalance every bar | Exactly matches T3's measured turnover (~19.5% mean one-way/bar) and its confirmed cost-hurdle survival | ✓ |
...
| Option | Description | Selected |
|--------|-------------|----------|
| 15m, full 80-symbol equity universe | Exactly what T3 measured, including the 21/21-year temporal-stability check | ✓ |
| Broader (5m/1h/1d and/or other asset classes) | Untested for this construction; T3's evidence is 15m/equity-specific | |
