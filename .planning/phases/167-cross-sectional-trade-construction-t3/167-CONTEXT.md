# Phase 167: Cross-Sectional Trade Construction (T3) - Context

**Gathered:** 2026-07-26 (--auto mode — no interactive discussion; deep context already
established this session via T3's falsification script and results)
...
T3 from a validated one-off falsification script
...
**All decisions below were auto-resolved (--auto mode) using the "replicate exactly what T3
...
- **D-01:** Rank the cross-sectional universe directly on `ctf_momentum` (the specific feature
  T3 measured and validated), NOT on the existing linear `ensemble_alpha`/IC-weighted combiner
  ...
  T3 was that ranking a raw feature directly, bypassing the linear combiner, is what cleared
  the bar. Confirmed via direct code/data check this session: T3's script ranks
...
- **D-02:** Ship v1 wired specifically to `ctf_momentum` — the exact feature and construction
  T3 validated (decile spread, dollar-neutral, both lookahead scales, cost-hurdle-adjusted).
...
### Rebalance cadence: replicate T3's exact per-bar rebalance for v1 validation
- **D-03:** The first shadow-validation pass must rebalance every bar, exactly matching how T3
...
- **D-04:** v1 targets `tf=15m` only, over the full active equity universe (the same 80-symbol
  set, `i.is_active=true AND i.contract_details->>'asset_class'='equity'`, T3's script used).
  No other timeframe or asset class in this phase. T3's temporal-stability check (run this
...
- **D-05:** The construction must be built cost-aware from the start, not as an afterthought.
  This session already ran the todo-030-convention cost-hurdle sweep directly against T3's
...
- **185 — `ic_math.py` causal per-entity demeaning primitive** (filed this session): not
  directly required by T3's construction (T3 ranks, it doesn't pool absolute per-symbol
  predictions the way T5's non-linear combiner does), but the shadow-measurement/monitoring
...
- **186 — `ic_math.py` cross-sectional block-bootstrap gap** (filed this session): directly
  ...
  version T5's rigor pass used ad hoc. Planner should scope calibrating and landing this
...
- `docs/research/data-edge-source-thesis.md` — T3 section (today's validated result, both
  ...
  input), T5 section (the sibling non-linear-combiner candidate — informative, not this
  phase's scope)
...
- `services/counterfactual_tracker.py` — `frame_gate_passes`/`evaluate_frame_gate` (the
  day-clustered BCa/CLT bootstrap machinery T3's script already reuses verbatim — same pattern
...
- `.planning/STATE.md` Current Focus section — the fork this phase resolves (T2 dead per todo
  179, T3 the chosen path forward) and the standing caveat that T2's death is still provisional
  pending todo 183's corpus recompute (does not affect T3's own result, which has no regime
  dependency)
...
- `services/counterfactual_tracker.py`'s `frame_gate_passes`/`evaluate_frame_gate` — day-
  clustered BCa/CLT bootstrap gate evaluation, already proven correct and already used by T3's
...
this session's T3 validation work — this phase exists specifically because that prior research
