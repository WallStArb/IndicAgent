...
tech-stack:
  ...
  patterns:
    ...
    - "Deterministic (feature_value, symbol) tie-break, recorded in the docstring as an intentional reproducibility divergence from the T3 proof script"
...
key-decisions:
  - "Flat equal-weight legs, no vol-scaling — built exactly what T3's proof script measured, not the design doc's aspirational vol-scaled version (Pitfall 1)"
  - "Deterministic (feature_value, symbol) ascending tie-break in decile_legs, recorded as an intentional reproducibility divergence from the T3 script's row-order-dependent pandas sort_values tie order"
...
**Five pure construction primitives (decile split, dollar-neutral spread, leg turnover, live cost-hurdle sweep, APR range validation) productionizing T3's proven ranking mechanic, with a deterministic tie-break and a loud non-finite-value guard the proof script never needed.**
...
## Intentional Divergences from the T3 Proof Script (for Plan 06 Task 3's write-up)
...
1. **Deterministic `(feature_value, symbol)` tie-break** (design decision 2). The T3 script
...
2. **Missing/non-finite feature values raise `ValueError`** (design decision 5). The T3 script
...
Neither divergence changes what T3 measured — both are pinned by test and documented as
