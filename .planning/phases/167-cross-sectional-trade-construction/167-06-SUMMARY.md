...
provides:
  ...
  - "docs/research/data-edge-source-thesis.md cross_sectional_relative_value section updated from falsification-script to productionized-and-gated"
...
- Four equivalence tolerance bands, pre-registered BEFORE the backfill ran, all PASSED against
  the cross_sectional_relative_value falsification script's originally published values (turnover mean/median, gross spread
...
**Pre-registered tolerance bands** (each +/-20% relative of cross_sectional_relative_value's originally published
full-history values):
...
silently sorted; (4) Gate 1 is evaluated on the OOS segment while the cross_sectional_relative_value script reported
...
matching the cross_sectional_relative_value script's own filter set) is 1,601,884 rows / 24,925 distinct `bar_ts` -- a large,
fully explained divergence from the naive count (see Deviations below), not a corpus-availability
problem. Confirms this is the same corpus cross_sectional_relative_value measured: 24,925 distinct eligible bars is within
one of cross_sectional_relative_value's own published full-history `n_bars=24,924`.
...
- `docs/research/data-edge-source-thesis.md` - cross_sectional_relative_value section updated to record productionization
...
indicate any construction defect: the productionized service reproduces cross_sectional_relative_value's own measured
...
- The plan's own Task 1 `<verify>` automated command literally asserts
  ...
  the true value matches cross_sectional_relative_value's own published measurement almost exactly.
