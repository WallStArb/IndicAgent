...
must_haves:
  truths:
    ...
    - "All 13 alpha.ensemble.* and alpha.quant.threshold.* APR keys are seeded and loadable via ConfigService"
...
<objective>
Build the Phase 139 foundation: DB schema (three v3.0 tables), APR seeds (13 keys), the pure-function ensemble math library (including LW cluster deflation), the Kafka topic key, and OTel metric definitions — all unit-tested and independent of corpus data.
...
Output: Migration 168, `src/intelligence/ensemble/` module (5 pure-function files including cluster deflation), `topic_alpha_events()` in stream_keys.py, ensemble/alpha OTel gauges in metrics.py, and `tests/unit/test_ensemble_math.py`.
</objective>
...
<task type="auto">
  ...
  <action>
    ...
    - alpha.ensemble.max_cluster_correlation float 0.80 min 0.10 max 0.99 [initial_estimate] — pairwise LW correlation threshold above which two features are considered the same cluster. Features exceeding this threshold share weight within the cluster; the cluster's total weight is then capped at max_cluster_weight. Prevents double-counting when multiple momentum z-scores or similar features are collinear. NOT an ML learning target (recalibrate by examining LW correlation matrix post corpus run).
    - alpha.ensemble.max_cluster_weight float 0.40 min 0.10 max 1.0 [initial_estimate] — maximum total weight a correlated cluster can receive after LW cluster deflation. A cluster of 3 highly-correlated momentum features will be capped at 0.40 total regardless of how many features it contains. NOT an ML learning target.
    - alpha.ensemble.top_features_count int 10 min 1 max 61 [operator_controlled] — number of top-contributing features to include in alpha_events.top_features JSONB. Features ranked by abs(weight) descending after cluster deflation. Default 10 balances traceability with payload size. NOT an ML learning target.
    - alpha.ensemble.ci_independence_assumption str 'acknowledged' (text type, no min/max) [operator_controlled] — sentinel that confirms the CI propagation formula (var = dot(weights**2, sigma_ic**2)) assumes independent IC estimation errors across features. Correlated feature clusters produce correlated IC estimates; actual CI width is wider than computed. Cluster deflation (max_cluster_correlation, max_cluster_weight) mitigates this at the weight level. Full fix (var = w^T Sigma_IC w using full LW covariance) is deferred; this key forces a conscious confirmation that the approximation is intentional. NOT an ML learning target.
    ...
    Section 3 — config_state INSERTs (config_key, config_value, version=1) for the same 13 keys with values matching defaults (min_passing_features = '5', max_cluster_correlation = '0.80', max_cluster_weight = '0.40', top_features_count = '10', ci_independence_assumption = 'acknowledged'); ON CONFLICT (config_key) DO NOTHING.
  ...
  <verify>
    ...
    PGPASSWORD=postgres psql -U postgres -h localhost -d indicagent -c "SELECT count(*) FROM config_state WHERE config_key LIKE 'alpha.ensemble.%' OR config_key LIKE 'alpha.quant.threshold.%'" returns 13.
    ...
    PGPASSWORD=postgres psql -U postgres -h localhost -d indicagent -c "SELECT config_value FROM config_state WHERE config_key = 'alpha.ensemble.max_cluster_correlation'" returns '0.80'.
    PGPASSWORD=postgres psql -U postgres -h localhost -d indicagent -c "SELECT config_value FROM config_state WHERE config_key = 'alpha.ensemble.ci_independence_assumption'" returns 'acknowledged'.
  ...
  <acceptance_criteria>
    ...
    - SELECT count over config_state for the two namespaces returns exactly 13
    - SELECT config_value WHERE config_key = 'alpha.ensemble.min_passing_features' returns '5' (not '3')
    - SELECT config_value WHERE config_key = 'alpha.ensemble.max_cluster_correlation' returns '0.80'
    - SELECT config_value WHERE config_key = 'alpha.ensemble.ci_independence_assumption' returns 'acknowledged'
    - SELECT config_value WHERE config_key = 'alpha.ensemble.top_features_count' returns '10'
...
<task type="auto">
  ...
  <action>
    ...
    weights.py — three functions.
      ...
      cluster_deflate_weights(weights: np.ndarray, corr_matrix: np.ndarray, max_cluster_corr: float, max_cluster_weight: float) -> np.ndarray: given the post-cap weight vector and the LW correlation matrix (n_features × n_features), find all clusters of features where pairwise correlation > max_cluster_corr using a greedy union-find (merge any pair exceeding the threshold). For each cluster whose total weight exceeds max_cluster_weight, scale each member's weight down proportionally so the cluster sums to max_cluster_weight. Return a renormalized weight vector (sum to 1.0, or zeros if all deflated to zero). This is the actual quant use case for LW in ensemble construction: preventing double-counting of collinear features (e.g., three momentum z-scores at different lookbacks that all carry the same signal). Parameters come from APR keys alpha.ensemble.max_cluster_correlation and alpha.ensemble.max_cluster_weight.
      effective_n(weights: np.ndarray) -> float: return 1.0 / float(np.sum(weights ** 2)) computed on the post-cap post-deflation post-renorm vector; return 0.0 if sum of squares is 0.

    alpha_score.py — function compute_alpha_score(feature_values, weights, ic_signs, ic_ci_lower, ic_ci_upper) -> tuple[float, float, float]: alpha = sum(weights * ic_sign * safe_feature_values) where non-finite feature values are treated as 0; CI variance = dot(weights**2, ((ci_upper - ci_lower)/3.92)**2) with non-finite CI widths treated as 0; margin = 1.96 * sqrt(var); return (alpha, alpha - margin, alpha + margin). Per RESEARCH.md Pitfall 3, do NOT z-score the composite again — feature values are already z-scores. Module docstring must include: "CI bounds assume independence of feature IC estimation errors (var = dot(weights**2, sigma_ic**2)). For correlated feature clusters, actual CI width is wider than computed — cluster deflation (see weights.cluster_deflate_weights) mitigates this at the weight level. The ci_independence_assumption APR key acknowledges this approximation. Full fix (var = w^T Sigma_IC w) is deferred pending corpus calibration."
  ...
  <acceptance_criteria>
    ...
    - cluster_deflate_weights exists in weights.py: when two features have correlation > max_cluster_corr AND their combined weight exceeds max_cluster_weight, the cluster's total weight is capped at max_cluster_weight
    - cluster_deflate_weights output sums to 1.0 (within 1e-6) after renorm (unless all weights deflated to zero)
    ...
    - alpha_score.py module docstring contains `ci_independence_assumption` and the word `independence` (acknowledging the CI variance formula assumption)
...
<task type="auto">
  ...
  <acceptance_criteria>
    ...
    - `.venv/bin/pytest tests/unit/test_ensemble_math.py -q` exits 0 with at least 13 test cases (11 original + 2 cluster deflation tests)
    ...
    - cluster_deflate_weights deflation test: two features with corr=0.95 and combined weight 0.60 are deflated to total 0.40; output sums to 1.0
    - cluster_deflate_weights no-op test: two features with corr=0.10 are NOT deflated regardless of combined weight
  </acceptance_criteria>
</task>
...
<verification>
- Migration 168 applied: three tables present, two hypertables, 13 APR keys in config_state
- alpha_events PRIMARY KEY is (event_id, bar_ts) — verified via \d alpha_events
- alpha.ensemble.min_passing_features = '5', max_cluster_correlation = '0.80', ci_independence_assumption = 'acknowledged' in config_state
- src/intelligence/ensemble/ pure module imports with no DB/Kafka dependency
- weights.py exports derive_weights, cluster_deflate_weights, effective_n
- topic_alpha_events() and 7 OTel instruments importable
- `.venv/bin/pytest tests/unit/test_ensemble_math.py -q` green (>= 13 tests)
...
- 13 alpha.ensemble.* / alpha.quant.threshold.* APR keys seeded and loadable; min_passing_features = 5, max_cluster_correlation = 0.80 (success criteria 3, 6)
- effective_N = 1/sum(w^2), 0.20 per-feature weight cap, and LW cluster deflation implemented as pure tested functions (success criteria 1, 5)
