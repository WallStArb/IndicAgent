...
<task type="auto">
  ...
  <action>
    ...
    - alpha.ensemble.min_passing_features int 3 min 1 max 61 [initial_estimate]
    ...
    Section 3 — config_state INSERTs (config_key, config_value, version=1) for the same 9 keys with values matching defaults; ON CONFLICT (config_key) DO NOTHING.
...
- 9 alpha.ensemble.* / alpha.quant.threshold.* APR keys seeded and loadable (success criteria 3, 6)
