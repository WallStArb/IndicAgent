...
- `AgentRegistry` — reads `agents.yaml`, resolves classes via `__init_subclass__` self-registration, constructs instances via `SwarmDeps`
- `SwarmDeps` dataclass — typed dependency container (`llm_chain`, `pool`, `settings`) replacing loose constructor kwargs
...
- `BaseSwarmCoordinator._setup()` — calls `self._agents = AgentRegistry.build(self.group, self._swarm_deps)` automatically; all subclasses inherit
...
- Zep episodic memory injection into `SwarmDeps` (Phase 097 adds `memory_client`)
...
**Solution:** `SwarmDeps` dataclass — a typed dependency container:
```python
@dataclass
class SwarmDeps:
...
`AgentRegistry.build()` accepts `SwarmDeps`. Each agent's `__init__` takes `deps: SwarmDeps` and reads what it needs. `mypy` catches misuse at type-check time. When Phase 097 adds `memory_client`, it's added to `SwarmDeps` once — no constructor changes needed across agents.
...
`BaseSwarmCoordinator._setup()` calls `self._agents = AgentRegistry.build(self.group, self._swarm_deps)` automatically after infrastructure setup. Subclasses do not call this explicitly — it is structurally enforced by the base class. Adding a new `BaseSwarmCoordinator` subclass = agents auto-populate from YAML; no registry wiring required.
...
### Claude's Discretion
- Whether `SwarmDeps` lives in `src/core/ai/` or `src/intelligence/ai/` (recommend `src/core/ai/` — it is Ring 0 infrastructure)
...
- `src/intelligence/ai/base_group_service.py` — `BaseSwarmCoordinator` lifecycle, `_setup()`, `_shadow_registry_ensure_agents()`
...
- `src/config/settings.py` — `Settings` class (part of `SwarmDeps`)
- `docs/foundation/naming-system.md` — Ring 0/1/2 vocabulary; `SwarmDeps` belongs in Ring 0
...
- `_shadow_registry_ensure_agents()` in `base_group_service.py` — already iterates `self._agents` and enrolls; keep this, just call it after registry-built agents list
...
### Integration Points
- `BaseSwarmCoordinator._setup()` in `base_group_service.py` — the injection point for `AgentRegistry.build()` call
- `AlphaSwarm._setup()` — `super()._setup()` must complete (sets `self._llm_chain`, `self._pool`) before agents are built; `SwarmDeps` is constructed from these post-super values
...
- **Phase 097 extensibility hook:** `SwarmDeps` is designed to accept `memory_client: ZepMemoryClient | None = None` with no agent constructor changes when Phase 097 ships.
