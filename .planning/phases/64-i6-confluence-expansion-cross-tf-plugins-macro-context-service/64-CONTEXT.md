...
- **D-20:** **Cross-asset instrument identifiers** (from todo 011): Define shared constants for FX pairs, rate futures, sector ETFs, factor ETFs in `src/intelligence/macro/constants.py`. NOT in `schemas.py` — schemas.py is for data shapes, not domain configuration. Macro factor plugins import from here — no hardcoded instrument strings. Plan 64-01 creates this file; Plan 64-02 imports from it.
- **D-21:** **Macro instrument availability**: MacroContextComputeAgent subscribes to `market.bars` and filters by `MACRO_ALL_SYMBOLS` set. For macro factors to produce non-None results, the referenced instruments (EURUSD, VX, TLT, ZN etc.) must be in IBKR active contracts. Each factor degrades gracefully when its instruments are absent — this is by design. Deployment note: add macro reference instruments to `src/config/settings.py` alongside trading instruments.
- **D-22:** **MacroContextComputeAgent computation efficiency**: Only trigger full macro recomputation when a macro-relevant instrument's bar arrives (i.e., `symbol in MACRO_ALL_SYMBOLS`). Pass-through all other bars (update bar cache but skip recompute + publish). This avoids O(instruments × macro_factors) compute per bar for non-macro symbols.
- **D-23:** **Automated validation gate**: Plan 64-01 creates `tools/validate_i6_field.py` — a CLI script that queries `signal_ledger`, computes IC for a specified `_shadow` field, and exits 0 if p < 0.05 and N ≥ min_n. Used before executing Plan 64-03: `python tools/validate_i6_field.py --field ctf_momentum_divergence --min-n 30 --alpha 0.05`. Replaces the manual "check DB" step — no human SQL queries required.
