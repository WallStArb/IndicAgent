...
Fix stop placement geometry (A2/A3) so stops are measured from actual entry price, not zone edge. Migrate all 16 hardcoded constants in `trade_framer.py` to APR. Verify `stopped_at_entry` outcome rate drops from ~25% to <5% on a 1-month sample replay.
...
### D-01: A2 three-part stop geometry fix — implement together, causally linked

All three sub-fixes ship in the same plan. They are causally linked: narrow zone → entry at edge → stop immediately behind entry.

**(a) Enforce minimum zone width at all paths:** `MIN_ZONE_WIDTH_ATR = 0.25` exists in zone_engine but fast paths in the feature layer bypass it. Every path that produces a zone must enforce the minimum. Add assertion in zone_engine output: `assert zone_width >= min_zone_width_atr * atr`.

**(b) Stop calculated from entry price, not zone edge:** Current: `stop = zone_low - 1×ATR`. Correct: `stop = entry - max(zone_low - entry, min_stop_atr * ATR)`. Sample data confirms the bug: QQQ entry 723.09 with zone [723.14, 723.16] — entry was below zone_low.

**(c) Emission gate:** `stop_distance = entry - stop`. Gate: `stop_distance >= feature.trade_framer.min_stop_atr * ATR`. Seed value: `1.0` ATR. This is an **ML learning target** — goes into APR, not as a constant.

### D-02: A3 per-asset-class stop geometry — empirical seed values required

Four per-asset-class APR keys for minimum stop floor. Seed values must be computed empirically from actual bar data (`median_ATR / tick_size` per asset class), not guessed:
...
### D-03: A5 APR migration — 16 constants, all in one phase

Full constant table is in `.planning/todos/pending/2026-06-14-trade-framer-apr-migration.md`. Migrate all 16. Per-constant process:
