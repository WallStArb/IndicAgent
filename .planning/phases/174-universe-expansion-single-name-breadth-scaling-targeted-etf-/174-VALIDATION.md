...
created: 2026-09-15
updated: 2026-09-15
revision: mid-execution addition (D-09/D-10, 1d-only down-cap scope + pre-registered correlation gate) — new plans 174-13/14/15 inserted, 174-11/174-12 wave numbers shifted, rows added only for new test obligations
---
...
- **Before `/gsd:verify-work`:** Full suite green, plus Plan 11's manual re-run of the previously-OOM'd 182-symbol `5m/high_bear` cell under the disk-bound path. No Russell-3000-scale backfill/compute proceeds until that measurement lands (enforced structurally: Plan 12 depends on Plan 11, Plan 11 depends on Plan 15's pre-registered correlation gate as well as Plan 09, and `alpha.universe.target_sample_size` stays `0`/crash-loud-unset until Plan 11 sets it — which it may only do after Plan 15's gate records PASS).
...
| 13-T1 | 174-13 | 3 | D-07a, D-09 | T-174-53 | `compute_eligible_1d` additive column, measured (not assumed) backfill for existing rows; `COMPUTE_READY_PREDICATE_SQL` byte-identical, no edit | migration/DB | `psql -tA -c "SELECT column_name FROM information_schema.columns WHERE table_name='instruments' AND column_name='compute_eligible_1d'"` returns 1 row; `git diff production/migrations/337_instruments_governance_split.sql` empty | created by task | ⬜ pending |
| 13-T2 | 174-13 | 3 | D-07a, D-09 | T-174-51, T-174-52, T-174-54 | `compute_1d` dimension structurally excludes 1d-only symbols from `compute`/`live`; the three pre-existing dimension clause strings pinned as literals and unchanged; no path to `live_tradeable` | unit/regression | `.venv/bin/pytest tests/unit/config/test_settings_active_contracts_dimension.py tests/unit/services/test_service_contract_resolution.py -v` | created by task | ⬜ pending |
| 14-T1 | 174-14 | 3 | D-10 | T-174-55, T-174-58, T-174-59 | Diagnostic reproduces the published raw-correlation baseline table to stated tolerances; D-10's two thresholds are literal constants, not APR-backed; residualized (SPY) view computed and reported but never fed to the gate | script | `.venv/bin/python scripts/analysis/universe_expansion_correlation_structure_check.py --tag single_name_equity --json-out /var/tmp/p174_baseline_repro.json` reproduces published figures within tolerance | created by task | ⬜ pending |
| 14-T2 | 174-14 | 3 | D-10 | T-174-56, T-174-57 | Statistics proven against synthetic data with analytically known answers; both thresholds pinned at their boundaries; missing `high_bear` fails closed; distinct exit codes (0/1/2); residualization function proven against a known-beta construction and structurally forbidden from reaching the gate | unit | `.venv/bin/pytest tests/unit/scripts/test_universe_expansion_correlation_structure_check.py -v` | created by task | ⬜ pending |
| 15-T1 | 174-15 | 4 | D-01, D-02, D-10 | T-174-53 | Pilot draw (30-50 symbols) via the sampler, `target_sample_size` still `0` before and after; onboarded `compute_eligible=false` | migration/DB/script | `psql -tA -c "SELECT config_value FROM config_state WHERE config_key='alpha.universe.target_sample_size'"` returns 0 before and after | created by task | ⬜ pending |
| 15-T2 | 174-15 | 4 | D-09, D-10 | T-174-51 | 1d-only backfill; promotion via `compute_1d` dimension only, never the four-timeframe predicate | DB | `psql -tA -c "SELECT count(*) FROM instruments i WHERE i.compute_eligible = false AND i.is_active = true AND (SELECT count(*) FROM backfill_status b WHERE b.symbol = i.symbol) <> 1"` returns 0 | created by task | ⬜ pending |
| 15-T3 | 174-15 | 4 | D-01, D-10 | T-174-55, T-174-56 | Gate run against the pilot; PASS/FAIL verdict document with both thresholds quoted, both figures with margin, union-cohort and residualized diagnostics recorded as explicitly non-gating; FAIL branch writes a pivot recommendation and leaves `target_sample_size` at 0 | script + doc gate | exit code from `--gate` run recorded; `grep -qE "VERDICT:\*\* (PASS\|FAIL)" docs/research/phase174-down-cap-correlation-gate-verdict.md` | created by task | ⬜ pending |
| 11-T1 | 174-11 | 5 | D-04c | T-174-32, T-174-35 | Peak RSS measured and phase-attributed on the exact cell that OOM'd; no ceiling bump | manual + doc gate | `grep -q "5m/high_bear" docs/research/phase174-ic-engine-memory-fix-verification.md` | created by task | ⬜ pending |
| 11-T2 | 174-11 | 5 | D-04c | T-174-33 | Stopgap swapfile removed only if measurements show it is unneeded; fix re-proven without it | manual + CLI | `swapon --show` no longer lists `/swapfile_iceng`, or the document records the measured reason | created by task | ⬜ pending |
| 11-T3 | 174-11 | 5 | D-01 | T-174-31, T-174-34, T-174-45 | Supported count stated as "N symbols, bound by X" from a measured rows-to-cost curve at >=3 sizes with ACHIEVED row counts recorded; the above-corpus point reached by chunk-stream replay under aliased symbol names (not a duplicated `IN` list, which cannot multiply rows); zero result-table writes proven by before/after `count(*)` on `feature_ic_scores`, `feature_ic_scores_history` and `ic_cell_fingerprints`; runs only after Plan 15's gate records PASS | DB + static | `psql -tA -c "SELECT config_value FROM config_state WHERE config_key='alpha.universe.target_sample_size'"` is a positive integer; `grep -ci "insert into" scripts/analysis/ic_engine_cell_scale_harness.py` returns 0; `.venv/bin/ruff check scripts/analysis/ic_engine_cell_scale_harness.py` clean | created by task | ⬜ pending |
| 12-T1 | 174-12 | 6 | D-02, D-03, V5 | T-174-02, T-174-03, T-174-22, T-174-23 | Sample traceable to file sha256 + seed + APR values; zero partial onboardings | DB | `psql -tA -c "SELECT count(*) FROM instruments i WHERE i.is_active AND NOT EXISTS (SELECT 1 FROM instrument_metadata m WHERE m.symbol=i.symbol AND i.compute_eligible=false)"` returns 0 | created by task | ⬜ pending |
| 12-T2 | 174-12 | 6 | D-02 | T-174-38, T-174-47 | Progress measured through `backfill_status`; zero-row fetch treated as failure, not success; backfill launched `setsid`-detached and proven session-independent, with launch/status/restart/reap commands recorded literally and IBC nightly-restart handling stated | DB + CLI | `psql -tA -c "SELECT count(*) FROM backfill_status WHERE status <> 'pending'"` greater than 0, and `ps -o pid,ppid,sid,cmd -p <PID>` shows the run in its own session | n/a | ⬜ pending |
| 12-T3 | 174-12 | 6 | D-01, D-02, D-09 | T-174-28, T-174-01, T-174-37, T-174-46 | Promotion uses `compute_1d` dimension / `COMPUTE_READY_1D_PREDICATE_SQL`, never the four-timeframe predicate (down-cap symbols are 1d-only per D-09); the pre-existing four-timeframe `compute_eligible` promotion path is provably unaffected; nothing live-tradeable; effective breadth explicitly not reclaimed; the three-tier implementation/data/empirical completion table reproduced with reached/not-reached marks | DB + static | `psql -tA -c "SELECT count(*) FROM instruments i WHERE i.compute_eligible_1d = true AND NOT EXISTS (SELECT 1 FROM market_data_ohlcv_tradeable m WHERE m.symbol = i.symbol AND m.timeframe = '1d')"` returns 0 AND `psql -tA -c "SELECT count(*) FROM instruments i WHERE i.compute_eligible = true AND (SELECT count(*) FROM backfill_status b WHERE b.symbol = i.symbol AND b.fetch_complete = true) < 4"` returns 0 | created by task | ⬜ pending |
...
| D-09 | Down-cap sample backfilled 1d bars only, structurally isolated from the four-timeframe compute universe | 174-13, 174-15, 174-12 |
| D-10 | Pre-registered correlation-structure pilot gate (≤0.10 unconditional, ≤0.30 high_bear-conditioned) before the full-scale draw | 174-14, 174-15, 174-11 |
...
|----------|-------------|------------|-------------------|
| Correlation-structure pilot gate verdict | D-10 | Requires a real 1d backfill of a drawn pilot sample and the resulting numbers cannot be known before the pilot runs | Plan 15 Task 3: run `universe_expansion_correlation_structure_check.py --gate` against the pilot cohort; record PASS/FAIL against the two thresholds fixed in Plan 14, before any pilot data existed |
...
| Supported-scale determination | D-01 | Requires real cell computation at multiple sizes; only 231 active symbols exist corpus-wide, so a real 400-symbol peer set does not exist | Plan 11 Task 3, two arms: Arm A runs real cells at ≥3 real sizes (up to the full 231-symbol corpus, across 1d and 5m); Arm B uses a write-free chunk-stream-replay harness (`ic_engine_cell_scale_harness.py`) that reaches a ~2,311-symbol-equivalent 1d cell via aliased symbol names, recording achieved row counts from the accumulator's write offset, never a duplicated `IN` list |
