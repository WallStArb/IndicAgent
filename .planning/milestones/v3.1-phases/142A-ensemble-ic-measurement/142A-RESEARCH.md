...
- **Stratifying on the 4-label `bull/bear/sideways/volatile` regime namespace (OQ-1 trap):** The schema doc (`docs/plans/archive/2026-06-25-v30-alpha-lifecycle-schema.md`) keys `hold_max_bars.*` with these. The LIVE regime system is 9 cross-sectional `market_regimes.regime_label` values. Using the 4-label set means `alpha_ensemble_ic.regime` cannot be joined back to `alpha_events.regime` and the hold_max_bars APR keys have no population path. Use the 9 labels.
...
**What goes wrong:** The schema design doc (`docs/plans/archive/2026-06-25-v30-alpha-lifecycle-schema.md` lines 302-317) seeds `alpha.frame.hold_max_bars.bull.5m`, `.bear.5m`, `.sideways.5m`, `.volatile.5m` — but the LIVE regime system is 9 cross-sectional labels.
...
- `.planning/milestones/v3.1-phases/142A-ensemble-ic-measurement/142A-CONTEXT.md` — locked decisions EIC-01..05, deferred ideas, open questions OQ-1/2/3.
- `docs/research/phase142-redesign-musk5step-audit.md` — the authority on scope (KEEP/DELETE/SIMPLIFY verdicts).
- `docs/plans/archive/2026-06-25-v30-alpha-lifecycle-schema.md` — `alpha_ensemble_ic` DDL, APR key lists (regime namespace STALE — see OQ-1 resolution).
...
### File Created
`/home/bg/dev/indicagent/.planning/milestones/v3.1-phases/142A-ensemble-ic-measurement/142A-RESEARCH.md`
