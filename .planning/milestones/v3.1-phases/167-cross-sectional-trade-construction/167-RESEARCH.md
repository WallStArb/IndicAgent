# Phase 167: Cross-Sectional Trade Construction (cross_sectional_relative_value) - Research
...
- **D-03:** Rebalance every bar, exactly matching cross_sectional_relative_value's measured construction (mean one-way leg
...
| Step 2: Ranking → buckets | Confirmed exact mechanic in cross_sectional_relative_value script's `_decile_spread_per_bar`/`_legs_per_bar` — see Code Examples |
| Step 3: Weights | Doc's v1 calls for vol-scaled equal-weight; cross_sectional_relative_value's proven script uses **flat equal-weight, no vol-scaling** — flagged as a real gap, see Common Pitfalls #1 |
...
| Step 6: Measurement | Portfolio-level, vs. flat and shuffled-ranking null — cross_sectional_relative_value's script already implements the null; the shadow measurement service must persist the same and add the flat-benchmark comparison (currently absent from the script, see Open Questions) |
...
| Validation Gate 3 (comparison to DiscreteTrack directional) | Already satisfied — this comparison IS T2 vs cross_sectional_relative_value, already run and recorded in `data-edge-source-thesis.md` |
...
day-clustered BCa/CLT bootstrap the cross_sectional_relative_value script already reuses verbatim. `services/alpha_scorer.py`
...
(`frame_gate_passes`, `BaseBatch`, `_load_apr_dict_async`/`cfg`, the cross_sectional_relative_value script's own
...
what a *portfolio-level* (not pooled-panel) time series needs, and is what cross_sectional_relative_value's script and
...
demeaning) is also not needed — cross_sectional_relative_value does not pool absolute per-symbol predictions the way nonlinear_interaction_combiner's
...
|---|---|---|---|
| Cross-sectional ranking + decile bucket construction | Compute (Ring 2 batch, in-process) | — | Pure numpy/pandas transform over an in-memory DataFrame, same shape as `_decile_spread_per_bar` in the cross_sectional_relative_value script — no DB round-trip per bar |
...
| `frame_gate_passes` / `evaluate_frame_gate` | `services/counterfactual_tracker.py` | Day-clustered BCa/CLT bootstrap CI, exactly the machinery Validation Gate 1 needs | `[VERIFIED: codebase]` Already used verbatim by the cross_sectional_relative_value falsification script itself — proven correct, no reason to reimplement |
...
| `numpy`/`pandas` | already project dependencies | Cross-sectional rank/decile math, turnover computation | Same libraries the cross_sectional_relative_value script and `alpha_scorer.py` already use |
...
`ic_engine.py`/`ensemble_ic_engine.py` scope to a `weight_epoch`/watermark). The cross_sectional_relative_value falsification
...
**What:** The cross_sectional_relative_value script's `_legs_per_bar()` builds `(short_leg, long_leg)` frozensets keyed by
...
|---|---|---|---|
| Day-clustered bootstrap CI for an autocorrelated per-bar return series | A new bootstrap routine | `frame_gate_passes` (`services/counterfactual_tracker.py`) | Already proven correct (day-clustering handles overlapping-hold-horizon autocorrelation), already reused verbatim by the cross_sectional_relative_value script itself — a second implementation risks silently diverging in a way that changes the gate verdict |
...
which is itself a small, mechanical extension of what the cross_sectional_relative_value script already computes in one
shot.
...
leg doesn't dominate the spread)." The cross_sectional_relative_value script that actually earned this phase (`
...
without vol-scaling.
**How to avoid:** Per CONTEXT.md's own "replicate exactly what cross_sectional_relative_value proved" principle (the
...
**What goes wrong:** A production service that re-runs the cross_sectional_relative_value script's full-history query every
...
**Why it happens:** The cross_sectional_relative_value script was correct for a one-off research pass; copy-pasting it
...
**Why it happens:** Direct port of the cross_sectional_relative_value script's `_legs_per_bar`, which computes turnover
...
(
    ...
    'each leg -- matches cross_sectional_relative_value''s validated _DECILE_FRACTION exactly. NOT an ML learning target.'
),
(
...
(NOW(), 'alpha.construction.decile_fraction', 1, '0.10',
 'migration_260', 'Initial estimate: matches cross_sectional_relative_value''s validated decile fraction [initial_estimate]'),
...
1. Todo 186's actual target is a **pooled multi-symbol panel IC bootstrap** — resampling blocks
   ...
   observations in one panel). This is what nonlinear_interaction_combiner's non-linear combiner rigor pass needed (a
...
2. **cross_sectional_relative_value's construction is a different statistical object entirely.** Once the decile split is
   ...
   problem `counterfactual_tracker.py`'s FRAME-04 gate solves for per-symbol frames, and cross_sectional_relative_value's
   own script already reuses it verbatim with a valid result (`ci_lower`/`ci_upper` computed
   successfully at both lookahead scales, per `data-edge-source-thesis.md`'s cross_sectional_relative_value result table).
...
4. Todo 186 remains real, valuable work — but its consumer is a *future* pooled-panel IC
   estimate (e.g., if nonlinear_interaction_combiner's non-linear combiner or a future multi-feature composite construction
...
absolute per-symbol predictions" (nonlinear_interaction_combiner's non-linear combiner leaked exactly because a
tree-based model implicitly learned each ETF's own long-run drift as a fixed-membership factor
exposure). cross_sectional_relative_value's construction ranks symbols *relative to each other within the same bar* — the
...
1. **Validation Gate 2 (attribution honesty) has no existing code to reuse — how should the
   ...
   **RESOLVED by Plan 167-05:** built from scratch as `--evaluate-attribution`, a static-tilt
   ...
   - What's unclear: whether this belongs in the same `--evaluate-gate` CLI mode as Gate 1, or
     as a separate one-off analysis script (mirroring how the cross_sectional_relative_value falsification itself started
...
2. **Should the flat-benchmark comparison (Minimal Design step 6: "vs. two benchmarks: flat,
   ...
   the cross_sectional_relative_value script only implements the shuffled-ranking null, not the flat benchmark?**
...
- **APR mandate (migrate-as-you-go):** `_DECILE_FRACTION`, `_COST_HURDLE_BPS_ROUND_TRIP`, and
  `_N_NULL_SHUFFLES`/`_SHUFFLE_SEED` in the cross_sectional_relative_value script are all hardcoded module-level constants —
