...
## Issues Encountered
- `echo '!123Angelina' | sudo -S -u postgres psql` failed with "user 'postgres' not found" — PostgreSQL runs inside Docker (timescaledb container), not as a local OS user. Used `docker exec timescaledb psql` with the workspace volume mount instead.
