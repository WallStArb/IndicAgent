...
**Resume File / exact next action:** A Fable 5 dispatch is checking whether rolling-track VP primitives (`poc_price_rolling`/`vah_rolling`/`val_rolling`, computed by the port source `ctx_VolumeProfile` but currently unused in the 10-column scope) should be added — specifically `poc_rolling_dist_atr` and `poc_session_rolling_divergence_atr` as candidates. **Check for that dispatch's result first** (it may have completed and notified after this session ended — look for a message/notification referencing "Phase 163 rolling-track" or re-ask if unclear). If it recommends additions: apply them the same way the two prior corrections were applied (update `163-CONTEXT.md` with a new decision entry, then resume the planner agent — name/ID may have expired across a session boundary, in which case just directly edit `163-01-PLAN.md`/`163-02-PLAN.md` following the same pattern D-17's fixes used). If it recommends no changes: proceed straight to `/gsd-execute-phase 163`.
