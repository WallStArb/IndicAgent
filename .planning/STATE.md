...
*Tier 6 — explicitly gated:* Phases 149-159 (PrecedentEngine, Feature Primitives Expansion, Alt
Data, Portfolio State, Position Sizing, Live Execution, Cost Calibration) — v4.0-adjacent scope.
Per "prove edge before production infra," this gate is now MORE binding for the
portfolio/execution/risk phases (156-159): building that infrastructure before Tier 1 resolves
whether there's a real, tradeable regime-conditional edge would be building on an unproven
foundation twice over. **Phase 151 is the one exception, planned 2026-07-24** (8 plans, 7 waves,
verified) at explicit user direction — it is a measurement/discovery phase (IC-screen new
features), not production infra, the same category `ic_engine`-class work is already exempt from
under "prove edge before production infra." Not yet executed; still an open question whether to
spend the multi-hour corpus recompute before Tier 1's strategic fork (todo 179: invest in better
features vs. accept no OOS edge on the current branch) is resolved with the user.
...
- Phase 166 added (2026-07-23): Frame/Execution Recalibration — the direct follow-on to Phase 148's Gate 2 FAIL verdict, per that phase's own pre-registered "frame problem, recalibrate against IC decay curve" playbook. Formalizes todo 174's investigation scope (never-calibrated `stop_atr_mult`/`target_r_multiple`/`hold_max_bars` APR defaults, `mid_bull`-only OOS coverage question, possible porting of v2.x's structural/regime-adaptive stop hierarchy). Discussed same day: structural candidate scope was broadened mid-discussion to the full v2.x confluence toolkit (zone_engine.py + SMC/swing/fib/anchored-VWAP), then research found the full toolkit's feature columns 100% absent from v3's live schema — resolved to a two-part structural candidate (Part 1: VP/SR confluence, Phase-163-gated, built now; Part 2: SMC/swing/fib/anchored-VWAP, deferred to todo 175). **PLANNED same day** (6 plans, 4 waves, `gsd-plan-checker` VERIFICATION PASSED, decision-coverage gate 6/6). **Executed and COMPLETE 2026-07-23** — verdict: neither candidate (baseline or scalar) promoted, both fail gate166 on drawdown (9.6x/26.2x vs 0.25 ceiling). Direct follow-on investigation (todo 179, same day) found the real cause: `mid_bull`'s raw un-barriered forward return is negative at every horizon — a genuine market-data finding, not a frame/execution artifact.
- Phase 151 planned 2026-07-24 (`/gsd-plan-phase 151`): 8 plans across 7 waves — 28 tier-0 atomic + 5 tier-1 interaction candidates from todos 066/104/123/180 (Wave 1-3), a 10-feature Theory-Motivated Interaction Layer (Wave 5), a historical-corpus recompute + tier-0 IC sweep (Wave 6), and interaction-layer IC screening + regime-conditioned clustering (Waves 2/7). `gsd-plan-checker` VERIFICATION PASSED after 1 revision (1 blocker: wrong test paths in 151-02/151-08, fixed). Corrected two errors in ROADMAP's own text during planning: interaction `parent_features` must be exactly 2 (not `[]`), and every new-column plan must also edit `feature_vector_persistence.py` (2026-07-08 incident precedent) or fields get silently discarded. Planned at explicit user direction ahead of STATE.md's Tier 6 gate — see that tier's note above for the reasoning. Todos 066/123/180 closed same day (content fully captured in the plans).
- Phase 151 cross-AI reviewed and revised, same day (`/gsd-review 151` then `/gsd-plan-phase 151 --reviews`): Codex (this project's configured `review.default_reviewers`) rated the plan set HIGH overall risk with 3 HIGH-severity findings — `151-07`'s `--recompute` mode wasn't failure-atomic per partition, `151-04` shipped a live-path cross-asset bug as a non-blocking todo (Codex disagreed), and the multi-hour recompute/sweep chain was operationally fragile. All three fixed as real plan changes, not just acknowledged: `151-07` gained a partition-completion manifest with atomic tmp+rename writes and a live kill-and-resume drill; a new plan `151-09` (wave 5) was added to actually fix the live-path bug (confirmed real via a live query: 9,183 rows frozen at `vix_z=flight_quality=yield_slope_z=0.0` vs. 139,093 correct batch rows, growing daily — `FeatureVectorPipeline` is `active running` and writes straight into `feature_vectors`, so this wasn't hypothetical); orphan-worker reap hygiene added to 151-07/151-08. 3 MEDIUM findings also addressed (counted overflow guard in 151-06, wall-clock budget check in 151-02, NamedTuple hardening in 151-04/151-05). `gsd-plan-checker` re-verified: VERIFICATION PASSED, 2 non-blocking warnings (NamedTuple keyword-only isn't runtime-enforced, just grep-audited; 151-04 is at the plan-size warning threshold). Now 9 plans total. Not yet executed.

## Session

**Last session:** 2026-07-24T11:30:00.000Z
**Stopped At:** Phase 151 planned (9 plans, 7 waves) and cross-AI reviewed — Codex review (`151-REVIEWS.md`, HIGH risk verdict, 3 HIGH-severity findings) incorporated via `/gsd-plan-phase 151 --reviews`; a new plan `151-09` was added to fix a real live-path bug the review surfaced (`vix_z`/`flight_quality`/`yield_slope_z` frozen at 0.0 for every live-written `feature_vectors` row). `gsd-plan-checker` VERIFICATION PASSED (2 non-blocking warnings). Not yet executed. Earlier same day: Phase 163 executed and verified (15/15 must-haves). Same-day ad hoc investigation (not a GSD phase) found the actual cause of Gate 2's failure and overrode Phase 166's own recommended next step — see the 2026-07-24 session closeout below for the full chain, and `.planning/todos/pending/179-gate166-concurrent-exposure-diagnostic.md` for the complete reasoning trail. Also closed todo 168 (regime-coverage gap, root cause was a compressed hypertable, not a modeling bug) and shipped todo 169 (coverage monitor).

**Resume File / exact next action (updated 2026-07-24, superseding the paragraph below — that
investigation is done, see todo 179's file for the full result: zero cells pass at any
regime/symbol_hmm/scale cut tested):** Do NOT run `/gsd-execute-phase` for anything related to
Phase 166 Part 2 (todos 175/176) — deprioritized, see below. Do NOT run `/gsd-execute-phase 151`
without first resolving the Tier 1 strategic fork with the user (todo 179's own recommendation:
invest in better features/signal — Phase 151/164/165 — vs. accept no OOS-detectable edge on this
branch and reprioritize). Phase 151 is now planned (8 plans) as a candidate first step on the
"invest in features" side, but planning it was not itself that decision.
