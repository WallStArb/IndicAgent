...
last_updated: "2026-07-09T18:26:54.092Z"
progress:
  total_phases: 9
  completed_phases: 4
  total_plans: 24
...
**Current focus (updated 2026-07-09):** Phase 142.5 (Renaissance Primitives) COMPLETE (8/8 plans, 2026-07-07; corrected 2026-07-09 — 89 primitives/150 total `FeatureVector` columns, not 91/152, after `new_high_flag`/`new_low_flag` were proven redundant with `dist_from_high`/`dist_from_low` and removed via migration 211). The corpus rerun to populate the primitives and produce fresh IC went through 5 failed attempts 2026-07-07/08 (write-connection idle-timeout, two OOM incidents, then the 91-column silent-NULL persistence bug — full history in ROADMAP.md's Phase 142A EIC-04 Verdict Log and [Corpus pipeline state](project_corpus_pipeline_state.md)), all root-caused and fixed. **6th attempt is in progress as of this writing (~07:50 EDT):** steps 1-3 (`backfill_feature_factory`, `regime_writer`, `forward_return_writer`) completed clean. Two blockers hit and fixed since: (1) `market_regimes` was empty because `services/equity_regime_model.py` was never wired into the orchestrator's step sequence — fixed by adding it as its own step; `ops_corpus_pipeline_run.sh` is now 8 steps, not 7 (`feature_factory` → `regime_writer` → `forward_return_writer` → `equity_regime_model` → `ic_engine` → `ic_shrinkage` → `ensemble_trainer` → `alpha_publisher`). (2) A separate per-symbol OOM in `ic_engine.py`'s `ProcessPoolExecutor` pass, fixed via a chunked named server-side cursor (commit `e9b3bcde`, migration 212). Both fixes are confirmed live in the currently-running rebuild — `feature_ic_scores` has 920,649 rows (past `ic_engine` cleanly) and it's now in `ensemble_trainer` (step 7). Applied migration 211 in the earlier safe window (no query holding locks on `feature_vectors`/`feature_registry`) before resuming. Check [Corpus pipeline state](project_corpus_pipeline_state.md) or live process/DB state for current status — don't trust this note once significant time has passed.

Separately: Phase 142B.1 machinery is complete (verified 2026-07-04) but the E1/E2 A/B judgment (`ops_ensemble_weight_compare.py`) hasn't been run yet — it now flags a winner's-curse caveat on every WIN verdict (commit `ac9e7f25`, todo 069) since OQ7's peer-group-for-shrinkage question is still genuinely unresolved, not silently ignored. Phase 142B (frame simulation) is still 0/2 plans, blocked on EIC-04 (FAILED 2026-07-03, 0/50 qualifying cells). Phase 144 (Cross-Sectional Regime Model) is now unblocked for `/gsd-discuss-phase` — its HMM weak-separation fallback decision was made 2026-07-08 (see `docs/research/fable-2026-07-07-phase144-conditioning-decision.md`), though its own evidence needs re-measuring once the corpus rerun actually succeeds. A large backlog of Renaissance-refinement proposals (todos 068-085) was filed 2026-07-08 from a full layer-by-layer Fable review (`docs/research/fable-2026-07-07-renaissance-layer-refinements.md`) — none are urgent, most batch into whichever corpus rerun eventually lands. Separately, a large unrelated test-debt cleanup landed 2026-07-09: 22 pre-existing `tests/unit/` failures reduced to 1 (the already-tracked todo-086 false positive) — dead I1-I7 replay code removed from `infrastructure_run_historical_pipeline.py` (3055→1329 lines), a real wall-clock bug fixed in `ops_roll_batch.py`'s `detect_rolls()`, and the binary-pattern scanner's docstring false-positive fixed via proper `tokenize`-based line detection. Todos 045 and 062 closed.
