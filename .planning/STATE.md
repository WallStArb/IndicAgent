...
**Current focus (updated 2026-07-16, verified live):** Phase 143.1 (Measurement and Eligibility Integrity) is 6/8 plans complete; Wave 6 (143.1-07, the full-pipeline corpus re-run) crashed at step 5 on 2026-07-15 ~15:59 EDT after a 30h51m run (`server closed the connection unexpectedly` — a transient dropped connection, not a recurrence of todo 102; no data lost, `feature_ic_scores` had 0 rows at crash time). Resumed 2026-07-15 16:45 EDT via `--from-step 5`, which redoes the full 80-symbol pass from scratch (no intra-step checkpoint into `feature_ic_scores` — gap tracked as todo 121). **Per-symbol pass completed 80/80 at 18:36 UTC 2026-07-16 (verified live); the cross-sectional POOLED pass started immediately after and is in progress as of this update** — duration uncertain, the prior attempt took ~9h for just its first regime group and this pass covers many more (regime × tf) groups than that one. `feature_ic_scores` still 0 rows until this pass flushes. This run also carries todo-096's `ic_sharpe` stride-bias estimator fix (migration 230, commit `d06ac60d`) and closes out todo 102 (idle-session-timeout connection-lifecycle bug, triple-confirmed fixed). Wave 7 (143.1-08, shadow-mode validation + E1-vs-E2 A/B re-run) and Phase 144's D-05 acceptance gate are both `BLOCKED-ON-143.1-07` and unblock together once this run finishes. Same-day, unrelated to this run: todo 084 (ensemble degradation ablation protocol) and todo 087 (`Float32ChunkAccumulator`) both shipped and closed; GSD's `execute-phase` workflow gained a `code_simplifier_gate` (root-cause fix — Phases 142B, 143.1, and 144 had all landed without a `/simplify` pass since GSD's own automation never called it, only manual sessions did); todo 058 (Concept Registry MVP) has a complete implementation plan saved but execution was deliberately deferred behind this P0 work, stays a `pending/` todo, not promoted to a ROADMAP phase.
...
**Next actions, in order (items 1-3 corrected 2026-07-14 - they carried stale pre-renumbering phase IDs and predated the backfill run):** (1) DONE — the `alpha_frames` backfill ran (todo 093 closed) and FRAME-04 was evaluated, failing 16/17 cells on the pre-143.1-fix baseline; re-run it once 143.1-07 completes. (2) SUPERSEDED — what this item called "Phase 146 (I7 Alpha Scorer Transition)" is current Phase 147, cancelled outright 2026-07-14 (its CORPUS-07 gate was never actually built into Phase 141, and topdown D7 had pre-answered retirement-by-default; the "already done" claim here was also wrong, CORPUS-07 never ran - see ROADMAP.md's Phase 147 section). (3) the scoring phase is current Phase 148, retitled "Alpha Scoring System (OOS Proof Gates)" 2026-07-14 with zero I7/v2.x dependency; it still needs ≥60 trading days of closed `alpha_frames` plus the corrected 143.1 corpus before its gates can evaluate. v2.x's own fate (retire vs run as an independent parallel system) is a separate open question, todo 056. (4) todo 026 flags two candidate (non-bug) explanations for the corpus's remaining regime-conditional IC tail worth a look eventually: FDR-tail concentration (expected, not actionable) and P3's still-uncalibrated `equity_regime_model.py` cut points (`vix_low_pct`/`vix_high_pct`/`breadth_bear`/`breadth_bull`, still guessed defaults, never empirically fit) — the latter is a real, bounded, currently-open todo worth prioritizing.
...
**Phase 142B — Frame Simulation + Counterfactual Tracking:** ✅ COMPLETE 2026-07-10 (2/2 plans, verified) — `alpha_frames` hypertable + `AlphaFrameWriter` + `CounterfactualTracker` + frozen `docs/research/concept-promotion-reversion-gate.md`. See "Current focus" above for code review/fix detail.
