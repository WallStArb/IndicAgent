...
3. Attempt 3 (2026-07-08 06:18): killed — exit 137, OOM (cgroup `oom_kill` 1). Reduced `infra.ic_engine.workers` 12→6 as a first mitigation.
4. Attempt 4 (07:05, workers=6): OOM'd again (`oom_kill` 1→2), on `5m/low_bull` specifically — proved worker count was the wrong lever. **Root-caused precisely:** `_compute_cross_sectional_tf` (single-threaded, doesn't use the worker pool at all) holds two full-width float64 copies (`X_raw`, `X_nd`) of the raw feature matrix per (tf, regime) cell; `low_bull` at 5m is ~9.36M rows × 152 features ≈ 20GB+ combined. **Fixed** (`services/ic_engine.py`, commit `95a57806`): switched to float32 — every downstream use only ranks these arrays (`rankdata()`), never operates on raw values, so this is correctness-neutral (empirically verified, max IC diff 1.1e-08) and halves memory. Reverted `infra.ic_engine.workers` back to 12 (config_state, both changes have `config_history` audit entries) since worker count was never the actual cause.
5. Attempt 5 (07:27, workers=6 — already running when the revert landed, unaffected): per-symbol pass finished in ~5 min (vs 53 min before). **Cross-sectional pass passed `5m/low_bull` cleanly** — the exact cell that killed attempts 3-4. Fix confirmed working. Still in progress as of this writing (partway through `5m` regimes); check [Corpus pipeline state](project_corpus_pipeline_state.md) or live process/DB state for current status — don't trust this note once significant time has passed.
