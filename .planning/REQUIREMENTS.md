...
- [x] **DASH-02**: Drill panel shows a Signal Scorecard — all ranked signals for the current bar with confidence, direction, composite rank, regime eligibility, and suppression reason
- [x] **DASH-03**: Suppressed signals display a human-readable suppression label (`< 60% conf` / `< 5 bars` / `wrong regime`)
- [x] **DASH-04**: `GET /api/signals/recent?symbol=&timeframe=&limit=` returns recent signals from `signal_ledger` for drill panel history
...
- [x] **QUAL-02**: Alpha decay multiplier applied in aggregator: repeated same-direction signals from the same setup within `alpha_half_life` bars are down-weighted
- [x] **QUAL-03**: Signal freshness exponential decay applied in `signal_lifecycle_service`: active signal confidence decays as `exp(-λ × bars_since_fire)`
...
- [x] **QUAL-10**: CUSUM performance drift detection — detects when per-setup win rates are degrading relative to historical baseline; alerts before losses accumulate; uses `setup_performance` data (setups with N≥30 already active)
