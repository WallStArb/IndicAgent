...
**Plans:** 8 plans in 7 waves. Waves 1-3 (plans 01-04) are executable NOW — schema, hardening, seed, service port — and have no `alpha_ensemble_ic` dependency. Waves 4-7 (plans 05-08) are GATED: each re-asserts the data gate and aborts if unmet, so `/gsd-execute-phase` cannot open the one-way door early. Ending the phase at plan 07 with the DROP pending on the next corpus run is a correct terminal outcome.
...
- [ ] 170-05-PLAN.md — GATED: H-1 F3 evidence-mass floor viability + M-B live-data rehearsal of the never-executed recording path (scratch DB) [wave 4]
- [ ] 170-06-PLAN.md — GATED: cut `ic_engine`/`ensemble_trainer` over to the concept tables; dual-write shadow mode with a loud divergence assertion [wave 5]
- [ ] 170-07-PLAN.md — GATED: repoint the 8 remaining read-only consumers + replace the operator actuator + repoint 3 test modules [wave 6]
- [ ] 170-08-PLAN.md — GATED (one-way door): assert shadow evidence, remove the dual write, `DROP TABLE` both tables, delete `feature_registry_service.py`, update canonical docs, close todo 118 (migration 285) [wave 7]
