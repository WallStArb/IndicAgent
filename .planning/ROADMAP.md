...
**Plans:** 8 plans in 6 waves. **Gate corrected 2026-08-04**: only Plan 05 (rehearsing
...
Plans:
- [ ] 170-01-PLAN.md — Concept-table schema extensions: `concept_parent` join table + cycle guard, generalized cascade trigger, `is_control`/`control_expectation`/`group_name`, shadow-recovery counters (migration 283) [wave 1]
- [ ] 170-02-PLAN.md — Registry-mechanism hardening: fail-closed FDR enforcement inside `record_comparison_outcome`, non-zero REGISTRY failure exits, challenger-concept validation (L-2/L-3/L-4/L-6) [wave 1]
- [ ] 170-03-PLAN.md — Seed `domain='feature'`: 249 registry+gate rows, 16 lineage edges, genesis transitions, full `feature_transition_log` replay (migration 284) + parity verifier [wave 2]
- [ ] 170-04-PLAN.md — `ConceptRegistryService` sync psycopg lifecycle port: CAS transitions, counter resets, operator-only deprecation, promotion eligibility [wave 3]
- [ ] 170-05-PLAN.md — GATED on `alpha_ensemble_ic`: H-1 F3 evidence-mass floor viability + M-B live-data rehearsal of the never-executed `record_comparison_outcome` path (scratch DB) — blocks only itself [wave 4]
- [ ] 170-06-PLAN.md — Cut `ic_engine`/`ensemble_trainer` over to the concept tables via `record_transition_sync`; dual-write shadow mode with a loud divergence assertion against real `feature_ic_scores` data — not gated on Plan 05 [wave 4]
- [ ] 170-07-PLAN.md — GATED on Plan 06's dual-write parity: repoint the 8 remaining read-only consumers + replace the operator actuator + repoint 3 test modules [wave 5]
- [ ] 170-08-PLAN.md — GATED on Plan 06's dual-write evidence (one-way door): assert shadow evidence, remove the dual write, `DROP TABLE` both tables, delete `feature_registry_service.py`, update canonical docs, close todo 118 (migration 285) [wave 6]
