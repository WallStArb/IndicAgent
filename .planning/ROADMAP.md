...
- 🔄 **v3.4 Edge Proof** - Phases 177-181 (set 2026-09-24; active). Data floor, recompute throughput, a weight-level walk-forward verdict on the cross-asset sleeve and a parallel construction track (the two alpha-generation tracks), cross-asset breadth. Plan: `docs/plans/2026-09-24-edge-proof-program.md`.
...
| 5 | 181 Construction track | H-A/H-B both FAIL 2026-09-24; queue re-ranked by prior (plan's "Phase 181 queue"): TSMOM on the sleeve FAIL 2026-09-24 (419, p 0.22), next short-term reversal (420), then cross-TF divergence | Active, parallel with 179 |
...
### Phase 151: Feature Primitives Expansion + Theory-Motivated Interaction Layer 🔄 IN PROGRESS (7/9 plans, waves 1-5 done 2026-08-05)
...
**Waves 6-7 (corpus recompute + interaction IC sweep) status, updated 2026-09-01:** intentionally paused behind the corpus pipeline finishing rather than running the IC sweep twice — that pipeline (the post-Phase-173 broadcast-significance-correction recompute) completed 2026-08-31. Waves 6-7 are now unblocked; running them is the concrete next step to formally close this phase, not just an audit. Phase 151's own atomic columns already get swept by regular `ic_engine` runs regardless (confirmed via the 2026-08-25 feature-phase audit, `project_n1_nonlinear_combiner_and_feature_phase_audit_2026_08_25` memory: tier-0 atomics cleared gates at 64%), but the formal Wave 6-7 registry-lifecycle verification step itself has never run.
...
- [x] 151-03-PLAN.md — Wave 2: atomics B, 10 `bars_since_*` recency primitives + `abs_ret_autocorr_1` (migration 261, 2 APR keys)
- [x] 151-04-PLAN.md — Wave 3: atomics C, 2 macro spreads + stock-bond correlation + 2 factor betas (migration 262, 7 APR keys)
- [x] 151-05-PLAN.md — Wave 4: interaction layer A, the 5 named roster candidates (3 cross-TF divergences + 2 event flags, migration 263)
- [x] 151-06-PLAN.md — Wave 5: interaction layer B, 10 theory-motivated compounds with stated hypotheses (migration 264, zero APR keys by design)
- [x] 151-09-PLAN.md - Wave 5: live-path cross-asset fix - shared Ring-1 series builder, live/batch parity assertion, once-per-UTC-day refresh (no migration, added by the cross-AI review pass)
...
**Related, recommend sequencing before or alongside:** todo 252 (`ic_engine.py`'s fingerprint invalidation hard-deletes `feature_ic_scores` history with no archive on any code/APR change) — independent bug, but whichever fix it lands on should reuse the same fingerprint (`code_content_key`/`apr_snapshot_key`/`upstream_watermark`) as the provenance key this phase's `concept_transition_log` rows key off of, rather than inventing two unrelated versioning schemes for the same underlying "what changed" question.
**Status (2026-08-04): 6/8 plans genuinely complete, 2/8 correctly BLOCKED — not "phase done."**
Plans 01-04, 06-07 shipped for real: schema (migration 283), registry hardening, seed (migration
284, 249 rows), CAS lifecycle port, `ic_engine`/`ensemble_trainer` dual-write shadow-mode cutover,
and all 8 read-only consumers repointed. **`feature_registry` still exists and is still the table
`concept_registry` is shadow-compared against — the DROP has NOT happened.** Plan 05 (ensemble
rehearsal) correctly BLOCKED on `alpha_ensemble_ic` (0 rows, unchanged). Plan 08 (the DROP)
correctly BLOCKED for a different reason: zero `registry_dual_write_verified` facts exist since
Plan 06's merge — no real `ic_engine` corpus/lifecycle-hook run has happened yet under dual-write
mode (Plan 06 shipped the *code*, not an execution of it). Plan 08 itself surfaced that the
corpus pipeline is separately FATAL-halted at `ops_canary_integrity_assert.py` pending todo 230
(per STATE.md Tier -1) — that's the actual next blocker for producing dual-write evidence, an
issue this phase did not create and cannot resolve on its own. All 8 plans show `[x]` below
because each executed cleanly and produced a SUMMARY (including the 2 that correctly
self-blocked per their own designed gates) — that checkbox means "ran," not "achieved its
business objective." Do not read this phase as closed; re-run Plan 08 once a real corpus cycle
completes under dual-write.
`record_comparison_outcome`, the ensemble_strategy method) needs `alpha_ensemble_ic` — Plans
06-08's earlier inherited dependency on it was an unjustified artifact of mechanical `depends_on`
chaining. Feature-domain governance uses `record_transition_sync` instead (a different method,
ported in Plan 04), proven live by Plan 06's own dual-write comparison against real
`feature_ic_scores` data — which exists today. Worse, `record_comparison_outcome` never exercises
`concept_parent`/the cascade trigger/cycle guard at all (`ensemble_strategy` concepts have no
parent relationships), so Plan 05 passing would have given zero coverage of this migration's
actual highest-risk new code. Waves 1-4 (plans 01-06) are executable NOW. Waves 5-6 (plans 07-08)
depend on Plan 06's dual-write evidence, not on `alpha_ensemble_ic`. Plan 05 alone remains gated
on the corpus rebuild, for its own narrower purpose. Ending the phase at plan 07 with the DROP
...
- [x] 170-04-PLAN.md — `ConceptRegistryService` sync psycopg lifecycle port: CAS transitions, counter resets, operator-only deprecation, promotion eligibility [wave 3]
- [x] 170-05-PLAN.md — GATED on `alpha_ensemble_ic`: H-1 F3 evidence-mass floor viability + M-B live-data rehearsal of the never-executed `record_comparison_outcome` path (scratch DB) — blocks only itself [wave 4]
- [x] 170-06-PLAN.md — Cut `ic_engine`/`ensemble_trainer` over to the concept tables via `record_transition_sync`; dual-write shadow mode with a loud divergence assertion against real `feature_ic_scores` data — not gated on Plan 05 [wave 4]
- [x] 170-07-PLAN.md — GATED on Plan 06's dual-write parity: repoint the 8 remaining read-only consumers + replace the operator actuator + repoint 3 test modules [wave 5]
- [x] 170-08-PLAN.md — GATED on Plan 06's dual-write evidence (one-way door): assert shadow evidence, remove the dual write, `DROP TABLE` both tables, delete `feature_registry_service.py`, update canonical docs, close todo 118 (migration 285) [wave 6]
...
- [x] 171-05-PLAN.md — Run D-01's staged 8-symbol pilot and render the go/no-go verdict (REQ-3, REQ-5)
