...
- **Gap-fill service** — detect + backfill gaps in `market_data_ohlcv` caused by service downtime or TWS disconnects. Query for gaps in 1m series, fetch only missing windows from IBKR, run Stage 2 replay for those windows. Distinct from full historical backfill.
...
- **Parallel DAG execution** — plugins within same tier that share no dependencies could run concurrently (async tasks). Today execution is sequential within each tier. Benefit is mostly for I3–I5 which are `compute_full()` and most expensive. Distinct from per-TF worker refactor (which is about service-level sharding, not intra-stage plugin parallelism).
...
- **Plugin Validation Layer** — Renaissance-aligned validation at service startup: tier list integrity, required attributes (regime_type, outputs, inputs), schema coverage, TREND_SETUPS auto-sync, metrics instrumentation (registered_plugins_total, plugin_validation_status). Hard-crashes on config errors to prevent silent production failures. Phase 1 ready: `/home/bg/.claude/plans/parsed-wobbling-hanrahan.md`. Follow-on: **Plugin Performance Scoring System (Alpha Phase)** — CIS scorer baseline exists; extend to A/B test framework; measure win rate, Sharpe, MAE, P&L per plugin/setup combination; promote statistically significant outperformers via adaptive weights; evidence-based alpha selection (not just absence of errors).

- **Roll Detection Architecture Improvements** — CodeRabbit review suggestions for future roll detection system:
    - **Dedicated roll_events table** — Separate system events from ML feature data. Current design writes roll markers to `intelligence_features.i7` JSONB field, which mixes event data with ML features. Better: dedicated `roll_events` table with `event_ts`, `base_symbol`, `old_contract`, `new_contract`, `roll_gap` columns.
    - **Signed roll_gap convention** — Use `new_close - old_close` with positive/negative semantics instead of separate `roll_gap` (DOUBLE) and `roll_direction` (VARCHAR). Positive = contango (far month at premium), negative = backwardation (far month at discount). Direction is derived from gap sign, not stored separately.
    - **Contract specifications lookup** — Different futures trade different month cycles. `derive_roll_chain()` currently assumes quarterly (H, M, U, Z) for all symbols. Need `FUTURES_SPECS` config: ES/NQ = quarterly, CL = monthly, GC = specific months (G, J, M, Q, V, Z), etc. Validation required before generating symbols.
