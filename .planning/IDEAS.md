...
- **ITR Extension Opportunities**: five ranked candidates for extending the Instrument Tag Registry concept beyond the interaction-primitive idea -- point-in-time history (highest leverage, closes a real lookahead gap), regime-conditioned betas (already designed as TAG-02, just unscheduled), unimplemented measurement types (MI partially scoped, narrower than it first looks), walk-forward re-validation over a tag's life, and cross-tag collinearity auditing. See `docs/ideas/itr-extension-opportunities.md`.
