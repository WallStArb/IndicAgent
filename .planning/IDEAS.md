...
- **Sensitivity x Regime Interaction Primitives**: three ITR-adjacent pipelines (`market_regimes`, ITR's `sensitivity`/`macro_driver` tags, `feature_vectors.equity_beta_z`/`rate_beta_z`) independently measure overlapping macro-sensitivity concepts with zero cross-awareness -- proposes a cheap interaction feature reusing what's already measured instead of a new measurement system. Fable-reviewed 2026-09-18; core finding held, several concrete revisions required before promotion (a real structural lookahead gap, a wrong-tool remediation for the raw-loading trust question). Revision-required, not yet promoted to `docs/research/`. See `docs/ideas/signal-sensitivity-regime-interaction-primitives.md`.
- **ITR Extension Opportunities**: six ranked candidates for extending the Instrument Tag Registry concept beyond the interaction-primitive idea above -- point-in-time history (highest leverage, closes the lookahead gap that doc's Fable review found), regime-conditioned betas (already designed as TAG-02, just unscheduled), unimplemented measurement types (MI partially scoped, narrower than it first looks), walk-forward re-validation over a tag's life, cross-tag collinearity auditing, and instrument-level structural break detection (a company's own identity changing, e.g. MSTR software-to-BTC-treasury -- distinct from TAG-02's market-regime conditioning). See `docs/ideas/itr-extension-opportunities.md`.
