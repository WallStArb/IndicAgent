...
title: Regime Gate Violates Renaissance Signal Data Collection
area: intelligence
priority: 1
tier: immediate
files:
  - services/signal_generator_agent.py
  - src/intelligence/composites/signal_aggregator.py
...
1. **Remove hard regime gate** from signal publish path in `services/signal_generator_agent.py`
...
- `services/signal_generator_agent.py` — find the regime gate logic, likely in `_apply_regime_filter()` or similar
- `src/intelligence/composites/signal_aggregator.py` or equivalent — check where `regime_eligible` suppression happens
- `src/intelligence/trading/signal_ledger.py` — verify `regime_type_at_fire` is in `LedgerEntry.to_insert_params()` (currently at $58 params per CLAUDE.md)
