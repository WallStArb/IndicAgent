...
source: doc-organization session — Controlled Vocabulary had zero todo tracking it at all,
  found while consolidating Concept Registry/Stratification sub-items per user request
---

# Controlled Vocabulary — index

Single entry point for Controlled Vocabulary (Type 3b of `docs/research/concept-governance-registries.md`).
...
None yet — this todo itself is the only tracked item for this system. If/when work starts, split
out a build todo (migration + `VocabularyService` + the 10+ namespace seed) and turn this into a
proper index.
