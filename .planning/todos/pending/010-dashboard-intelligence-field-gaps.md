...
- ~~`adx_14` + `plus_di_14` / `minus_di_14` — trend strength + direction components~~ done
- ~~`volume_ratio` — current vs average volume~~ done
- ~~`roc_14` — rate of change momentum~~ done
- ~~`supertrend_dir` — supertrend direction signal~~ done
- ~~`sma_20_gt_50` — MA cross boolean~~ done
- ~~AO (Awesome Oscillator), AC (Accelerator Oscillator)~~ done
...
- ~~GARCH: `garch_sigma`, `garch_vol_ratio`, `garch_vol_regime`, `garch_shock`~~ done
- ~~Kalman: `kalman_trend`, `kalman_slope`, `kalman_price_position`, `kalman_uncertainty`~~ done
- ~~Session/killzone timing: `in_london_killzone`, `in_ny_killzone`, `minutes_to_ny_open`, `bars_since_session_start`, `is_monday/friday`~~ done (via SMC drill panel)
...
- ~~Demand/supply zones: `nearest_demand_high/low`, `demand_freshness/strength/dist_atr`, `in_demand_zone` (and supply equivalents)~~ done
- ~~Killzones: `in_asia/london/ny_am/ny_pm_killzone`, `killzone_name`, `minutes_until_next_killzone`~~ done
- ~~AMD phase: `amd_phase` (accumulation/manipulation/distribution), `amd_manipulation_detected`~~ done
- ~~Breaker blocks: `breaker_block_active/type/top/bottom/dist_atr`~~ done
- ~~BSL/SSL details: `bsl_dist_atr/touches/significance`, `ssl_dist_atr/touches/significance`~~ done (Phase 28)
- ~~Premium/discount: `price_in_premium`, `premium_discount_pct`, `equilibrium_level`~~ done (Phase 28)
...
- ~~`intelligence_i7:SYMBOL:TF` stream not subscribed in SSE route~~ done
- ~~Plugin competition view: `all_ranked` list with `composite_rank`, `regime_eligible`, `suppression_reason` per plugin~~ done (Signal Scorecard component)
- ~~New SSE domain + SymbolData field + new UI component~~ done
