...
**Correction (2026-07-01):** `momentum_rank_z`, `volume_rank_z`, `volatility_rank_z` are NOT zero-IC — verified live in the DB, they are `NULL` for every row. They have never been computed; todo 013 (Cross-Sectional Rank Features) implements them and hasn't shipped. These 3 should be removed from this todo's scope entirely until todo 013 ships and produces actual values to measure. Re-add them here only if IC comes back at/near zero on real (non-null) data.
...
These 2 ideas are kept here as forward-looking notes for whoever picks up todo 013, since they're improvements to the same feature family — but they are NOT part of *this* todo (033), which only covers features with real, measured IC. Do not act on these until todo 013 ships and produces a baseline measurement.
