...
# `ic_math.py` has a per-symbol circular block bootstrap but no cross-sectional (pooled-panel) variant — every future cross_sectional_relative_value/nonlinear_interaction_combiner-style dollar-neutral test approximates this ad hoc
...
own history." nonlinear_interaction_combiner's within-bar_ts (cross-sectional-neutral) rigor check needed a block bootstrap
...
guessed constant). Any future cross_sectional_relative_value/nonlinear_interaction_combiner-style cross-sectional/dollar-neutral construction test will
...
block a one-off research script; do this once a second consumer (a real cross_sectional_relative_value/nonlinear_interaction_combiner production
candidate, not just the exploratory test) actually needs it.

## References

- `scripts/analysis/nonlinear_interaction_combiner_lightgbm_check.py` — the approximated `block_size`,
