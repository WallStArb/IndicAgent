# Phase 146 gate dangles on unrun work; Phase 147's v2.x retirement half describes retiring an already-dead system

**Renumbered 2026-07-04:** phases 143-152 were renumbered into logical dependency order. What
this todo originally called "Phase 143.5" is now **Phase 146** (I7 Alpha Scorer Transition);
"Phase 144" (Alpha Scoring System + v2.x Retirement Gate) is now **Phase 147**. Body below
updated to match; original finding and scope unchanged.
...
1. **Phase 146's conditional gate cites CORPUS-07, which was never run.** Phase 141 is
   ...
   `docs/analysis/ic-validation-report-58sym.md`. As written, 146's gate can never fire.
2. **Topdown D7 already pre-committed the outcome the gate was waiting for:** "Phase 146
   ...
   ROADMAP's 146 still specs the full I7-01..05 conversion apparatus (3 plan waves, mixing-weight
...
3. **v2.x is dead in fact, not merely deprecated** (bottomup audit §1.1/§5.1): `intelligence-pipeline.service`
   ...
   CLAUDE.md itself marks the whole tier ARCHIVED. Phase 147's SCORE-04 (v3 vs v2.x counterfactual
...
- Phase 146 collapses to one wave: run the CORPUS-07 plugin-capture analysis itself as this
...
- Phase 147: change Depends-on from `i7_conversion_complete = 1` to "Phase 146's CORPUS-07
...
should execute *before* Phase 147 (cheap cleanup, independent of the v3 proof gate) rather than
...
**Action:** get the operator call above, then rewrite Phase 146/147 per the sketch. Not urgent
in the sense that nothing breaks today, but 146 currently can't ever pass its own gate as
