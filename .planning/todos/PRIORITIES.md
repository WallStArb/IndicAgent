...
| [151](pending/151-known-corrupt-ohlcv-print-cleanup.md) | 148's Fix item 3 (verify/correct/tombstone the ~27 known corrupt OHLCV prints) was never done — confirmed still live in `market_data_ohlcv` at 148's closeout (2026-07-19). Backward-looking cleanup, distinct from 149's forward-looking ingestion guard. **Unblocked 2026-07-20** — [152](../completed/152-price-sanity-guard-flags-real-crisis-events.md) shipped and its cross-symbol corroboration signal is now reused here (`MARKET_EVENT` verdict); re-ran the dry-run against live data, CONFIRMED_CORRUPT dropped 27→18 rows, exactly 1 row (`RSP` 2010-05-06) correctly downgraded to `MARKET_EVENT`, `CWB`/`VTV`/`VYM` dropped out of candidacy entirely. **Only remaining step is `--apply`, which needs a human (project owner) to review the CONFIRMED_CORRUPT table first per the script's own design — not agent-executable.** **[147](pending/147-vol-normalized-target-low-bull-divergence.md)'s 2026-07-20 investigation independently found 2 more unambiguous corrupt prints not yet on this list: `DIA` 2009-06-02 (`high=100000`) and `KRE` 2007-09-18 (`high=400` vs `open=44.43`, a distinct occurrence from the already-known 2008-09-18 KRE event) — worth adding before `--apply`.** |
