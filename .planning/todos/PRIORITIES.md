...
| Todo | Gap |
|---|---|
| [165](pending/165-regime-stratified-promotion-criteria.md) | New 2026-07-21, found investigating 143.1-08's HOLD verdict. `alpha.validation.oos_start` is a single fixed window (2025-12-24 onward, a +8.82% SPY rally) -- structurally blind to any edge concentrated in a regime that window doesn't contain. Confirmed empirically: short frames profited during the real COVID crash (+0.0094 avg pnl_r) and were breakeven through the 2022 bear market (-0.0039), but lost money specifically in the rally window actually tested (-0.0230). The HOLD verdict answered "does this help in a rally," not "does this have real regime-conditional edge." Likely affects Phase 144's D-05 and Phase 148's OOS gates too, same mechanism. Needs a real design decision on how per-regime results combine into a verdict.
...
| Todo | Why now |
|---|---|
| [164](pending/164-per-timeframe-ensemble-eligibility-thresholds.md) | New 2026-07-21, sibling finding to todo 165. `1h` produces zero alpha signals (either direction) because `min_passing_features=5`/`max_feature_weight=0.20` are global constants calibrated for 5m/15m's data volume -- live-verified `1h` is NOT power-starved (CI width identical to 15m's) so this is a population-scarcity/threshold-calibration problem, not absent signal. `1d` is a different, genuine small-N power problem needing its own statistical treatment. Needs per-timeframe APR keys, not a blanket parameter change. |
