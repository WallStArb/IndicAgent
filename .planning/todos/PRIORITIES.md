...
| [354](pending/354-context-features-intraday-autocorrelation-per-symbol-pooled-cell.md) | Filed 2026-08-25 (Phase 173 Plan 02). Deleting `ic_engine.py`'s daily-cadence `CONTEXT_FEATURES` path (proven zero live rows lost, pre-deletion gate 0 at 2026-08-25T16:02:30Z) leaves `vix_z`/`yield_slope_z`/`flight_quality` measured only via `_compute_symbol_tf`'s ordinary per-symbol intraday path -- one daily value duplicated across every intraday bar (~78x at 5m), temporal pseudo-replication. A DIFFERENT bug than Phase 173's cross-sectional broadcast-cell fix (that fixes independence across SYMBOLS; this is independence across BARS within one symbol) -- Phase 173's broadcast cell does NOT cover this. Proposed fix: temporal-decimation stride for broadcast features in the per-symbol pooled cell, analogous to `subsample_min_stride`. Not implemented. |
...
| [351](pending/351-alpha-publisher-chunked-inserts-not-in-delete-transaction.md) | New 2026-08-23, found fixing the 2026-08-22 `alpha_publisher` OOM incident. Chunked `alpha_events` INSERTs use a separate pooled connection (`wconn`) from the DELETE's own transaction (`conn`) -- commits independently, before the DELETE commits, weakening the "atomic replace, never a partially-empty table visible to readers" guarantee the code's own comment claims. Pre-existing (the `skip_kafka=True` path had this same split before today's fix; the OOM fix unified both paths onto shared chunking without changing this behavior). Not yet confirmed live-impacting -- needs checking whether anything reads `alpha_events` mid-run today, and what a partial-run crash (some chunks flushed, DELETE not yet committed) actually leaves behind. |
