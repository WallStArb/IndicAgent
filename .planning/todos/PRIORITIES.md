...
| Todo | Gap |
|---|---|
| [091](pending/091-fisher-z-ci-empirical-null-miscalibration.md) | **Confirmed 2026-07-19 against fresh 143.1-07 corpus: SUSPECT rate improved 38% -> 21% (4/19) post-bootstrap-fix, not fully resolved.** 3 of 4 SUSPECT cells share one feature (`ctf_momentum`) — looks feature-specific (block-size vs. autocorrelation decay), not a systemic CI failure. Fixed a schema-drift bug in the confirmation script along the way (`mr.asset_class` -> `mr.regime_group`). Open decision for project owner: accept residual 21% or recalibrate block size for autocorrelation-heavy features — see todo file's new "Confirmation run" section. |
