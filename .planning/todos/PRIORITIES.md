...
| Todo | Why now |
|---|---|
| [240](pending/240-nonlinear-interaction-combiner-baseline-is-single-feature-not-the-linear-ensemble.md) | New 2026-08-03, from a rigor review of the Edge Source Thesis doc. nonlinear_interaction_combiner's pre-registered falsification bar says the tree must beat "the existing linear ensemble"; every run actually compares it to `ctf_momentum` alone (`_nonlinear_interaction_combiner_shared.py:516`, `baseline_feature="ctf_momentum"`). The tested claim is therefore "263-column GBM beats 1 column," not "non-linearity is the bottleneck" -- part of the 4x uplift belongs to input breadth the linear ensemble already has. Add the linear-ensemble arm; keep `ctf_momentum` as a secondary arm since it's what the live tracker ranks by. Gates todo 238. |
| [239](pending/239-nonlinear-interaction-combiner-embargo-passed-in-pooled-panel-rows-not-bars.md) | New 2026-08-03, same review. `_nonlinear_interaction_combiner_shared.py` passes `embargo_bars` into `build_walk_forward_folds(n_valid=len(X))` where `X` is the **pooled** ~80-rows-per-bar panel, so the intended 1-day embargo is 24/96/5 *rows* ≈ 0.3/1.2/0.06 bars at 1h/15m/1d, and fold boundaries split inside a single `bar_ts`. Bounded blast radius (~800 rows of ~2-8.5M, so it does NOT explain the 0.18-0.25 IC) but it is cited in the research doc as a rigor credential and is cheap to fix. Fix at the nonlinear_interaction_combiner call sites only -- `build_walk_forward_folds` is correct where `n_valid` really is a bar count. |
...
via Phase 167/cross_sectional_relative_value's decisive pass (cross_sectional_relative_value cross-sectional long-short passed decisively — see
...
**[188](../completed/188-nonlinear-interaction-combiner-replication-15m-deferred-memory-contention.md) CLOSED 2026-08-03** —
15m replication complete via todo 234's fix: cross-sectional-neutral `point_ic`=0.2506, much
closer to 1h's 0.1822 than 1d's 0.0127 -- nonlinear_interaction_combiner is substantial at the tf Phase 167's live
...
**[231](../completed/231-nonlinear-interaction-combiner-1h-lightgbm-check-oom-corpus-outgrew-script.md) CLOSED 2026-08-02** —
fixed a chunked-fetch OOM (shared fix across all 3 nonlinear_interaction_combiner scripts) and used it to re-verify the 1h
finding under corrected `forward_returns`: holds, magnitude down ~30-40%.

**[234](../completed/234-nonlinear-interaction-combiner-15m-lightgbm-oom-survives-both-prior-fixes.md) CLOSED 2026-08-03** —
