...
| [146](pending/146-lookahead-grid-per-tf-recalibration.md) | `alpha.ic.lookahead.*`'s uniform 1/5/20/60-bar grid confirmed broken on intraday tfs, now confirmed on 1d too — full-corpus re-run (2026-07-20) + a stride-correction fix to the diagnostic itself (Fable 5 caught the CI was computed on serially-dependent, unstrided observations) found 1d's `extended=60` cell also fails (CI half-width > IC point estimate for every horizon ≥20). **Step 3 shipped 2026-07-29 (migration 269) — per-tf grid is live in `config_state`.** But the grid itself for 5m/15m/1h is now PROVISIONAL, not final: [208](pending/208-intraday-same-session-forward-return-gate-inconsistent-with-trade-construction.md) found live 1h `mid` completeness at 53.5% and disputes the "session-boundedness is required by Invariant 1" premise 146's grid was derived under. Do not treat 146's table as locked for those three tfs until 208's empirical check runs — see 146's own Status section for the correction. |
| [208](pending/208-intraday-same-session-forward-return-gate-inconsistent-with-trade-construction.md) | New 2026-07-30: the same-ET-session completeness gate 146 accepted as a hard ceiling for 5m/15m/1h has no analog in `counterfactual_tracker.py`/`alpha_frame_writer.py` (the trade-construction layer is already contiguous/bar-indexed, `hold_max_bars` seeded at 60 bars uniformly even for 1h). Invariant 1's own text doesn't require session-boundedness either — only executable open-to-open pricing, which a cross-session forward return already satisfies. Fable's 2026-07-19 review flagged this exact fork and deferred it pending data; `ops_lookahead_horizon_response.py --allow-overnight` (built this session) is the tool to resolve it empirically, blocked only on the in-flight corpus rebuild repopulating `feature_ic_scores`. Run Step 1 before 146's Step 3 locks in a session-bounded grid that may be leaving real signal on the table. |
...
| Todo | What |
|---|---|
| [207](pending/207-hmm-column-name-collision-k3-k5.md) | New 2026-07-30, found closing todo 205: `hmm_regime_prob`/`hmm_entropy`/`hmm_duration` are written by two different HMMs sharing one column name (regime_writer's BIC-selected K=5 vs FeatureFactory's inline fixed-params K=3). Verdict reached (K=5 authoritative, no ambiguity) and the one live-integrity gap closed by 205's fix — this is a naming/robustness cleanup, not an open measurement-integrity question. |
