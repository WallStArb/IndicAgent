...
| [179](pending/179-gate166-concurrent-exposure-diagnostic.md) | New 2026-07-23, filed same session as a strategic review of Gate 2's FAIL: risk-budget sizing sweep + single-symbol standalone tests + a regime-asset-class-mismatch check (falsified) converge on the same conclusion — the ensemble is long-only-biased and loses money in `mid_bull`/`mid_neutral` at the individual-trade level, universe-wide. Directly connects to the 143.1-08 sign-symmetric HOLD verdict and todo 147. Load-bearing finding, not a minor diagnostic — read before touching Phase 166's frame/execution work further. |
