...
| Todo | Gap |
|---|---|
| [382](pending/382-nightly-backfill-batch-size-conflates-check-cost-with-fetch-cost.md) | New 2026-09-18, found via manual corpus audit. Nightly OHLCV backfill's `batch_size=20` throttles candidate *selection* instead of IBKR *fetch* budget -- 151/233 compute-eligible symbols (65%) silently stuck at the original 2026-08-10/12 freeze despite the 2026-09-16 ranking fix and nightly `status=success` logs. Not a neutral gap: stuck population skews toward longer-tenured/mature symbols, fresh population skews toward recently onboarded ones -- a correlated bias inside the population the in-flight `ic_engine` full-corpus run is training on. Fix + instrumentation shape agreed with user, not yet implemented. Also forces a real status decision on [366](pending/366-live-ingestion-consumer-services-never-restarted-after-gateway-fix.md) (live daemons: reactivate-by-date or archive, no more limbo). |
| [340](pending/340-ihf-5m-feature-compute-zero-row-positive-input-error.md) | **Re-tiered P2→P0 2026-09-18**, corrected same audit as 382. Originally dismissed 11 of 12 failed `backfill_status` cells as "stale checkpoint desync, not real gaps" -- wrong; verified live these are real, severe, multi-year stalled feature-compute backfills for 7 symbols (BIL/VRP/ENPH/GLD/NAD/SHY/STIP, 5-11 years of missing 5m feature history each), all sharing `error_msg='value out of range: underflow'`, likely a `real`-column overflow on a near-zero-variance instrument (BIL already flagged in 341/362 for a related degenerate-HMM finding). `IHF`/`5m` (the 12th cell, distinct error, zero rows) is unaffected by the correction -- still the single bounded case originally described. Don't run `backfill_feature_factory.py` concurrently with the in-flight `ic_engine` run (RAM/CPU contention) -- investigate after it finishes. |
...
| [378](pending/378-vixy-emlc-feature-backfill-then-gate-b-and-portfolio-diagnostic.md) | Updated 2026-09-17 end of session. Gate A PASSED for real (0.0879/0.1309, both under threshold). Feature backfill + forward_returns for VIXY/EMLC: DONE, verified. Unplanned detour also DONE: found+fixed a corpus-wide `ic_engine.py` regime-routing bug (144/273 instruments ambiguously routed, Codex+Fable reviewed, see 379 for the sibling bug deliberately left unfixed). `ic_engine.py` confirmed unblocked (dry-run only). Remaining: real `ic_engine` run → `ensemble_trainer` scope decision (no `--symbols` flag, always full rebuild under a `weight_version`) → Gate B → the portfolio diagnostic. |
