...
| [158](pending/158-above-wk-vwap-frozen-live-path.md) | Found 2026-07-20 incidentally during Phase 163 planning: `above_wk_vwap` is silently frozen at `0.0` for every LIVE bar since v3's inception — `feature_vector_pipeline.py`'s live path never calls `FeatureCache.update_wk_vwap()` (only `compute_batch()` does). Batch/backfill corpus sees real values; any live-path consumer gets a constant false signal. Live/batch computation divergence, not just an unmeasured stub — same DAG-invariant violation class CLAUDE.md flags. Fix is small (add the missing call in `_process_bar_compute`, same call-site pattern Phase 163's 163-02 plan is adding for `update_session_vp()`). |
...
| [147](pending/147-vol-normalized-target-low-bull-divergence.md) | **`ops_vol_normalized_target_ab.py --all-regimes` re-run COMPLETE 2026-07-20** (post-151/154 corrections) — 106 reliable strata, median rank_corr 0.7087 (vs. pre-fix 0.7173/0.351 split reported by 097). `low_bull`'s worst cells (5m/low_bull/20-60 at 0.004/0.001, 15m/low_bull at 0.17-0.29, 1h/low_bull/1 at 0.25) no longer read as a `low_bull`-*specific* anomaly — comparably low rank_corr now appears in other regimes too (e.g. 1h/mid_bull/1 at **-0.018**, 1d/mid_bull/20-60 at 0.01-0.03), consistent with the report's own reliability-floor framing (extended-lookahead/thin-N cells collapsing generally, not a `low_bull`-only defect). **This is encouraging but NOT yet a closed verdict** — todo 147's fix note asked for TWO checks: this A/B rank-corr re-run (done) AND a direct re-check of `true_range_pct`'s per-(tf,regime) CV (the original 10-100x-outlier metric that diagnosed the corrupt prints in the first place) — that second check has NOT been re-run this session. Don't mark 147 closed until the CV numbers themselves are re-pulled and confirmed at parity with other regimes; the A/B rank-corr improvement is suggestive, not the same measurement. Qualifying-feature-set diff this run: raw_only=265, vol_only=4436, both=920 — carries into Plan 07's definitive corpus-wide verdict per the script's own footer. |
