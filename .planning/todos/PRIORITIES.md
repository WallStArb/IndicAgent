...
| [149](pending/149-bar-ingestion-price-sanity-guard.md) | **IN PROGRESS 2026-07-20** — design+plan done (Fable-reviewed), Task 1/6 complete and live-verified (`price_sanity_status` schema, view predicate, APR keys, 18-row reconciliation from 151). Executing in worktree `todo-149-price-sanity-guard`. No price-plausibility check at bar ingestion previously — todo 148's `forward_returns.return_{scale}_suspect` guard protects the measurement layer, but every other OHLCV consumer still inherits corrupt IBKR prints unguarded. |
...
| [154](pending/154-dia-kre-corrupt-print-cleanup-residual.md) | New 2026-07-20, filed when [151](../completed/151-known-corrupt-ohlcv-print-cleanup.md) closed: todo 147's investigation independently found 2 more corrupt prints (`DIA` 2009-06-02, `KRE` 2007-09-18) not covered by 151's 18-row `--apply` batch (different symbols than the 4 151 scanned). Small, same tooling/procedure 151 already documented. |
