...
**Effectively empty as of 2026-07-22** — the two rows below are both already resolved/
downgraded (kept for record, not real open items); todo 160 (the last genuine P0) closed
2026-07-22, see `completed/160-vwo-dia-kre-corrupt-prints-uncorrected.md`.
...
| [176](pending/176-feature-vectors-historical-backfill-new-structural-columns.md) | New 2026-07-23, found executing Phase 163: migration 255's 17 new `feature_vectors` VP/SR columns are NULL on every pre-existing historical row, and `FEATURE_VECTOR_INSERT_SQL`'s `ON CONFLICT DO NOTHING` means a naive backfill re-run silently skips them — confirmed blocker, not theoretical. Direct hard gate on todo 175 / Phase 166 Part 2 (structural candidate rescoring needs real historical VP/SR values, the exact 100%-NULL wall Phase 166 Part 1 already hit). |
