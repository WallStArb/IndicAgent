...
| [133](pending/133-cross-sectional-bootstrap-threads-not-per-tf.md) | `cross_sectional_bootstrap_threads` is a single scalar (=6, benchmarked only for the 5m worst case) applied uniformly to every tf, unlike `bootstrap_block_size` which is already per-tf. Needs a benchmark run + new migration — deferred until 143.1-07's DB/CPU load clears. |
