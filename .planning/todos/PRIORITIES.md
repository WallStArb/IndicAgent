...
| [378](pending/378-vixy-emlc-feature-backfill-then-gate-b-and-portfolio-diagnostic.md) | New 2026-09-17. Gate A run for real for the first time — PASSED (0.0879 unconditional / 0.1309 high_bear, both under threshold, all 13 symbols retained). Checking Gate B readiness found VIXY/EMLC have zero `alpha_events` (onboarded 2026-09-16, feature-compute stage never triggered) — feature backfill running in background as of filing. Remaining chain before the new portfolio-covariance diagnostic (merged 2026-09-16, never run against real data) can produce a real result: feature backfill → forward_returns → ic_engine/ensemble_trainer → Gate B → the diagnostic itself. |
