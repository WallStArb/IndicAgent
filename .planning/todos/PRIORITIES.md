...
| [148](pending/148-forward-return-corrupt-print-guard.md) | New 2026-07-19, filed by Fable 5's emission-threshold verdict review (`docs/research/fable-2026-07-19-emission-threshold-alpha-verdict.md` Q4): corrupt price prints with real volume (e.g. UUP 5m open=1000 on a $25 ETF, volume=200) pass the tradeable view and produce ±50-370% "executable" forward returns — 27 rows corpus-wide, one of which single-handedly fabricated the largest number in the first EM-CAL sweep (a 1.05% mean-return stratum). Rank-IC layers immune (Spearman); every mean-based consumer (EM-CAL, FRAME-04, Kelly) is not. APR-backed plausibility guard + integrity_monitor flag + ~27-row cleanup; blocks trusting any future mean-return report. |
