...
| [167](pending/167-equity-cross-sectional-vs-symbol-hmm-never-falsifier-tested.md) | **Still BLOCKED as of 2026-07-29, in-progress run stopped mid-flight, not the reason.** The scoped `ic_engine.py --symbols <49 equity symbols>` pass this todo needs was started 2026-07-28, restarted twice (once for todo 182's fix, once deliberately stopped by the user pending a real fingerprint-gate bug — see [198](../completed/198-ic-engine-fingerprint-gate-false-invalidation.md)), and is now stopped, not relaunched. Both blockers that caused the restarts are fixed (182, 198). **Concrete next step unchanged: relaunch the scoped run**, then re-run `scripts/analysis/equity_regime_separation_gate.py` for the real verdict. |
