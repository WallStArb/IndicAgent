...
| [368](pending/368-range-pct-successor-preregistration-decision.md) | New 2026-09-02, after pre-registration 1 (`range_pct_fast_xs_ls_h5`) returned DEAD (`de7c33bb2`; real signal p=0.0010 but market-beta tilt, beta +1.14 / R2 0.75, net-negative at all 9 personal cost combos, 52/231 per-symbol BH-FDR survivors). User decision: design a successor pre-registration (built around per-symbol survivors, answering beta loading + personal costs on 0.45-churn quintile constructions) vs. deprioritize the branch. Includes a verification note that 277/278's completed/ status conflicts with the program doc's workstream-2 framing (resolved same day: neither stale -- 278 is a completed decision todo, the diagnostic is its mandated work). Scope addition: pre-reg-1's script never got cross-AI review (AGY down, Codex quota); any successor reusing its harness, or any decision citing its verdict numbers, gets the review first. |
| [367](pending/367-paper-placement-against-personal-hurdle.md) | Workstream 0c of the personal-scale edge determination program (`docs/plans/2026-09-02-personal-scale-edge-determination-plan.md`). Partially executed 2026-09-02: the flat library screen placed the 5-10d range/vol mass (208 FDR-passing 1d cells, all clear), decision rule 2 fired, `range_pct_fast` @ H=5 falsified DEAD -- branch closed. Re-scoped remainder, consumes existing numbers only: place Phase 148's Gate-1-passing construction against the 0b personal hurdle (0b's own headline says the institutional Gate 2 that killed it measured the wrong trader; feeds todo 368 and decision-gate rules 1/3), record gap_z's explicit verdict, alpha_score residual superseded by workstream 2's stronger diagnostic, N1 residual blocked on structural instability. Construction-level verdicts land in `concept_registry` `domain='construction'`. |
