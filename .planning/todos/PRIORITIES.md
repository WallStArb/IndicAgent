...
| [381](pending/381-equity-regime-model-dead-code-broken-insert.md) | New 2026-09-17, found while scoping todo 379's blast radius. `services/equity_regime_model.py` (Phase 144's deprecated "emergency rollback path") writes `INSERT INTO market_regimes (asset_class, ...)`, but the live `market_regimes` schema has no `asset_class` column at all (only `regime_group`, since Phase 144) -- running this script today would hard-crash. Decide: delete it (no live callers, no tests, broken write path -- there's nothing left to roll back to) or fix it against the current schema and keep it as a real rollback path. Five-minute decision, not a design question. |
...
| [377](pending/377-futures-backfill-needs-continuous-contract-construction-not-just-gateway.md) | New 2026-09-13, surfaced by user correction during universe-expansion scoping — STATE.md's "22 registered futures/FX, zero rows" framing didn't distinguish FX (4 spot pairs, no roll/curve issue, blocked only on `ib-gateway`) from the 18 futures (genuinely blocked on a continuous-contract construction methodology that doesn't exist anywhere in this codebase — `ops_roll_batch.py` only handles live forward rolls, and the existing commodity contango/backwardation regime signal is an ETF-proxy workaround, not real curve data). **Same-session overlap check: 14 of the 22 are redundant with ETFs already in the universe (ES/NQ/RTY/YM vs SPY/QQQ/IWM/DIA, ZN/ZB/ZF/ZT vs TLT/IEF/SHY, GC/SI vs GLD/SLV, EURUSD/USDJPY vs FXE/FXY) — only CL/NG/HG/ZC/ZS/ZW/VX + GBPUSD/USDCHF (9) target genuine gaps.** Not urgent standalone, but must be read before scoping any futures-backfill phase — target the 7-futures/2-FX list, not all 18/4. |
