...
| [160](pending/160-vwo-dia-kre-corrupt-prints-uncorrected.md) | Found 2026-07-20 re-checking todo 147's CV metric post-151/154: 3 corrupt `market_data_ohlcv` rows (`VWO` `high=99999.99`, `DIA` `high=100000`, `KRE` `high=400`) that todo 147 explicitly named for 151/154's correction pass were never actually corrected — verified live, still present. Blocks 147's close; correction mechanism and pattern already established (`price_sanity_status` + recompute), just needs applying to these 3 rows. |
...
| [147](pending/147-vol-normalized-target-low-bull-divergence.md) | **`true_range_pct` CV re-check COMPLETE 2026-07-20 — NOT at parity.** The A/B rank-corr re-run (previously reported complete, 106 strata, median 0.7087) was one of two required checks; this session ran the second (direct per-(tf,regime) CV pull). Result: `low_bull`'s CV improved from 582.9/469.4 to 313.24 (5m) / 279.22 (15m) but is still ~150-300x every clean regime's CV (~1-2) — not resolved. Root cause re-confirmed as the same corrupt-print contamination, but 3 specific rows (VWO/DIA/KRE) that todo 147 itself named for the 151/154 correction pass were never actually corrected — verified live via psql, `market_data_ohlcv` still has the sentinel values. Filed as **[160](pending/160-vwo-dia-kre-corrupt-prints-uncorrected.md)**, blocks 147's close. Separately confirmed a new outlier cluster (1h/mid_bull, 5m/high_bear, 2010-05-06) is the real Flash Crash — legitimate, no action. |
