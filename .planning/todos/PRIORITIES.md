...
| [147](pending/147-vol-normalized-target-low-bull-divergence.md) | **Root cause CONFIRMED 2026-07-20** — `low_bull`'s vol-normalized/raw IC rank-correlation collapse is corrupt-print contamination of `true_range_pct` (the vol-normalization denominator), same class as 148/149/151/152: `low_bull`'s CV is 10-100x every other regime's while its mean is normal — traced to a handful of fabricated OHLCV prints (`VWO`/`UUP`/`XRT` already corrected via 151, closed 2026-07-20; `DIA`/`KRE` split off to [154](pending/154-dia-kre-corrupt-print-cleanup-residual.md)). No separate fix needed here — **151 and 149 (bar-ingestion guard) are both now closed 2026-07-20; the SOLE remaining blocker is 154 landing**, then re-check the CV and re-run the A/B; should resolve as a side effect. |
| [154](pending/154-dia-kre-corrupt-print-cleanup-residual.md) | **Promoted from P2 2026-07-20 — do next.** Sole remaining blocker on 147 (above) fully closing. Small, same tooling/procedure 151 already documented (`DIA` 2009-06-02, `KRE` 2007-09-18, 2 known rows). Run `--apply`, targeted 2-symbol recompute (same shape as 151's, proportionally faster), then re-check 147's CV. |
