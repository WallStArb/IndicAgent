...
| [185](pending/185-ic-math-causal-entity-demeaning-primitive.md) | New 2026-07-26, `/simplify` altitude review of the T5 non-linear-combiner script: a naive pooled-panel model showed a 3x-inflated IC from implicitly learning each symbol's static long-run drift (train/test half-correlation of per-symbol mean return = 0.27) — fixed inline via causal per-symbol demeaning, but the same bug will recur for any future pooled-panel test unless this becomes a named, tested `ic_math.py` primitive. |
