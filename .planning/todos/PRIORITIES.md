...
**One live P0 as of 2026-07-26** — the rest of this section (below) is stale/resolved, kept
for record only.

| Todo | Gap |
|---|---|
| [183](pending/183-ic-engine-max-cell-rows-breached-by-todo092-rebalance.md) | **Fix applied 2026-07-26, resume in progress.** Root cause was two-fold: `market_regimes` wasn't cleanly recomputed (leftover rows from the retired `equity_regime_model.py`, fixed by a real re-run — 776,108 rows written) and the ceiling was extrapolated from a pre-Phase-162-fix incident that no longer reflects current memory shape. Measured empirically instead: true largest cell (equity/5m `mid_neutral`, 2.96M rows) peaks at 11.87GB RSS, well within host headroom. Migration 259 raised the ceiling 1.2M→4M with real margin. `ic_engine --from-step 5` relaunched 2026-07-26 ~18:19 UTC, same weight epoch as the original run. **Do not mark closed until the full run completes** — re-verify via `ps aux \| grep ic_engine` before trusting this note. |
...
| [176](pending/176-feature-vectors-historical-backfill-new-structural-columns.md) | New 2026-07-23, found executing Phase 163: migration 255's 17 new `feature_vectors` VP/SR columns are NULL on every pre-existing historical row, and `FEATURE_VECTOR_INSERT_SQL`'s `ON CONFLICT DO NOTHING` means a naive backfill re-run silently skips them — confirmed blocker, not theoretical. **Deprioritized 2026-07-24 by [179](pending/179-gate166-concurrent-exposure-diagnostic.md)'s decisive finding**: `mid_bull`'s raw, un-barriered forward return is negative at every horizon — no stop-placement candidate (structural or otherwise) fixes a genuinely negative raw return. Don't do this backfill to unblock the structural candidate until 179's real next step (regime-eligibility investigation) is resolved first. |
...
| [167](pending/167-equity-cross-sectional-vs-symbol-hmm-never-falsifier-tested.md) | **Bumped P2->P1 2026-07-27, concretely staged.** Migration 262 applied (`dual_write_symbol_hmm=true` for equity, sibling of 247's rates fix) -- zero-cost config change, does not disturb todo 183's in-flight recompute. Falsifier gate script written and verified: `scripts/analysis/equity_regime_separation_gate.py` (generalized Phase 144 D-05 gate, majority-rule across equity's ~49-symbol universe). Caught and fixed a real bug in its own development -- the obvious `asset_class='equity'` symbol filter is wrong (returns bond/commodity/fx ETFs routed elsewhere); fixed to query `instrument_tags` directly, matching `ic_engine.py`'s actual routing logic. Confirmed correctly BLOCKED (zero symbol_hmm rows for the real universe, as expected). **Concrete next step: once todo 183 exits, run a scoped `ic_engine.py --symbols <49 equity symbols>` pass (not a full rebuild, not concurrent with 183 -- single-writer discipline) to populate the missing data, then re-run the gate for the real verdict.** |

| [182](pending/182-ic-engine-15m-cross-sectional-bootstrap-threads-stale.md) | New 2026-07-25, found live during todo 092's corpus recompute: `alpha.ic.cross_sectional_bootstrap_threads.15m=1`'s config description ("finishes in minutes serially") is empirically false at today's scale (80 symbols, 171 features) — observed 100% single-core CPU, zero active Postgres queries, 121 minutes on one 15m cell alone. Same gap as `162-HUMAN-UAT.md` item 3 (never benchmarked, only assumed from the original 5m-only migration), now with concrete evidence it fails. Cheap to re-benchmark once the current recompute finishes; safe to change (config loads once at process startup, won't disturb the in-flight run). |
...
| [188](pending/188-t5-replication-15m-deferred-memory-contention.md) | T5's 1d replication (2026-07-27) partially confirmed the non-linear-combiner finding but at ~16x smaller magnitude than the original 1h result -- confirmed SMALL not LARGE. 15m (the tf Phase 167's live construction actually trades) is the directly actionable replication, deferred on memory contention with todo 183's concurrent recompute. The `ctf_momentum` 1d-vs-15m sign flip this originally surfaced is resolved -- see [189](pending/189-ctf-momentum-1d-self-referential-htf-not-cross-timeframe.md). |
...
| [193](pending/193-service-registry-no-check-against-archived-units.md) | New 2026-07-27, found closing the `feature_vector_writer` deploy gap: `service_auditor.py`'s `_AGENT_ID_TO_UNIT`/`_DAG_ORDER` had silently pointed a live v3.0 agent at an archived v2.x unit for weeks (masking an 18-day undeployed-writer outage) — second occurrence of this exact collision (Phase 138-P0 partially fixed it once already, missing the value half of the rename). Adjacent to 157 but distinct: needs a registry-vs-archived-unit integrity test, not base-class compliance. |
...
| Todo | What |
|---|---|
| [129](pending/129-ic-engine-short-lived-conn-helper.md) | **Mostly resolved 2026-07-27.** `cross_sectional_regime_model.py`'s local `_connect_db` was a byte-identical drop-in for the existing `connect_db_from_url`, migrated. `regime_writer.py`/`backfill_feature_factory.py` have deliberate, incident-motivated connection configs (session-timeout option; autocommit=True + UUID adapter) a generic helper would silently break -- correctly left alone. `equity_regime_model.py` confirmed dead, not touched. Remaining: `ic_engine.py`'s own local helper, deliberately deferred while todo 183's recompute is live. |
