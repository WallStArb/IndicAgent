...
| Todo | Gap |
|---|---|
| [243](pending/243-ctf-momentum-batch-join-lookahead-bias.md) | New 2026-08-03, found via code review of todo 241's fix. Batch's `ctf_momentum`/`ctf_vwap_align`/`ctf_regime_align` join (`feature_factory.py`'s `bisect_right` against period-start-stamped HTF bars) selects the still-forming HTF bar, not the last completed one -- real lookahead (up to 55min at 5m/15m, up to a full day at 1h; 1d is unaffected, self-referential by design). `ctf_momentum` is Phase 167's live tracker's sole ranking signal -- its passing Validation Gates may have been measured partly on leaked future information. Mechanism confirmed against source; magnitude not yet measured. Per user direction: measure impact before touching the join or triggering any corpus recompute. **Blast radius wider than first scoped -- see 245.** |
| [245](pending/245-nonlinear-interaction-combiner-trains-on-lookahead-contaminated-ctf-momentum.md) | New 2026-08-03, found cross-checking 243 against this session's own 239/240 work -- the two todos never cross-referenced each other. `ctf_momentum` is one of the ~247 trained columns in nonlinear_interaction_combiner's matrix (not excluded), and the tree's own feature_importances_ measured it at 400+ (one of its most-relied-on features) -- so the published "substantial at 1h/15m" result may be partly riding the same lookahead leak 243 found, not genuine non-linear structure. **Verified (not assumed) the live production ensemble is NOT contaminated** -- `ctf_momentum` has zero `ensemble_weights` rows, fails BH-FDR eligibility at every tf. Gates the pending 239/240 re-run at 1h/15m/5m specifically; **1d re-run is safe right now, unaffected.** Cheap diagnostic available: re-run with `ctf_momentum` excluded, see if the tree's uplift survives. |
| [203](pending/203-canary-rng-seed-not-per-symbol-cross-sectional-pseudo-replication.md) | Canary seeding fixed and the "wire into a live check" item turned out to already exist (2026-07-29) -- but the finding exposed a broader, still-open gap: `vix_z`/`yield_slope_z`/`flight_quality` and every session/calendar feature are symbol-invariant (broadcast) at a given `bar_ts`, so a per-symbol block bootstrap or Fisher-z test run on the pooled cross-sectional sample overstates their effective N by ~n_symbols. No significance claim on any broadcast feature can be trusted at face value under the current test. Real methodology design needed (collapse to one row per `bar_ts` before bootstrapping, or a dedicated time-series-only test) -- not yet scoped as its own todo. |

## P1 — High value, quick, fully unblocked

| Todo | Why now |
|---|---|
| [240](pending/240-nonlinear-interaction-combiner-baseline-is-single-feature-not-the-linear-ensemble.md) | From a rigor review of the Edge Source Thesis doc. nonlinear_interaction_combiner's pre-registered falsification bar says the tree must beat "the existing linear ensemble"; every run actually compared it to `ctf_momentum` alone. **Code landed + committed 2026-08-03** (`816032e2`): a fold-local linear-ensemble arm (`fit_linear_ensemble_weights`/`score_linear_ensemble`, reusing `ensemble_trainer.py`'s own weighting primitives) plus a paired-bootstrap PRIMARY VERDICT (tree vs linear), `ctf_momentum` kept as secondary. Independent review caught and fixed 2 blocking issues (features weren't z-scored before weighting; memory footprint too close to this module's prior OOM history) -- both fixed in the same commit. **Re-run at 1h/15m/5m now gated on [245](pending/245-nonlinear-interaction-combiner-trains-on-lookahead-contaminated-ctf-momentum.md)** (the training matrix includes lookahead-contaminated `ctf_momentum`, tree importance 400+) -- **1d re-run is safe and unblocked right now.** Gates todo 238. |
| [239](pending/239-nonlinear-interaction-combiner-embargo-passed-in-pooled-panel-rows-not-bars.md) | Same review. `_nonlinear_interaction_combiner_shared.py` passed `embargo_bars` into `build_walk_forward_folds(n_valid=len(X))` where `X` is the **pooled** ~80-rows-per-bar panel, so the intended 1-day embargo was 24/96/5 *rows* ≈ 0.3/1.2/0.06 bars at 1h/15m/1d, and fold boundaries split inside a single `bar_ts`. Bounded blast radius (~800 rows of ~2-8.5M, does NOT explain the 0.18-0.25 IC) but cited in the research doc as a rigor credential. **Code landed + committed 2026-08-03** (`816032e2`, same commit as 240): new `_pooled_panel_folds()` builds folds over the distinct `bar_ts` index and maps back to row slices; `build_walk_forward_folds` itself untouched. **Re-run gating: same as 240 (see 245) -- 1d safe now, 1h/15m/5m wait.** |
| [238](pending/238-nonlinear-interaction-combiner-ranked-cross-sectional-relative-value-pre-registration.md) | New 2026-08-03, from a user-directed rigor review of Edge Source Thesis next steps. Both cross_sectional_relative_value (proven construction) and nonlinear_interaction_combiner (proven 3-5x-stronger signal) are independently validated at 15m; nobody has tested cross_sectional_relative_value ranked by nonlinear_interaction_combiner's tree score instead of `ctf_momentum` — highest-expected-value untested combination on the doc. Pre-registered falsification design (shuffled null, cost-hurdle, turnover, Gate-2-equivalent factor-attribution, breadth-preservation) written down before running, per this project's own pre-registration discipline. Gated on the in-progress nonlinear_interaction_combiner-at-5m replication landing first (may change which tf(s) to test). |
