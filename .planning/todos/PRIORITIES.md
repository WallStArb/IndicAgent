...
| [160](pending/160-vwo-dia-kre-corrupt-prints-uncorrected.md) | **2026-07-22: root cause was deeper than the original filing too.** `ops_known_corrupt_print_cleanup.py`'s candidate discovery was itself gated behind `forward_returns` suspect flags — structurally blind to any corruption confined to `high`/`low` with a sane `open`/`close` (confirmed: only 18-20 of 320 registered (symbol,tf) pairs were ever discoverable that way). Fixed the discovery mechanism to scan all registered pairs directly (`backfill_status`-sourced, ~320 pairs, cheap) instead of proxying through derived returns. Live full-corpus dry-run under the fix found **40 CONFIRMED_CORRUPT bars across 14 symbols** (not just VWO/DIA) — applied after human review (SPY's $1441.65 spike on a ~$142 bar was the most consequential single find). Full recompute (`backfill_feature_factory.py --compute-only`, `forward_return_writer.py`) for the affected 45 (symbol,tf) pairs in progress, closing this session. |
...
| [147](pending/147-vol-normalized-target-low-bull-divergence.md) | **Third CV re-check in progress 2026-07-22**, now that 160's systemic discovery-mechanism fix has landed (14 symbols corrected and recomputed, not just the 2 originally scoped). If `low_bull`'s CV reaches parity with clean regimes this time, close both 147 and 160 together. Prior state: CV re-check #2 (2026-07-20) improved `low_bull` from 582.9/469.4 to 313.24 (5m)/279.22 (15m) but stayed ~150-300x every clean regime's CV — traced to 160's gap. Separately confirmed a new outlier cluster (1h/mid_bull, 5m/high_bear, 2010-05-06) is the real Flash Crash — legitimate, no action. |
...
| [124](pending/124-market-ohlcv-tradeable-view-tier2-audit.md) | **Fixed 2026-07-21** — `backfill_feature_factory.py`, `regime_writer.py`, `forward_return_writer.py` all migrated to `market_data_ohlcv_tradeable`; boundary + all directly-relevant unit tests pass, ruff/black clean. Recompute of `confirmed_corrupt` rows (todo 160, expanded 2026-07-22 to 14 symbols via a discovery-mechanism fix) in progress. Remaining 11 Tier-2 files still need individual style/DRY-vs-correctness judgment calls, no live reproduction found for any of them yet. |
| [168](pending/168-seven-symbols-zero-per-symbol-hmm-regime-labels.md) | New 2026-07-22, found during the symbol_hmm restoration fix's live verification (Task 4): 7 corpus symbols (`LQD`, `PFF`, `RSP`, `USMV`, `UUP`, `VWO`, `XRT`) have `feature_vectors.regime` NULL for 100% of rows — `regime_writer.py`'s per-symbol HMM has never labeled them, despite decades of bar history each (not a data-sparsity issue). Not root-caused yet. 3 of the 7 (`RSP`/`USMV`/`XRT`) are equity-routed — this silently caps [167](pending/167-equity-cross-sectional-vs-symbol-hmm-never-falsifier-tested.md)'s equity falsifier gate if ever built, unless resolved first. |
| [169](pending/169-no-regime-coverage-completeness-check.md) | New 2026-07-22, filed alongside 168: nothing in the pipeline (not `DistributionDriftMonitor`'s design, not deferred todo 036's causal-fit monitor) checks whether a corpus symbol has ANY per-symbol HMM regime coverage at all — 168's 7-symbol gap went undetected for years because of this exact blind spot. Needs no new infra (a `GROUP BY symbol HAVING count(regime)=0` canary check), doesn't need to wait on Phase 152. |
| [170](pending/170-volatility-pct-substitution-test-never-run-for-rates.md) | New 2026-07-22, found closing Phase 144's D-05 gate: F2 triggered for `rates`/15m/5m (neither HMM nor cross-sectional separates IC there), and the pre-registered next step (a factor-augmented HMM challenger) is explicitly conditioned on confirming `volatility_pct` hasn't already separately passed its own substitution gate — proposed as far back as 2026-07-02/07, never tracked as run. Cheap, zero-schema-change probe; do this before building any new model. |
...
| [167](pending/167-equity-cross-sectional-vs-symbol-hmm-never-falsifier-tested.md) | New 2026-07-21, filed alongside the `rates`-group symbol_hmm restoration fix (migration 247): the same cross-sectional-suppresses-symbol_hmm mechanism has applied to every `equity`-routed symbol (~50+) since routing shipped, but unlike `rates`, no D-05-equivalent falsifier gate was ever built to test whether cross-sectional actually separates IC better for equity — an unproven default masquerading as settled. Needs either an equity-scoped falsifier gate or an explicit reasoned-default decision; not urgent. |
