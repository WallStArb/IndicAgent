...
| Todo | Gap |
|---|---|
| [091](pending/091-fisher-z-ci-empirical-null-miscalibration.md) | Fisher-z analytic CI empirically miscalibrated — 38% SUSPECT rate across strata; this is the exact mechanism behind every BH-FDR/EIC-04 gate in the stack. Bootstrap fix shipped (143.1-01). Corpus re-run (143.1-07) is **in progress**: per-symbol pass completed 80/80, the cross-sectional POOLED pass is now running (duration uncertain, prior first-group alone took ~9h and this covers many more regime×tf groups). Check `logs/ic_engine.log` (`grep symbol_computed\|cross_sectional`) and `ps aux \| grep ic_engine` for current progress. |
...
| [110](pending/110-controlled-vocabulary.md) | Controlled Vocabulary — design complete, previously untracked by any todo. No dependency blocks it. **Registered as ROADMAP Phase 161** (supersedes orphaned Phase 135; renumbered from 160 when Phase 160 was claimed by Concept Registry MVP). Fully unblocked, safe to plan now regardless of the in-progress 143.1-07 corpus rerun. |
