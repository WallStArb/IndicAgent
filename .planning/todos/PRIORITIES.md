...
| [147](pending/147-vol-normalized-target-low-bull-divergence.md) | **`true_range_pct` CV re-check COMPLETE 2026-07-20 — NOT at parity.** `low_bull`'s CV improved from 582.9/469.4 to 313.24 (5m) / 279.22 (15m) but is still ~150-300x every clean regime's CV — not resolved. Traced to a deeper gap than "uncorrected rows": `backfill_feature_factory.py` reads raw `market_data_ohlcv` instead of the `price_sanity_status`-aware `market_data_ohlcv_tradeable` view, so even correctly-flagged corrupt rows still corrupt `true_range_pct`. Filed as **[160](pending/160-vwo-dia-kre-corrupt-prints-uncorrected.md)**, real fix owned by **[124](pending/124-market-ohlcv-tradeable-view-tier2-audit.md)** (reclassified P3->P1). Blocks 147's close. Separately confirmed a new outlier cluster (1h/mid_bull, 5m/high_bear, 2010-05-06) is the real Flash Crash — legitimate, no action. |
...
| [124](pending/124-market-ohlcv-tradeable-view-tier2-audit.md) | **Fixed 2026-07-21** — `backfill_feature_factory.py`, `regime_writer.py`, `forward_return_writer.py` all migrated to `market_data_ohlcv_tradeable`; boundary + all directly-relevant unit tests pass, ruff/black clean. Recompute of the currently-flagged `confirmed_corrupt` rows (todo 160) deliberately held until the concurrent 143.1-08 backfill's DB load clears. Remaining 11 Tier-2 files still need individual style/DRY-vs-correctness judgment calls, no live reproduction found for any of them yet. |
