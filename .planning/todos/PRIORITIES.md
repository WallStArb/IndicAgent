...
| [159](pending/159-feature-cache-advance-bar-not-warmed-from-seeded-history.md) | Found 2026-07-20 via Codex peer review of todo 158's fix: `FeatureCache` is never warmed from the historical bars `_seed_bar_history_from_db()` already loads at startup, so `above_wk_vwap`/`hmm_duration` start cold after every restart (deploy/crash/roll) instead of reflecting available history. `above_wk_vwap` stays wrong for up to ~7 days (until the next ISO week boundary) per restart — same silent-wrong-answer class as 158, narrower and self-bounded. |
| [124](pending/124-market-ohlcv-tradeable-view-tier2-audit.md) | **Reclassified P3->P1, 2026-07-20** — `backfill_feature_factory.py` reads raw `market_data_ohlcv` instead of `market_data_ohlcv_tradeable`, so `price_sanity_status='confirmed_corrupt'` rows still feed feature computation. Proven correctness bug (not style/DRY) via todo 160's live KRE reproduction: correctly-flagged row, `forward_returns` sane, `feature_vectors.true_range_pct` still corrupt. Root-cause owner for 160/147's blocking chain — do `backfill_feature_factory.py` first, then `regime_writer.py`/`forward_return_writer.py` (same exposure pattern, not yet reproduced live), then the remaining 11 Tier-2 files. |
