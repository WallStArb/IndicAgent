...
| [167](pending/167-equity-cross-sectional-vs-symbol-hmm-never-falsifier-tested.md) | **Still BLOCKED as of 2026-07-27, but todo 183 is no longer the reason.** Todo 183's full recompute completed, but its config loaded at process startup (2026-07-26T18:19 UTC) — *before* migration 262 (equity `dual_write_symbol_hmm=true`) landed (2026-07-27T15:10 UTC). Gate script (`scripts/analysis/equity_regime_separation_gate.py`) re-confirmed live: still zero `symbol_hmm` rows for equity. **Concrete next step unchanged: run a scoped `ic_engine.py --symbols <49 equity symbols>` pass** (now that todo 183's recompute has exited — single-writer discipline was the only remaining reason to wait) to populate the missing data under migration 262, then re-run the gate for the real verdict. |
