...
| [367](pending/367-paper-placement-against-personal-hurdle.md) | New 2026-09-02, workstream 0c of the personal-scale edge determination program (`docs/plans/2026-09-02-personal-scale-edge-determination-plan.md`). Evaluate the already-measured signal mass (5-10d range/vol family, N1 residual, alpha_score residual, gap_z, Phase 148's Gate-1 construction) against the program's 0b personal hurdle function -- kills or advances each on paper before any new code. Blocked on 0b's hurdle function existing (same program, in flight). Verdicts land in `concept_registry` `domain='construction'`. Connects to the program's pre-registered decision rule: needs-a-construction verdicts on the 5-10d band trigger exactly one new pre-registered construction; nothing clearing fires the kill criterion. |
