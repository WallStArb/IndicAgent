...
| [211](pending/211-ops-scripts-stale-scales.md) | **Part 1 of 2 DONE 2026-07-30** — `ops_ensemble_ablation.py` migrated to a per-tf `AblationConfig`; also fixed a second, independently-found stale-lookahead-key bug in the same file (same commit, `658378a6`). **Part 2 remaining**: `ops_interaction_primitives_pilot.py` — same `_SCALES` migration, plus check for its own instance of the same stale-key bug class. |
