...
| Todo | Gap |
|---|---|
| [091](pending/091-fisher-z-ci-empirical-null-miscalibration.md) | Fisher-z analytic CI empirically miscalibrated — 38% SUSPECT rate across strata; this is the exact mechanism behind every BH-FDR/EIC-04 gate in the stack. Bootstrap fix shipped (143.1-01). Corpus re-run (143.1-07) **completed 2026-07-19 14:25 UTC** (`ic_engine.run_complete`, status success, 6,820 rows committed, verified against live `logs/ic_engine.log` + manifest mtime — no process running). Next actionable step: re-run the empirical-null confirmation against the fresh `feature_ic_scores` to check whether the 38% SUSPECT rate improved. |
