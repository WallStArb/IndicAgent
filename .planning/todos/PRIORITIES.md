...
| [177](pending/177-bar-history-maxlen-caps-windows-beyond-200.md) | **Step 1 (enumeration) done 2026-07-31** — 22 `FeatureFactoryConfig` fields confirmed >200 bars; 19 genuinely `BarHistory`-capped, 2 (`vix_zscore_window`/`yield_curve_zscore_window`) turned out to be a worse, unrelated dead-code-path bug, split out to [221](pending/221-live-vix-z-flight-quality-yield-slope-z-permanently-zero.md). Steps 2-3 (fix-shape decision + IC verification) still correctly deferred pending corpus rebuild. |
...
| [224](pending/224-commodity-fx-regime-group-reenablement-decision-todo-041.md) | New 2026-08-01, refreshed/filed as its own todo (previously only referenced inline as "todo 041" across ROADMAP.md/decision-log, never a standalone file): `commodity_energy`/`commodity_metals`/`commodity_agri`/`fx` regime groups ship fully implemented but `enabled: false` since migration 222. **Superseded in part by [225](pending/225-multi-vector-systematic-regime-join-hybrid-sensitivity-symbols.md)** — the `OIH`/`XLE`/`XOP`-style collision isn't a routing-precedence question, it's the original design's deliberately-deferred "Job 2" (multi-vector join), now unblocked by Phase 146. `fx` (zero collisions on either job) can still enable standalone; `commodity_energy`/`commodity_metals` should wait on 225, not a precedence hack. |
| [225](pending/225-multi-vector-systematic-regime-join-hybrid-sensitivity-symbols.md) | New 2026-08-01, closing out the 224/todo-041 discussion; **revised same day twice** — first from an originally-proposed discrete multi-membership routing rewrite to a gradient/partial-IC approach once `ic_math.py`'s existing primitives (`partial_spearman_ic`/`magnitude_conditional_ic`) were found to already fit, weighted by Phase 146's calibrated `instrument_tags.weight`; then a minimum-effective-N gate added per conditioning subset (Fix step 5) — same thin-cell spurious-CI failure mode todo 218 already caught live in this corpus, expected to ease as the securities universe scales longer-term but the gate is needed regardless of universe size. Sidesteps `AmbiguousRegimeGroupError`/`symbol_regime_class` entirely (no routing rewrite needed), isn't capped at the 6 hand-built `regime_group` modules, doesn't touch the per-symbol idiosyncratic/HMM regime. |
