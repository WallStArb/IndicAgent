...
| [161](pending/161-counterfactual-tracker-update-throughput.md) | New 2026-07-21, blocks 143.1-08 directly. 3 backfill attempts (~18h cumulative) wrote zero rows — root cause was two real bugs in `counterfactual_tracker.py` (`exe.map()` submission-order stall + one-giant-implicit-transaction flush), both fixed and committed. But live throughput benchmarking after the fix found a THIRD, deeper problem: per-row UPDATE against `alpha_frames` measures 28-84 rows/sec regardless of batching strategy (naive executemany vs. UNNEST bulk), implying ~3-9.5 days for the full 23.16M-row backlog. Root cause not yet confirmed (leading hypothesis: poor buffer-cache locality across 1034 hypertable chunks spanning 2006-2026). Process is currently stopped, not grinding. |
