...
`scripts/analysis/t5_nonlinear_combiner_replication_15m.py` OOM-killed again, 2026-08-03
...
2. A follow-on fix (same session, documented in `_t5_nonlinear_combiner_shared.py`'s
...
`_t5_nonlinear_combiner_shared.py` has both). 15m still hit ~21.7GB anon-rss regardless. X alone
...
we don't have 15m's own number, but `_t5_nonlinear_combiner_shared.py`'s docstring estimates
...
15m remains the directly-actionable tf for T5 (Phase 167's live construction trades there) --
this blocks the one T5 replication that would actually confirm/deny relevance to production,
not just "is T5 real in general" (already answered yes, small, at 1h/1d).
...
**Fix:** `_t5_nonlinear_combiner_shared.py` rewritten (`fetch_training_matrix()` replaces
...
in production; the T5 scripts' own docstring had declined to adopt it on the grounds that the
...
0.0610 (`ci_lower`=0.0593). Much closer to 1h's magnitude than 1d's -- T5 is not uniformly small
across timeframes, it's substantial at both 1h and 15m (the tf that actually matters) and small
specifically at 1d. Full detail: `docs/research/data-edge-source-thesis.md`'s T5 section (v1.8).
