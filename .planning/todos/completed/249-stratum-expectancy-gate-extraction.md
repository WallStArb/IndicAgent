...
**COMPLETE 2026-08-04** -- shipped via Subagent-Driven Development (4 tasks + final
whole-branch review + one fix round), merged to `main` at `8c86cc48`. Final review found the
design doc's consumer census was incomplete (missed `services/alpha_scorer.py` and 3
`scripts/analysis/*.py` files also importing this machinery from `counterfactual_tracker.py`)
-- folded into the fix wave, so all 6 real consumers now import from `gate_math.py` directly,
not just the 2 originally scoped. One residual finding (a re-export comment still omitting one
test-file consumer) parked as non-load-bearing, comment-accuracy-only. `gate_math.py` has zero
live consumers of `evaluate_stratum_expectancy_gate` by design -- ready infrastructure, not
wired to anything.
