...
filed: 2026-07-26
closed: 2026-07-26
source: live diagnostic while running scripts/analysis/t5_nonlinear_combiner_lightgbm_check.py
  (Edge Source Thesis T5 falsification test)
---

# T5's LightGBM combiner shows a suspiciously large OOS IC uplift — needs a canary-leakage check before it counts as evidence

**CLOSED 2026-07-26 — check ran, this specific failure mode is ruled out (not a full pass).**
Built `scripts/analysis/t5_canary_leakage_check.py`, reusing
`t5_nonlinear_combiner_lightgbm_check.py`'s `_train_and_predict_oos`/`_per_symbol_ic_ci`
...
needle; it didn't. Full write-up: `docs/research/data-edge-source-thesis.md`'s T5 section.
...
**What this does NOT prove:** T5's 0.30 result is still ~3x anything else measured in this
...
`scripts/analysis/t5_nonlinear_combiner_lightgbm_check.py` (Edge Source Thesis T5: does a
...
- `scripts/analysis/t5_nonlinear_combiner_lightgbm_check.py` — the script itself, full result
  in its stdout and now `docs/research/data-edge-source-thesis.md`'s T5 section
- `scripts/analysis/t5_canary_leakage_check.py` — the canary check this todo asked for, built
  and run 2026-07-26
- `docs/ideas/measurement-nonlinear-interaction-combiner.md` — T5's design and overfitting
  controls
- `docs/research/data-edge-source-thesis.md` — T5 section, full result including the
