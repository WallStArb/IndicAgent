...
filed: 2026-08-02
closed: 2026-08-02
source: re-running the original T5 1h finding script from a prior session that appeared "hung"
---

## What

`scripts/analysis/t5_nonlinear_combiner_lightgbm_check.py` (the ORIGINAL 1h script that produced
the T5 finding cited throughout `docs/research/data-edge-source-thesis.md` and STATE.md,
...
Added `_fetch_frame_chunked()` to `t5_nonlinear_combiner_lightgbm_check.py`: streams `_FV_SQL`
...
(`t5_nonlinear_combiner_replication_1d.py`, `..._15m.py`) updated to import and use the same
